...
```
C:\Users\gamep\Documents\68030085\asdad\Docker-LabSheet-2025\photo\version.png
---
...
**บันทึกผลการทดลอง**
```
C:\Users\gamep\Documents\68030085\asdad\Docker-LabSheet-2025\photo\nginx.png
```
...
```text
อธิบายคำสั่งที่นี่
1.คำสั่ง docker pull nginx:alpine : ใช้สำหรับดาวน์โหลด Image (ต้นแบบซอฟต์แวร์) จาก Docker Hub มาเก็บไว้ในเครื่อง
2. คำสั่ง docker images : ใช้สำหรับ ตรวจสอบรายชื่อ Image ทั้งหมดที่มีอยู่ในเครื่องคอมพิวเตอร์ของเราในขณะนั้น
3. คำสั่ง docker run -d --name my-nginx -p 8080:80 nginx:alpine คำสั่งนี้คือการสร้างและสั่งให้ Container เริ่มทำงาน โดยมีรายละเอียดส่วนประกอบดังนี้:
-docker run: คำสั่งสร้างและรัน Container ใหม่
-d (Detached): สั่งให้ Container ทำงานอยู่เบื้องหลัง (Background) เพื่อให้ Terminal ยังใช้งานต่อได้
--name my-nginx: กำหนดชื่อให้กับ Container เพื่อให้ง่ายต่อการเรียกใช้งาน (แทนการใช้ ID ยาวๆ)
-p 8080:80 (Port Mapping): เชื่อมต่อพอร์ต 8080 ของเครื่องเรา (Host) เข้ากับพอร์ต 80 ภายใน Container
- nginx:alpine: ระบุชื่อ Image ที่จะนำมาสร้างเป็น Container
4. คำสั่ง docker ps : ใช้สำหรับ ตรวจสอบสถานะของ Container ที่กำลังทำงานอยู่ (Running)
```
...
**บันทึกผลการรันคำสั่ง docker exec -it และ ls /usr/share/nginx/html**
   ...
   ```bash
   บันทึกรูปภาพที่นี่ C:\Users\gamep\Documents\68030085\asdad\Docker-LabSheet-2025\photo\ตรวจสอบ Container.png
   ```
</details>
...
> **📝 บันทึกผล**: Container ID คือ: `1ef9ebb7c27d6272ef6c9125e8eb26e86ebef9d44da2232c22adb81d75064371` และ IP Address คือ: `172.17.0.2`
...
**บันทึกรูปผลการทดลอง**
```text
C:\Users\gamep\Documents\68030085\asdad\Docker-LabSheet-2025\photo\ทดลอง Node.js Container.png
```
---
...
1. `docker ps` กับ `docker ps -a` ต่างกันอย่างไร?
   ...
   > _คำตอบ_: docker ps จะแสดงเฉพาะ Container ที่กำลังทำงานอยู่ (Running) เท่านั้น ส่วน docker ps -a จะแสดง Container ทั้งหมดที่มีอยู่ในเครื่อง ทั้งที่กำลังทำงานและที่หยุดทำงานไปแล้ว (Exited)

2. ขนาดของ `nginx:alpine` image คือเท่าไร?

   > _คำตอบ_: ประมาณ 40-50 MB (สามารถตรวจสอบขนาดที่แน่นอนได้โดยใช้คำสั่ง docker images)

3. flag `--rm` ใน `docker run` มีประโยชน์ในกรณีใด?

   > _คำตอบ_: มีประโยชน์สำหรับการรัน Container ชั่วคราวเพื่อทดสอบหรือรันคำสั่งสั้นๆ เพราะ Docker จะลบ Container ทิ้งให้โดยอัตโนมัติทันทีที่เราออกจาก Container หรือจบการทำงาน ทำให้ไม่เปลืองพื้นที่จัดเก็บ

4. `docker exec -it` ต่างจาก `docker run -it` อย่างไร?

   > _คำตอบ_: docker run -it คือการ "สร้างและเริ่ม" Container ใหม่จาก Image ส่วน docker exec -it คือการ "มุดเข้าไปรันคำสั่งเพิ่มเติม" ใน Container ที่กำลังทำงานอยู่แล้ว
...
6. ต้องการลบ image ชื่อ nginx:alpine จะต้องเขียนคำสั่งอย่างไร หากมีหลายขั้นตอนที่ต้องเขียน ให้เขียนลำดับคำสั่งให้ถูกต้อง
   ```bash
    - docker stop my-nginx (ถ้ามี Container รันอยู่)

    - docker rm my-nginx (ลบ Container)

    - docker rmi nginx:alpine (ลบ Image)
   ```
...
ตอบคำถามที่นี่
```
"คือการแบ่งขั้นตอนการสร้าง Image ออกเป็นหลายช่วง เพื่อแยกส่วนที่ใช้ Build (ไฟล์ขยะ/เครื่องมือ) ออกจากส่วนที่ใช้ Run จริง (เฉพาะไฟล์แอป) ผลลัพธ์คือได้ Docker Image ที่มีขนาดเล็กลงมากและปลอดภัยยิ่งขึ้น"
---
...
> **📝 บันทึกผล**: ขนาดของ booking-backend:1.0 คือ: `456.24 MB (โดยประมาณ)` 
...
```
-e DB_PATH=/app/data/booking.db : คือการกำหนด Environment Variable (ตัวแปรสภาพแวดล้อม) เพื่อบอกโปรแกรมใน Container ว่าไฟล์ฐานข้อมูลเก็บอยู่ที่ตำแหน่งไหน

-v $(pwd)/...:/app/data : คือการทำ Bind Mount (Volume) เพื่อเชื่อมโยงโฟลเดอร์ในเครื่องเราเข้ากับใน Container ช่วยให้ข้อมูลฐานข้อมูลไม่หายไปเมื่อ Container ถูกลบ และเราสามารถแก้ไขไฟล์จากนอก Container ได้ทันที
```
...
```
C:\Users\gamep\Documents\68030085\asdad\Docker-LabSheet-2025\photo\ทดสอบรัน Backend Container.png
---
...
1. `booking-backend:1.0` คืออะไร?
   ...
   > _คำตอบ_: คือ Docker Image ที่ถูกสร้างขึ้น (Build) มาจาก source code ของฝั่ง Backend โดยมีการระบุชื่อ Image (Repository Name) ว่า booking-backend และระบุเวอร์ชัน (Tag) เป็น 1.0 เพื่อใช้เป็นต้นแบบในการสร้าง Container

2. ถ้าต้องการให้ ทดสอบ API ด้วย `curl http://localhost` ต้องทำอย่างไร?

   > _คำตอบ_: ต้องทำการเปลี่ยนการ Map Port ในคำสั่งรัน Docker จากเดิม -p 5000:5000 ให้เป็น -p 80:5000 (เนื่องจากโดยปกติโปรโตคอล http จะเรียกใช้งานผ่าน Port 80 เป็นค่าเริ่มต้น การแมปพอร์ต 80 ของเครื่องโฮสต์เข้ากับพอร์ต 5000 ของคอนเทนเนอร์ จะทำให้เราเรียกผ่าน localhost ได้โดยไม่ต้องระบุเลขพอร์ตต่อท้ายครับ)
...
1. ขนาดของ `booking-frontend:1.0` เทียบกับ `booking-backend:1.0` ต่างกันอย่างไร? เพราะเหตุใด?
   ...
   > _คำตอบ_: - Backend: มีขนาดใหญ่เพราะต้องบรรจุ Node.js Runtime และโฟลเดอร์ node_modules ที่มี Library จำนวนมากเพื่อให้ Server ทำงานได้ - Frontend: มีขนาดเล็กเพราะใช้ Base Image เป็น Nginx Alpine (Linux ขนาดจิ๋ว) และบรรจุเพียงไฟล์ Static (HTML/JS) ที่ถูกคอมไพล์มาแล้วเท่านั้น

2. `location /api/` ใน nginx.conf ทำหน้าที่อะไร?

   > _คำตอบ_: ทำหน้าที่เป็น Reverse Proxy เพื่อดักจับ Request ที่ส่งมายัง Path /api/ แล้วส่งต่อไปยังเครื่องเซิร์ฟเวอร์ Backend ที่ระบุไว้ (เช่น http://backend:5000) ทำให้ Frontend สามารถสื่อสารกับ API ได้ผ่าน URL เดียวกันกับ Web Server

3. ทำไม Frontend Container ถึงใช้ชื่อ `backend` ในการ Proxy ได้?

   > _คำตอบ_: เพราะทั้งสองคอนเทนเนอร์เชื่อมต่ออยู่บน Docker Network เดียวกัน (ในที่นี้คือ booking-network) ซึ่ง Docker มีระบบ Embedded DNS ที่ช่วยให้คอนเทนเนอร์สามารถค้นหาและสื่อสารกันได้โดยใช้ ชื่อคอนเทนเนอร์ (Container Name) แทนการระบุหมายเลข IP Address ครับ
...
1. docker-compose.yml คืออะไร มีประโยชน์อย่างไร?
   ...
   > _คำตอบ_: คือไฟล์ที่ใช้กำหนดโครงสร้างของแอปพลิเคชันที่มีหลายคอนเทนเนอร์ (Multi-container) ในรูปแบบของไฟล์ YAML - ประโยชน์: ช่วยให้เราสามารถสั่งรัน (Start) หรือหยุด (Stop) คอนเทนเนอร์ทั้งหมดที่เกี่ยวข้อง (เช่น Frontend, Backend, Database) ได้พร้อมกันด้วยคำสั่งเดียว (docker-compose up) แทนที่จะต้องพิมพ์คำสั่ง docker run ยาวๆ แยกกันทีละตัว

2. `restart: unless-stopped` หมายความว่าอย่างไร?

   > _คำตอบ_:เป็นนโยบายการรันใหม่ของคอนเทนเนอร์ โดย Docker จะสั่งให้คอนเทนเนอร์ เริ่มทำงานใหม่โดยอัตโนมัติ หากมันเกิดพัง (Crash) หรือเครื่องเซิร์ฟเวอร์ถูก Restart ยกเว้นเพียงกรณีเดียวคือ ผู้ใช้สั่งหยุดทำงานด้วยตัวเอง (เช่น สั่ง docker stop) คอนเทนเนอร์ถึงจะไม่เริ่มใหม่ตอนเปิดเครื่องครั้งหน้า

3. Named Volume `sqlite_data` ต่างจาก Bind Mount อย่างไร? และทำไมต้องใช้กับ SQLite?

   > _คำตอบ_: * ความแตกต่าง: * Bind Mount: เราเป็นคนระบุตำแหน่งโฟลเดอร์ในเครื่องโฮสต์เอง (เช่น ./test-data) ข้อมูลจะผูกกับโฟลเดอร์นั้นๆ ,* Named Volume (sqlite_data): Docker จะเป็นคนจัดการพื้นที่เก็บข้อมูลให้เองในระบบจัดการของ Docker (ปกติจะอยู่ที่ /var/lib/docker/volumes/) ซึ่งมีความปลอดภัยและประสิทธิภาพในการรับ-ส่งข้อมูลสูงกว่า ,ทำไมต้องใช้กับ SQLite: เพราะ SQLite เป็นฐานข้อมูลแบบไฟล์ที่ไวต่อความเร็วในการอ่าน/เขียน (I/O) การใช้ Named Volume จะช่วยลดปัญหาเรื่อง File Locking และประสิทธิภาพที่อาจเกิดขึ้นได้ดีกว่า Bind Mount โดยเฉพาะเมื่อรันบนระบบปฏิบัติการที่ต่างกัน (เช่น Windows vs Linux)
...
**บันทึกรูปผลการทดลอง**
```
C:\Users\gamep\Documents\68030085\asdad\Docker-LabSheet-2025\photo\ทดสอบ Application.png
```
...
1. `docker compose ps` แสดงสถานะ Service อย่างไร?
   ...
   > _คำตอบ_: แสดงผลออกมาเป็น ตาราง โดยดูที่คอลัมน์ State หรือ Status (เช่น Up หมายถึงกำลังทำงาน, Exited หมายถึงหยุดทำงาน) พร้อมระบุชื่อ Container, Command และ Ports ที่ใช้งาน.

2. IP Address ของแต่ละ Container ในผลจาก `docker network inspect` คืออะไร?

   > _คำตอบ_: backend มี IP Address คือ 172.18.0.2
