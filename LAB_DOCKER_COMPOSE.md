...
12ab3de3da44   nginx:alpine   "/docker-entrypoint.…"   1 minute ago    Up 1 minute    0.0.0.0:8080->80/tcp   my-nginx
```
...
- `-d`: (detached) รัน container แบบ background ไม่ต้องบล็อก terminal
- `--name my-nginx`: ตั้งชื่อ container ว่า my-nginx แทนการสุ่มชื่อ
- `-p 8080:80`: เชื่อม port 8080 ของโฮสต์เข้ากับ port 80 ของ container
- `nginx:alpine`: ใช้ image ชื่อ nginx ที่มี tag alpine (เป็นเวอร์ชันที่มีขนาดเล็ก)
```
...
> **📝 บันทึกผล**: Container ID คือ: `f68892ac9316` และ IP Address คือ: `172.17.0.2`
...
v20.20.1
10.8.2
Hello from Docker! Running in: 530c5a426e39
```
---
...
1. `docker ps` กับ `docker ps -a` ต่างกันอย่างไร?
   ...
   > _คำตอบ_: `docker ps` แสดงเฉพาะ container ที่กำลังทำงานอยู่ (Up) ส่วน `docker ps -a` แสดง container ทั้งหมดรวมถึงตัวที่หยุดทำงานไปแล้ว (Exited)

2. ขนาดของ `nginx:alpine` image คือเท่าไร?

   > _คำตอบ_: ประมาณ 40-50 MB

3. flag `--rm` ใน `docker run` มีประโยชน์ในกรณีใด?

   > _คำตอบ_: ใช้เพื่อให้ Docker ลบ container ทิ้งอัตโนมัติทันทีที่หยุดทำงาน ช่วยประหยัดพื้นที่และไม่ให้มี container ขยะตกค้างหลังจากรันการทดสอบเสร็จ

4. `docker exec -it` ต่างจาก `docker run -it` อย่างไร?

   > _คำตอบ_: `docker run` ใช้สร้างและรัน container ใหม่ขึ้นมาทำงาน ส่วน `docker exec` ใช้เข้าไปทำงาน(รันคำสั่ง)เพิ่มเติมใน container ที่กำลังทำงานอยู่แล้ว
...
6. ต้องการลบ image ชื่อ nginx:alpine จะต้องเขียนคำสั่งอย่างไร หากมีหลายขั้นตอนที่ต้องเขียน ให้เขียนลำดับคำสั่งให้ถูกต้อง
   ```bash
   # 1. ลบ container ที่ใช้ image นี้อยู่ก่อน (ถ้ามี) แบบบังคับ
...
```
Multi-stage Build ใน Docker คือเทคนิคการเขียน Dockerfile ที่ช่วยให้เราสามารถสร้าง Docker Image ที่มีขนาดเล็กลง ปลอดภัยขึ้น และมีประสิทธิภาพมากขึ้น โดยการแบ่งขั้นตอน (stage) การทำงานออกเป็นหลายๆ ส่วนภายในไฟล์เดียว
```
---
...
> **📝 บันทึกผล**: ขนาดของ booking-backend:1.0 คือ: `662` MB
...
```text
- `-e DB_PATH=/app/data/booking.db`: เป็นการกำหนดตัวแปรสภาพแวดล้อม (Environment Variable) ให้กับ Container เพื่อระบุตำแหน่งของไฟล์ฐานข้อมูล SQLite
- `-v $(pwd)/test-data:/app/data`: เป็นการเชื่อมโยง (Bind Mount) โฟลเดอร์ test-data บนเครื่อง Host เข้ากับไดเรกทอรี /app/data ภายใน Container เพื่อไม่ให้ข้อมูลสูญหายเมื่อลบ Container
```
...
1. `booking-backend:1.0` คืออะไร?
   ...
   > _คำตอบ_: คือชื่อ Image (booking-backend) และ Tag (1.0) ของบริการ Backend ที่เราสร้าง (build) ขึ้นมาเองจาก Dockerfile

2. ถ้าต้องการให้ ทดสอบ API ด้วย `curl http://localhost` ต้องทำอย่างไร?

   > _คำตอบ_: ต้องทำการ Map Port ขาเข้าของโฮสต์ให้เป็นพอร์ต 80 โดยระบุ flag `-p 80:3001` (หรือพอร์ตที่ container ฟังอยู่) ในคำสั่ง docker run
...
> **📝 บันทึกผล**: booking-frontend:1.0 ขนาด `92` MB | booking-backend:1.0 ขนาด `662` MB
...
1. ขนาดของ `booking-frontend:1.0` เทียบกับ `booking-backend:1.0` ต่างกันอย่างไร? เพราะเหตุใด?
   ...
   > _คำตอบ_: Frontend image มีขนาดเล็กกว่ามาก เพราะใช้ nginx:alpine เป็น base และมีเฉพาะไฟล์ที่ build เสร็จแล้ว (static HTML/CSS/JS) ในขณะที่ Backend image ใช้ node:alpine และมี dependencies ใน node_modules, source code รวมถึง build tools สำหรับ SQLite 

2. `location /api/` ใน nginx.conf ทำหน้าที่อะไร?

   > _คำตอบ_: ทำหน้าที่เป็น Reverse Proxy เพื่อดักจับทุกเส้นทางที่มี /api/ และส่งต่อ (forward) request นั้นไปยัง Backend Container ที่ให้บริการ API

3. ทำไม Frontend Container ถึงใช้ชื่อ `backend` ในการ Proxy ได้?

   > _คำตอบ_: เพราะทั้ง Frontend และ Backend เชื่อมต่ออยู่ใน Docker Network เดียวกัน (booking-network) ซึ่งมีระบบ Container DNS ช่วยแปลง Service Name ให้เป็น IP Address ของตัว Container เป้าหมายได้อัตโนมัติ
...
1. docker-compose.yml คืออะไร มีประโยชน์อย่างไร?
   ...
   > _คำตอบ_: เป็นไฟล์ Configuration กำหนดโครงสร้าง Service ต่าง ๆ แบบ YAML ทำให้สาสมารถรันและจัดการหลายๆ Container (รวมทั้ง network และ volumes) ให้ทำงานร่วมกันได้อย่างเป็นระบบ ด้วยคำสั่งเพียงคำสั่งเดียว

2. `restart: unless-stopped` หมายความว่าอย่างไร?

   > _คำตอบ_: สั่งให้ Docker เริ่ม (Restart) Container ตัวนี้ขึ้นมาใหม่อัตโนมัติทุกครั้งที่เกิดข้อผิดพลาดหรือเมื่อ Docker daemon รันใหม่ เว้นเสียแต่ว่าผู้ใช้จะสั่ง `docker stop` ด้วยตัวเอง

3. Named Volume `sqlite_data` ต่างจาก Bind Mount อย่างไร? และทำไมต้องใช้กับ SQLite?

   > _คำตอบ_: Named Volume จัดการพื้นที่จัดเก็บภายในพาธเฉพาะของ Docker ซึ่งจัดการ Permission ถนอมประสิทธิภาพดีกว่า ผิดกับ Bind Mount ที่อ้างอิงกับพาทของ Host โดยตรง เราใช้ร่วมกับ SQLite เพี่อเก็บรักษาไฟล์ฐานข้อมูล (.db) ไม่ให้หายไปแม้รอบการชีวิตของ Container จะสิ้นสุด (ถูกลบ/แก้/อัปเดตใหม่)
...
**บันทึกรูปผลการทดลอง**
```json
{"id":11,"fullname":"นายทดสอบ ระบบ","email":"test@gmail.com","phone":"0123456789","checkin":"2025-08-01","checkout":"2025-08-03","roomtype":"delux","guests":2,"status":"pending","comment":null,"created_at":"2026-03-23 11:31:29"}
```
...
1. `docker compose ps` แสดงสถานะ Service อย่างไร?
   ...
   > _คำตอบ_: แสดงชื่อ Container, คำสั่งตอนรัน, สถานะ (Up / Exited), และการทำ Port Mapping จาก Host สู่ Container

2. IP Address ของแต่ละ Container ในผลจาก `docker network inspect` คืออะไร?

   > _คำตอบ_: คือไอพีแพอดเดรสที่ถูกส่งมอบให้กับ Container ใน Bridge Network ส่วนตัว (เช่น 172.18.0.x) เอาไว้สื่อสารกันเองในระบบ Network ของ Docker Compose
