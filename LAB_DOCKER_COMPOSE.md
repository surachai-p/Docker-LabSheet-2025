...
![ภาพผลการตรวจสอบ version](<images/ผลการตรวจสอบ version.png>)
---
...
![รูปผลการทดลอง คำสั่ง docker ps](<ผลการทดลอง คำสั่ง docker ps.png>)
...
```text
docker run สร้างและเริ่มรัน Container

-d สั่งให้ทำงานอยู่เบื้องหลัง (Background)

--name my-nginx ตั้งชื่อ Container นี้ว่า "my-nginx"

-p 8080:80 เอาพอร์ต 8080 ของเครื่องเรา ไปเชื่อมกับพอร์ต 80 ของ Container

nginx:alpine ใช้ Image Nginx เวอร์ชัน alpine (ซึ่งมีขนาดเล็กและเบามาก)
...
**บันทึกผลการรันคำสั่ง docker exec -it และ ls /usr/share/nginx/html**
   ...
   ```bash
   ![ผลการรันคำสั่ง docker](images/รันคำสั่ง.png)
   ![ผลการรันคำสั่ง docker](images/รันคำสั่ง1.png)
   ```
</details>
...
> **📝 บันทึกผล**: Container ID คือ: `1a536a6de94e138b7ddff0ca633f49d7cb4c6c7a167675721bb923c9abba6ac3` และ IP Address คือ: `172.17.0.2`
...
![รูปผลการทดลอง](images/ผลการทดลอง.png)
![รูปผลการทดลอง](images/ผลการทดลอง1.png)
![รูปผลการทดลอง](images/ผลการทดลอง2.png)
---
...
1. `docker ps` กับ `docker ps -a` ต่างกันอย่างไร?
   ...
   > _คำตอบ_: docker ps ดูเฉพาะตัวที่กำลังรันอยู่ docker ps -a ดูทั้งหมด (ทั้งรันอยู่และหยุดแล้ว)
...
3. flag `--rm` ใน `docker run` มีประโยชน์ในกรณีใด?
   ...
   > _คำตอบ_: ใช้ลบ Container ทิ้งอัตโนมัติเมื่อมันหยุดทำงาน เพื่อไม่ให้รกเครื่อง

4. `docker exec -it` ต่างจาก `docker run -it` อย่างไร?

   > _คำตอบ_: run คือสร้างตัวใหม่ exec คือมุดเข้าไปสั่งงานในตัวที่กำลังรันอยู่แล้ว
...
```
เทคนิคการสร้าง Docker Image แบบแบ่งหลายขั้นตอนในไฟล์เดียว เพื่อแยกส่วน "Build โค้ด" (ที่ไฟล์ใหญ่) ออกจากส่วน "รันจริง" ทำให้ได้ Image ตัวสุดท้ายที่ ขนาดเล็กมาก และ ปลอดภัยขึ้น (เพราะเหลือแค่ไฟล์ที่พร้อมใช้งานจริงๆ)
```
---
...
> **📝 บันทึกผล**: ขนาดของ booking-backend:1.0 คือ: `457` MB 
...
```
-e DB_PATH=/app/data/booking.db กำหนดตัวแปรบอกแอปพลิเคชันว่า "ไฟล์ฐานข้อมูล" เก็บไว้ที่ไหนภายใน Container

-v $(pwd)/test-data:/app/data เชื่อมโฟลเดอร์บนเครื่อง Mac เข้ากับ Container เพื่อ ป้องกันข้อมูลหาย (ข้อมูลจะถูกเซฟเก็บไว้ที่เครื่องเรา แม้ Container จะถูกลบไปแล้ว)
```
...
![รูปผลการรัน](<images/ผลการรัน .png>)
---
...
1. `booking-backend:1.0` คืออะไร?
   ...
   > _คำตอบ_:ชื่อและเวอร์ชัน ของ Docker Image ที่เราสร้างขึ้นมา (booking-backend คือชื่อ Image, 1.0 คือเวอร์ชัน) เพื่อนำไปสั่งรันเป็น Container

2. ถ้าต้องการให้ ทดสอบ API ด้วย `curl http://localhost` ต้องทำอย่างไร?

   > _คำตอบ_: ต้องเปลี่ยนการจับคู่ Port (Port Mapping) ให้เครื่องเราใช้ Port 80 โดยแก้คำสั่งรันเป็น -p 80:3001 (เพราะ http://localhost แบบไม่ระบุเลข จะวิ่งไปที่ Port 80 เป็นค่าเริ่มต้น
...
> **📝 บันทึกผล**: booking-frontend:1.0 ขนาด `61.7` MB | booking-backend:1.0 ขนาด `457` MB
...
1. ขนาดของ `booking-frontend:1.0` เทียบกับ `booking-backend:1.0` ต่างกันอย่างไร? เพราะเหตุใด?
   ...
   > _คำตอบ_: Frontend เล็กกว่ามาก (61.7MB vs 457MB) เพราะใช้เทคนิค Multi-stage Build ดึงมาเฉพาะไฟล์ที่ใช้งานจริง (dist) วางบน Nginx ที่มีขนาดเบา

2. `location /api/` ใน nginx.conf ทำหน้าที่อะไร?

   > _คำตอบ_: ทำหน้าที่เป็น Reverse Proxy เพื่อส่งต่อคำขอ (Request) ที่ขึ้นต้นด้วย /api/ จากหน้าเว็บไปหา Container ของ Backend โดยตรง

3. ทำไม Frontend Container ถึงใช้ชื่อ `backend` ในการ Proxy ได้?

   > _คำตอบ_: เพราะทั้งสอง Container อยู่ใน Docker Network เดียวกัน ทำให้สามารถสื่อสารกันผ่าน Service Name (ชื่อคอนเทนเนอร์) ได้ทันทีโดยไม่ต้องระบุ IP Address
...
1. docker-compose.yml คืออะไร มีประโยชน์อย่างไร?
   ...
   > _คำตอบ_: คือไฟล์ที่ใช้ นิยามและจัดการหลาย Container พร้อมกัน ประโยชน์คือช่วยให้สั่งรันระบบทั้งหมด (Frontend, Backend, Network) ได้ด้วยคำสั่งเดียว ไม่ต้องพิมพ์คำสั่งยาวๆ แยกกัน

2. `restart: unless-stopped` หมายความว่าอย่างไร?

   > _คำตอบ_: สั่งให้ Container เริ่มทำงานใหม่โดยอัตโนมัติ ถ้ามันพังหรือเครื่องรีสตาร์ท ยกเว้นกรณีที่เราเป็นคนสั่งหยุด (Stop) มันด้วยตัวเอง

3. Named Volume `sqlite_data` ต่างจาก Bind Mount อย่างไร? และทำไมต้องใช้กับ SQLite?

   > _คำตอบ_: Named Volume ให้ Docker จัดการพื้นที่เก็บข้อมูลให้เอง (ประสิทธิภาพสูงและปลอดภัยกว่า) ส่วน Bind Mount คือการเชื่อมกับโฟลเดอร์ในเครื่องเราโดยตรง ที่ต้องใช้กับ SQLite เพื่อให้ไฟล์ฐานข้อมูล ไม่หายไป เมื่อมีการลบหรืออัปเดต Container
...
> **📝 บันทึกผลการทดลอง**:
> - IP ของ frontend: `172.19.0.3/16`
> - IP ของ backend: `172.19.0.2/16`
...
1. `docker compose ps` แสดงสถานะ Service อย่างไร?
   ...
   > _คำตอบ_: แสดงรายชื่อ Container, สถานะการทำงาน (เช่น Running หรือ Up), Port ที่ใช้งาน และสุขภาพของระบบ (Health)

2. IP Address ของแต่ละ Container ในผลจาก `docker network inspect` คืออะไร?

   > _คำตอบ_: คือเลขที่อยู่ภายใน Network ของ Docker (เช่น 172.18.0.x) ที่ใช้สื่อสารกันเองระหว่าง Frontend, Backend และ Database

3. SQLite database file (`booking.db`) ถูกสร้างใน Path ใดภายใน Container?

   > _คำตอบ_: /app/bookings.db (หรือตามที่ระบุใน ENV คือ /app/data/booking.db)
