...
```text
อธิบายคำสั่งที่นี่
docker run การสั่งให้ docker สร้าง container
-d เป็นการสั่งให้ docker run อยู่เบื้องหลังเพื่อให้ใช้ terminal ต่อได้
--name my-nginx ตั้งชื่อ container เป็น my-nginx
-p ทำ port mapping 8080:80 ,host:container
```
...
> **📝 บันทึกผล**: Container ID คือ: `edb8b24cbfe7` และ IP Address คือ: `172.17.0.2`
...
1. `docker ps` กับ `docker ps -a` ต่างกันอย่างไร?
   ...
   > _คำตอบ_: docker ps แสดงผล container ที่รันอยู่เท่านั้น สถานะเป็น up
              docker ps -a แสดงผล container ทั้งหมดในเครื่อง
...
3. flag `--rm` ใน `docker run` มีประโยชน์ในกรณีใด?
   ...
   > _คำตอบ_: เมื่อมีการสั่ง down container จะลบ container อัตโนมัติ

4. `docker exec -it` ต่างจาก `docker run -it` อย่างไร?

   > _คำตอบ_:  docker exec -it เป็นการเข้าไปใน container ที่กำลังรันอยู่
              docker run -it เป็นการสร้าง container ใหม่ 
...
6. ต้องการลบ image ชื่อ nginx:alpine จะต้องเขียนคำสั่งอย่างไร หากมีหลายขั้นตอนที่ต้องเขียน ให้เขียนลำดับคำสั่งให้ถูกต้อง
   ```bash
   เขียนคำสั่งที่นี่
   สั่งหยุด container docker stop my-nginx
   หลังจากสั่งหยุด container บางตัวอาจจะทำงานอยู่ให้ใช้ docker rm my-nginx
   จากนั้นก็เริ่มลบ image docker rmi nginx:alpine อันนี้เป็นแบบใช่ชื่อและ tag
   หรือ ลบโดยใช่ image ID, docker rmi [IMAGE_ID]
   ```
...
```
ตอบคำถามที่นี่
เป็นเทคนิคสร้าง image ที่ขนาดเล็กและปลอดภัย โดยแบ่งการ build 
ออกเป็นหลาย stages ในไหล์เดียว  
```
---
...
-e DB_PATH=/app/data/booking.db
  บอก backend ว่าถ้าอยากได้ไฟลื DB ให้ไปหาที่ path ไหน
  -e เป็นการส่งค่าตัวแปรเข้าไปในโปรแกรมที่รันใน container รู้จัก
-v $(pwd)/test-data:/app/data
  ไฟล์ในโฟล์เดอร์ test-data จะเข้าไปอยู่ใน /app/data ของ container
  -v ทำให้ไฟล์ booking.db ถูกเขียนลงในเครื่อง เวลาลบ container ข้อมูลเลยไม่หาย
```
...
1. `booking-backend:1.0` คืออะไร?
   ...
   > _คำตอบ_: คือ Image Name และ Tag ที่ใช้สั่งให้ Docker ไปหยิบมาสร้างเป็น Container Name:Tag

2. ถ้าต้องการให้ ทดสอบ API ด้วย `curl http://localhost` ต้องทำอย่างไร?

   > _คำตอบ_: พิมพ์ curl http://localhost/enpoit ตามที่ต้องการทดสอบ
...
1. ขนาดของ `booking-frontend:1.0` เทียบกับ `booking-backend:1.0` ต่างกันอย่างไร? เพราะเหตุใด?
   ...
   > _คำตอบ_: ขึ้นอยู่กับวิธีการ build และ base image frontend ใช้ multi-stage build ทำให้มีขนาดเล็กกว่า backend ขณะที่ backend ใช้ single-stage build ทำให้มีขนาดใหญ่กว่า

2. `location /api/` ใน nginx.conf ทำหน้าที่อะไร?

   > _คำตอบ_: คอยดักจับ request ที่ขึ้นต้นด้วย /api/ แล้วส่งต่อไปยัง backend container

3. ทำไม Frontend Container ถึงใช้ชื่อ `backend` ในการ Proxy ได้?

   > _คำตอบ_: เพราะ backend คือชื่อ Service ใน docker-compose.yml
...
1. docker-compose.yml คืออะไร มีประโยชน์อย่างไร?
   ...
   > _คำตอบ_: blueprint สำหรับส่วนต่างๆของแอปว่ามี container อะไรบ้าง
   ช่วยให้จากเดิมที่ต้องใช้ docker run หลายๆคำสั่งก็สามารถรันทีเดียวได้

2. `restart: unless-stopped` หมายความว่าอย่างไร?

   > _คำตอบ_: ทำให้ container restart เองตราบใดที่ไม่ได้สั่ง stop

3. Named Volume `sqlite_data` ต่างจาก Bind Mount อย่างไร? และทำไมต้องใช้กับ SQLite?

   > _คำตอบ_: named volume จะถูกจัดการโดย docker เองส่วน bind mount จะถูกจัดการโดย host
   SQLite เก็บฐานข้อมูลทั้งหมดเป็นเพียงแค่ไฟล์เดียวถ้าใช้ Bind Mount เก็บ SQLite เมื่อมีการอ่านเขียนถี่ๆ หรือข้ามระบบปฏิบัติการอาจจะทำให้ข้อมูลเสียหายได้
...
> **📝 บันทึกผลการทดลอง**:
> - IP ของ frontend: `172.21.0.3/16`
> - IP ของ backend: `172.21.0.2/16`
...
1. `docker compose ps` แสดงสถานะ Service อย่างไร?
   ...
   > _คำตอบ_: แสดงหถานะภาพรวมของ service ทั้งหมดที่ถูกสร้างจาก docker-compose.yml

2. IP Address ของแต่ละ Container ในผลจาก `docker network inspect` คืออะไร?

   > _คำตอบ_: IP ของ frontend: `172.21.0.3/16`
              IP ของ backend: `172.21.0.2/16`
