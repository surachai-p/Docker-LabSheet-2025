...
```text
อธิบายคำสั่งที่นี่
docker run: สร้างและเริ่มการทำงานของ Container
-d: รันแบบเบื้องหลังไม่ค้างหน้าจอ Terminal
--name my-nginx: ตั้งชื่อ Container ว่า "my-nginx" เพื่อให้เรียกใช้งานง่าย
-p 8080:80: เชื่อมพอร์ตเครื่องเรา (8080) เข้ากับพอร์ตใน Container (80)
nginx:alpine: ระบุ Image ที่จะใช้ โดยเลือก Nginx เวอร์ชัน Alpine
```
...
> **📝 บันทึกผล**: Container ID คือ: `df414679fb49` และ IP Address คือ: `172.17.0.2`
...
**บันทึกรูปผลการทดลอง**![alt text](image-2.png)
...
1. `docker ps` กับ `docker ps -a` ต่างกันอย่างไร?
   ...
   > _คำตอบ_: docker ps แสดงเฉพาะตัวที่กำลังรัน; docker ps -a แสดงทั้งหมด

2. ขนาดของ `nginx:alpine` image คือเท่าไร?

   > _คำตอบ_: ประมาณ 5 - 10 MB

3. flag `--rm` ใน `docker run` มีประโยชน์ในกรณีใด?

   > _คำตอบ_: ใช้ลบ Container ทิ้งอัตโนมัติทันทีที่หยุดทำงาน เพื่อไม่ให้รกเครื่อง

4. `docker exec -it` ต่างจาก `docker run -it` อย่างไร?

   > _คำตอบ_: run คือสร้างตัวใหม่; exec คือสั่งงานเพิ่มในตัวที่รันอยู่แล้ว
...
6. ต้องการลบ image ชื่อ nginx:alpine จะต้องเขียนคำสั่งอย่างไร หากมีหลายขั้นตอนที่ต้องเขียน ให้เขียนลำดับคำสั่งให้ถูกต้อง
   ```bash
   docker rm -f my-nginx  # ลบ Container ที่ขวางอยู่
...
```
การเขียน Dockerfile แบ่งเป็นหลายขั้นตอนเพื่อแยกส่วน "ตอนสร้าง" (Build) ออกจาก "ตอนรัน" (Run)
```
---
...
คือการส่ง Environment Variableเข้าไปใน Container บอกโปรแกรมข้างในว่า ให้ไปอ่าน/เขียนไฟล์ฐานข้อมูลที่พาธ /app/data/booking.db นะ

2. -v $(pwd)/test-data:/app/data
คือการทำ Volume Mapping เชื่อมโฟลเดอร์ test-data ในเครื่องเรา เข้ากับโฟลเดอร์ /app/data ใน Container เพื่อให้ข้อมูลฐานข้อมูล ไม่หายไป แม้จะลบ Container ทิ้ง 
```
...
**บันทึกผลการทดลอง**![alt text](image-3.png)
...
1. `booking-backend:1.0` คืออะไร?
   ...
   > _คำตอบ_: mage Name และ Tag (Version) ของ Docker Image ที่เรา Build ขึ้นมา เพื่อใช้เป็นต้นแบบในการรัน Container

2. ถ้าต้องการให้ ทดสอบ API ด้วย `curl http://localhost` ต้องทำอย่างไร?

   > _คำตอบ_: ต้องทำการ Port Mapping โดยเพิ่ม flag -p 80:5000 ในคำสั่ง docker run (เพื่อให้พอร์ต 80 ของเครื่องเราชี้ไปที่พอร์ต 5000 ของ App ใน Container)
...
1. ขนาดของ `booking-frontend:1.0` เทียบกับ `booking-backend:1.0` ต่างกันอย่างไร? เพราะเหตุใด?
   ...
   > _คำตอบ_: Frontend เล็กกว่า เพราะมีแค่ไฟล์ Static และ Nginx ส่วน Backend ใหญ่กว่า เพราะต้องมี Runtime และ Library ต่างๆ ในการรัน Code

2. `location /api/` ใน nginx.conf ทำหน้าที่อะไร?

   > _คำตอบ_: เป็น Reverse Proxy ทำหน้าที่ดักจับ Request ที่ส่งมายัง /api/ แล้วส่งต่อไปยังเครื่อง Backend Server

3. ทำไม Frontend Container ถึงใช้ชื่อ `backend` ในการ Proxy ได้?

   > _คำตอบ_: เพราะรันอยู่ใน Docker Network เดียวกัน ทำให้สามารถสื่อสารกันผ่าน ชื่อคอนเทนเนอร์ ได้ทันทีผ่านระบบ DNS ของ Docker
...
1. docker-compose.yml คืออะไร มีประโยชน์อย่างไร?
   ...
   > _คำตอบ_: คือไฟล์ที่ใช้กำหนดค่าและจัดการ Container หลายๆ ตัวพร้อมกันในโปรเจกต์เดียว มีประโยชน์ช่วยให้เราสั่งรันระบบทั้งหมด ได้ด้วยคำสั่งเดียว แทนการพิมพ์ docker run แยกทีละตัว

2. `restart: unless-stopped` หมายความว่าอย่างไร?

   > _คำตอบ_: สั่งให้ Container เริ่มทำงานใหม่โดยอัตโนมัติ หากเกิดข้อผิดพลาด หรือเมื่อมีการรีสตาร์ทเครื่อง Docker Host เว้นแต่ว่าเราจะเป็นคนสั่ง stop คอนเทนเนอร์นั้นด้วยตัวเอง

3. Named Volume `sqlite_data` ต่างจาก Bind Mount อย่างไร? และทำไมต้องใช้กับ SQLite?

   > _คำตอบ_: Named Volume คือพื้นที่เก็บข้อมูลที่ Docker จัดการให้โดยเฉพาะ ต่างจาก Bind Mount ที่ต้องระบุพาธในเครื่องเราเอง สาเหตุที่ใช้กับ SQLite เพราะ Named Volume มีประสิทธิภาพการอ่าน/เขียน สูงกว่าและปลอดภัยจากการเผลอลบไฟล์จากฝั่ง Host ทำให้ฐานข้อมูลมีความเสถียรมากกว่า
...
1. `docker compose ps` แสดงสถานะ Service อย่างไร?
   ...
   > _คำตอบ_: แสดงรายการ Container ทั้งหมดที่ถูกจัดการโดย Compose ไฟล์นั้น พร้อมระบุStatusพอร์ตที่ใช้งาน และ ชื่อ Service

2. IP Address ของแต่ละ Container ในผลจาก `docker network inspect` คืออะไร?

   > _คำตอบ_: คือ Internal IP ที่ Docker Assign ให้แต่ละ Container ใช้สื่อสารกันเองภายในวง Network นั้นๆ โดยที่เครื่องภายนอกจะมองไม่เห็น IP นี้โดยตรง

3. SQLite database file (`booking.db`) ถูกสร้างใน Path ใดภายใน Container?

   > _คำตอบ_: ถูกสร้างในพาธ /app/data/booking.db
