...
```text
อธิบายคำสั่งที่นี่docker run: สั่งสร้างและรัน Container ใหม่

-d (Detached): ให้ Container ทำงานอยู่เบื้องหลัง (Background) ไม่ล็อกหน้าจอ Terminal

--name my-nginx: ตั้งชื่อให้ Container นี้ว่า "my-nginx" เพื่อง่ายต่อการจัดการ

-p 8080:80 (Publish): จับคู่พอร์ต โดยส่งข้อมูลจากพอร์ต 8080 ของเครื่องเรา (Host) เข้าไปยังพอร์ต 80 ของ Container

nginx:alpine: ระบุชื่อ Image ที่จะใช้ คือ Nginx (เว็บเซิร์ฟเวอร์) เวอร์ชัน alpine (เวอร์ชันที่มีขนาดเล็กและเบามาก)
...
1. `docker ps` กับ `docker ps -a` ต่างกันอย่างไร?
   ...
   > _คำตอบ_: docker ps จะแสดงเฉพาะ Container ที่กำลังทำงานอยู่ ในขณะนั้น ส่วน docker ps -a จะแสดง Container ทั้งหมดในระบบ ทั้งที่กำลังทำงานอยู่และที่หยุดทำงานไปแล้ว

2. ขนาดของ `nginx:alpine` image คือเท่าไร?

   > _คำตอบ_: มีขนาดประมาณ 40 - 45 MB ซึ่งมีขนาดเล็กและเบากว่า image nginx ตัวปกติ

3. flag `--rm` ใน `docker run` มีประโยชน์ในกรณีใด?

   > _คำตอบ_: ใช้เพื่อสั่งให้ Docker ลบ Container ทิ้งโดยอัตโนมัติทันทีที่มันหยุดทำงาน เหมาะสำหรับการรัน Container แบบชั่วคราวเพื่อทดสอบระบบรันสคริปต์ หรือดูผลลัพธ์สั้นๆ เพื่อไม่ให้มี Container ขยะตกค้างกินพื้นที่ในเครื่อง
4. `docker exec -it` ต่างจาก `docker run -it` อย่างไร?

   > _คำตอบ_: docker run -it ใช้สร้างและรัน Container ตัวใหม่ ขึ้นมาพร้อมกับเข้าไปโต้ตอบใน Shell ส่วน docker exec -it ใช้สำหรับมุดเข้าไปรันคำสั่งหรือเปิด Shell ใน Container ที่กำลังทำงานอยู่แล้ว
...
6. ต้องการลบ image ชื่อ nginx:alpine จะต้องเขียนคำสั่งอย่างไร หากมีหลายขั้นตอนที่ต้องเขียน ให้เขียนลำดับคำสั่งให้ถูกต้อง
   ```bash
      ...
      docker ps -a 
      docker rm -f <ชื่อ_หรือ_ID_ของ_Container>
...
```
เทคนิคการเขียน Dockerfile ที่แบ่งการสร้าง Image ออกเป็นหลายขั้นตอน (โดยใช้คำสั่ง FROM หลายครั้งในไฟล์เดียว)
```
---
...
```
    สร้างตัวแปรสภาพแวดล้อม ชื่อ DB_PATH เพื่อบอกให้แอปพลิเคชันใน Container ทราบว่าไฟล์ฐานข้อมูลอยู่ที่พาธ /app/data/booking.db

    ทำการแชร์โฟลเดอร์ (Volume Mount) โดยเชื่อมโฟลเดอร์ test-data จากเครื่องของเรา เข้าไปไว้ที่ตำแหน่ง /app/data ใน Container เพื่อให้แอปใช้งานไฟล์ร่วมกันได้ และป้องกันไม่ให้ข้อมูลสูญหายเมื่อ Container ถูกลบทิ้ง
```
...
1. `booking-backend:1.0` คืออะไร?
   ...
   > _คำตอบ_: mage Name (ชื่อโปรเจกต์) และ Tag (เวอร์ชัน) ที่เราสร้างขึ้น

2. ถ้าต้องการให้ ทดสอบ API ด้วย `curl http://localhost` ต้องทำอย่างไร?

   > _คำตอบ_: ต้องเพิ่มพารามิเตอร์ -p 80:80 (หรือ -p 80:<พอร์ตที่แอปใช้> เช่น -p 80:3000) เข้าไปในคำสั่ง docker run
---
...
1. ขนาดของ `booking-frontend:1.0` เทียบกับ `booking-backend:1.0` ต่างกันอย่างไร? เพราะเหตุใด?
   ...
   > _คำตอบ_: booking-frontend:1.0 จะมีขนาด เล็กกว่า booking-backend:1.0 มาก เนื่องจากตัว Frontend ถูกคอมไพล์จนเหลือเพียงไฟล์ Static แล้วนำไปรันบน Nginx ที่มีความเบาบาง ในขณะที่ Backend มีขนาด ใหญ่กว่า เพราะต้องรวมเอาตัวรันระบบ และไลบรารีทั้งหมด เข้าไปแบกไว้ใน Image ด้วย

2. `location /api/` ใน nginx.conf ทำหน้าที่อะไร?

   > _คำตอบ_: ทำหน้าที่เป็น Reverse Proxy เพื่อคอยดักจับ Request ที่เรียกหา API แล้วส่งต่อให้เครื่อง Backend เป็นคนประมวลผลแทน เทคนิคนี้ช่วยแก้ปัญหาเรื่อง CORS และทำให้ฝั่ง Client สามารถเรียกใช้งาน API ผ่านโดเมนหรือพอร์ตเดียวกันกับเว็บเพจได้เลย
3. ทำไม Frontend Container ถึงใช้ชื่อ `backend` ในการ Proxy ได้?

   > _คำตอบ_: สาเหตุที่ตั้งค่า Proxy ชี้ไปที่ชื่อ backend ได้ตรงๆ เป็นเพราะระบบ Docker Internal DNS เมื่อคอนเทนเนอร์ถูกรันอยู่ใน Network วงเดียวกัน Docker จะอนุญาตให้คอนเทนเนอร์เหล่านั้นสื่อสารและเรียกหากันผ่าน "ชื่อ" ได้ทันที โดยไม่ต้องพึ่งพาหมายเลข IP ครับ
...
1. docker-compose.yml คืออะไร มีประโยชน์อย่างไร?
   ...
   > _คำตอบ_: มันคือไฟล์ตั้งค่าที่ใช้สำหรับสั่งรันหลายๆ Container ขึ้นมาพร้อมกัน ประโยชน์หลักคือช่วยให้เราสามารถจัดการทรัพยากรทั้งหมด ทั้งฝั่ง Service, Network และ Volume ได้ครบจบในคำสั่งเดียว โดยไม่ต้องมานั่งพิมพ์คำสั่ง docker run ทีละตัวให้ยุ่งยาก

2. `restart: unless-stopped` หมายความว่าอย่างไร?

   > _คำตอบ_: เป็นการตั้งค่าให้ Container เริ่มทำงานใหม่ขึ้นมาเองโดยอัตโนมัติ หากตัวแอปเกิดข้อผิดพลาด หรือเมื่อเครื่องเซิร์ฟเวอร์มีการรีสตาร์ท โดยระบบจะไม่เปิด Container ให้ใหม่แค่ในกรณีเดียว คือการที่เราเป็นคนสั่งหยุด มันด้วยตัวเองเท่านั้น

3. Named Volume `sqlite_data` ต่างจาก Bind Mount อย่างไร? และทำไมต้องใช้กับ SQLite?

   > _คำตอบ_: Named Volume: จะให้ Docker เป็นคนจองและจัดการพื้นที่เก็บข้อมูลให้โดยตรง ซึ่งมีความปลอดภัยและประสิทธิภาพในการอ่าน/เขียนดีกว่า เรา (ผู้ใช้) ต้องเป็นคนระบุพิกัดโฟลเดอร์บนเครื่องของเรา เพื่อเชื่อมเข้าไปเองเหตุผลที่ใช้ Named Volume กับ SQLite: เพราะช่วยป้องกันปัญหาเรื่องสิทธิ์การเข้าถึงไฟล์ และช่วยลดโอกาสที่ไฟล์ Database จะพังหรือเสียหายเวลาที่มีการใช้งานแชร์กันข้ามระบบปฏิบัติการ (OS) ครับ
---
...
1. `docker compose ps` แสดงสถานะ Service อย่างไร?
   ...
   > _คำตอบ_: คำสั่งนี้ใช้สำหรับดูสถานะของ Service  ทั้งหมดที่รันอยู่ภายใต้โปรเจกต์ Compose นั้นๆ โดยจะแสดงข้อมูลสำคัญ เช่น ชื่อ Service/Container, สถานะปัจจุบัน , พอร์ต ที่เปิดให้ใช้งาน และคำสั่ง ที่กำลังรันอยู่

2. IP Address ของแต่ละ Container ในผลจาก `docker network inspect` คืออะไร?

   > _คำตอบ_: IP เหล่านี้คือ IP ภายใน ของระบบ Docker ที่ถูกแจกจ่ายให้กับแต่ละ Container เพื่อให้พวกมันสามารถใช้สื่อสารและเชื่อมต่อหากันเองได้ภายในวงเครือข่ายจำลองนั้น

3. SQLite database file (`booking.db`) ถูกสร้างใน Path ใดภายใน Container?

   > _คำตอบ_:ตำแหน่งที่จัดเก็บจะขึ้นอยู่กับโค้ดและการตั้งค่าของตัวแอปพลิเคชันเอง แต่โดยทั่วไปแล้วมักจะถูกสร้างไว้ในโฟลเดอร์หลักของการทำงาน เช่น /app/booking.db หรือ /usr/src/app/booking.db
---
