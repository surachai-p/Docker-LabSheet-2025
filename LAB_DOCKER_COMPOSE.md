...
> **📝 บันทึกผล**: Container ID คือ: `773ddb78be11` และ IP Address คือ: `161.246.146.190`
...
1. `docker ps` กับ `docker ps -a` ต่างกันอย่างไร?
   ...
   > _คำตอบ_: ความแตกต่างหลักของทั้งสองคำสั่งคือ สถานะของ Container ที่ถูกแสดงผลออกมา

2. ขนาดของ `nginx:alpine` image คือเท่าไร?

   > _คำตอบ_: จะอยู่ที่ประมาณ 20–25 MB

3. flag `--rm` ใน `docker run` มีประโยชน์ในกรณีใด?

   > _คำตอบ_: ประโยชต์คือ สั่งรันแล้วลบทิ้งทันทีเมื่อเลิกใช้

4. `docker exec -it` ต่างจาก `docker run -it` อย่างไร?

   > _คำตอบ_: ความแตกต่างที่สำคัญที่สุดคือ การสร้างใหม่ กับ การเข้าไปหาตัวเก่า
...
1. `booking-backend:1.0` คืออะไร?
   ...
   > _คำตอบ_: เป็นชื่อโปรเจกต์ และ Tag เวอร์ชัน ที่สร้าง

2. ถ้าต้องการให้ ทดสอบ API ด้วย `curl http://localhost` ต้องทำอย่างไร?

   > _คำตอบ_: ต้องรัน Container โดยระบุการเชื่อมต่อพอร์ต docker run -d -p 80:80 nginx:alpine
---
...
1. ขนาดของ `booking-frontend:1.0` เทียบกับ `booking-backend:1.0` ต่างกันอย่างไร? เพราะเหตุใด?
   ...
   > _คำตอบ_: ฝั่ง Frontend เล็กกว่าเพราะเหลือแค่ Static files (HTML/JS/CSS) บน Nginx ส่วน Backend ใหญ่กว่าเพราะต้องมี Runtime (เช่น Node.js, Python, JDK) และ Library ทั้งหมดของระบบ

2. `location /api/` ใน nginx.conf ทำหน้าที่อะไร?

   > _คำตอบ_: ทำหน้าที่เป็น Reverse Proxy ช่วยให้ Frontend เรียก API ผ่าน Path เดียวกัน ป้องกันปัญหา Cross-Origin Resource Sharing ได้อย่างเบ็ดเสร็จ

3. ทำไม Frontend Container ถึงใช้ชื่อ `backend` ในการ Proxy ได้?

   > _คำตอบ_: เป็นความสามารถของ Docker Embedded DNS ที่จะแมป Service Name ให้เป็น IP Address ภายใน Network วงเดียวกันโดยอัตโนมัติ
...
1. docker-compose.yml คืออะไร มีประโยชน์อย่างไร?
   ...
   > _คำตอบ_: คือไฟล์ในรูปแบบ YAML ที่ใช้สำหรับกำหนดค่า ของ Multi-container Docker applications

2. `restart: unless-stopped` หมายความว่าอย่างไร?

   > _คำตอบ_: เป็นนโยบายการคืนชีพให้ Container  Docker จะสั่งให้ Container เริ่มทำงานใหม่โดยอัตโนมัติหากมัน Crash หรือเครื่อง Server ถูก Reboot

3. Named Volume `sqlite_data` ต่างจาก Bind Mount อย่างไร? และทำไมต้องใช้กับ SQLite?

   > _คำตอบ_: * Named Volume: Docker เป็นคนจัดการพื้นที่เก็บข้อมูลให้เอง (อยู่ใน /var/lib/docker/volumes/) มีประสิทธิภาพสูงและปลอดภัยกว่าเพราะ Docker ควบคุมสิทธิ์การเข้าถึงทั้งหมด * Bind Mount: เราเป็นคนระบุตำแหน่ง (Path) บนเครื่อง Host เองเพื่อเชื่อมเข้ากับ Container เหมาะกับการแก้ไข Code แบบ Real-time
...
1. `docker compose ps` แสดงสถานะ Service อย่างไร?
   ...
   > _คำตอบ_: เป็นคำสั่งที่ใช้ตรวจสอบ "สุขภาพ" ของโปรเจกต์ Compose ในภาพรวม

2. IP Address ของแต่ละ Container ในผลจาก `docker network inspect` คืออะไร?

   > _คำตอบ_: Internal IP Address (Private IP) ที่ Docker Engine จ่ายให้กับแต่ละ Container ภายใน Virtual Network

3. SQLite database file (`booking.db`) ถูกสร้างใน Path ใดภายใน Container?

   > _คำตอบ_: โดยปกติจะอยู่ที่ Working Directory ที่เรากำหนดไว้ใน Dockerfile (มักจะเป็น /app/booking.db)
