...
```text
อธิบายคำสั่งที่นี่
คำสั่งนี้จะสร้าง container nginx ที่ทำงานเบื้องหลัง ชื่อ my-nginx และสามารถเข้าถึงเว็บเซิร์ฟเวอร์ผ่าน http://localhost:8080 ได้
```
...
> **📝 บันทึกผล**: Container ID คือ: `b8b7e5ad8a8d` และ IP Address คือ: `172.17.0.2`
...
1. `docker ps` กับ `docker ps -a` ต่างกันอย่างไร?
   ...
   > _คำตอบ_: docker ps จะแสดงเฉพาะ Container ที่ กำลังรันอยู่ (Running) เท่านั้น ส่วน docker ps -a (all) จะแสดง Container ทั้งหมด ในเครื่อง ไม่ว่าจะกำลังรันอยู่ หรือหยุดทำงานไปแล้ว (Exited) ก็ตาม

2. ขนาดของ `nginx:alpine` image คือเท่าไร?

   > _คำตอบ_: ประมาณ 5MB - 10MB

3. flag `--rm` ใน `docker run` มีประโยชน์ในกรณีใด?

   > _คำตอบ_: ใช้ในกรณีที่ต้องการให้ ลบ Container ทิ้งโดยอัตโนมัติทันทีที่หยุดทำงาน มีประโยชน์มากเวลาเราต้องการรันคำสั่งสั้นๆ หรือทดสอบอะไรบางอย่าง เพื่อไม่ให้มี Container ที่หยุดทำงานแล้วค้างรกเครื่อง

4. `docker exec -it` ต่างจาก `docker run -it` อย่างไร?

   > _คำตอบ_: docker run -it คือการ สร้างและเริ่มรัน Container ใหม่ จาก Image ส่วน docker exec -it คือการ สั่งงานเข้าไปใน Container ที่กำลังรันอยู่แล้ว
...
```
คือการเขียน Dockerfile ให้มี หลายขั้นตอน (Stages) โดยใช้คำสั่ง FROM หลายครั้งในไฟล์เดียว เพื่อแยกเครื่องมือที่ใช้ "สร้าง" (Build-time) ออกจากสิ่งที่ใช้ "รัน" (Run-time)
```
---
...
```
ทั้งสองคำสั่งทำงานร่วมกันเพื่อให้แอปใน Docker เข้าถึงไฟล์ฐานข้อมูลที่อยู่บนเครื่องของได้โดยตรง ทำให้คุณสามารถจัดการไฟล์ Database จากโฟลเดอร์ test-data นอก Docker ได้ทันทีครับ
```
...
1. `booking-backend:1.0` คืออะไร?
   ...
   > _คำตอบ_: คือ Docker Image Name และ Tag

2. ถ้าต้องการให้ ทดสอบ API ด้วย `curl http://localhost` ต้องทำอย่างไร?

   > _คำตอบ_: ต้องทำการ Map พอร์ตของ Container มาที่พอร์ต 80 ของเครื่อง Host
...
1. ขนาดของ `booking-frontend:1.0` เทียบกับ `booking-backend:1.0` ต่างกันอย่างไร? เพราะเหตุใด?
   ...
   > _คำตอบ_: Frontend เล็กกว่า Backend เพราะ Frontend มีแค่ไฟล์ Static (HTML/JS) และรันบน Nginx ที่เบามาก ส่วน Backend ต้องมี Node.js Runtime และ Library เสริม (node_modules) จำนวนมาก

2. `location /api/` ใน nginx.conf ทำหน้าที่อะไร?

   > _คำตอบ_: เป็น "บุรุษไปรษณีย์" ที่คอยส่งจดหมายจากหน้าเว็บไปให้ Server หลังบ้านเมื่อมีการเรียกใช้ข้อมูล
   
3. ทำไม Frontend Container ถึงใช้ชื่อ `backend` ในการ Proxy ได้?

   > _คำตอบ_: ทำหน้าที่เป็น Reverse Proxy เพื่อส่งต่อคำขอ (Forward) ที่ขึ้นต้นด้วย /api/ จากหน้าเว็บไปให้เครื่อง Backend ประมวลผลจริง
...
1. docker-compose.yml คืออะไร มีประโยชน์อย่างไร?
   ...
   > _คำตอบ_: คือไฟล์ตั้งค่าเพื่อ "สั่งรันหลายคอนเทนเนอร์พร้อมกัน" ด้วยคำสั่งเดียว ช่วยลดความยุ่งยากในการพิมพ์คำสั่งแยกทีละตัว

2. `restart: unless-stopped` หมายความว่าอย่างไร?

   > _คำตอบ_: สั่งให้คอนเทนเนอร์ "เริ่มทำงานใหม่เองเสมอ" เมื่อเครื่องเปิดหรือแอปพัง ยกเว้น กรณีที่เราสั่งหยุด (Stop) มันด้วยตัวเอง

3. Named Volume `sqlite_data` ต่างจาก Bind Mount อย่างไร? และทำไมต้องใช้กับ SQLite?

   > _คำตอบ_: Named Volume: Docker จัดการพื้นที่ให้เอง เสถียรกว่า และ Permission ไม่มีปัญหา
ทำไมต้องใช้: เพราะ "SQLite ทำงานได้เร็วกว่าและปลอดภัยกว่า" เมื่อรันบนพื้นที่ที่ Docker จัดการให้โดยเฉพาะ ลดโอกาสไฟล์พัง
...
**บันทึกรูปผลการทดลอง**![alt text](5.png)
```
บันทึกรูปที่นี่
```
...
1. `docker compose ps` แสดงสถานะ Service อย่างไร?
   ...
   > _คำตอบ_: แสดงรายชื่อ Container, Image ที่ใช้, สถานะการทำงาน (เช่น Up, Exit), และ การจับคู่พอร์ต (Port Mapping) ระหว่างเครื่องเรากับ Container

2. IP Address ของแต่ละ Container ในผลจาก `docker network inspect` คืออะไร?

   > _คำตอบ_: "หมายเลขประจำตัวภายในเครือข่าย Docker" (Internal IP) ที่ใช้ให้แต่ละ Container สื่อสารกันเองได้ โดยปกติจะอยู่ในวง 172.x.x.x

3. SQLite database file (`booking.db`) ถูกสร้างใน Path ใดภายใน Container?

   > _คำตอบ_: อยู่ที่ /app/data/booking.db ภายใน Container (ซึ่งถูกเชื่อมโยงกับ Named Volume ชื่อ sqlite_data เพื่อให้ข้อมูลไม่หายเมื่อลบคอนเทนเนอร์)
---
