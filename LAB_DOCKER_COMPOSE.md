...
```text
อธิบายคำสั่งที่นี่
เปิดเว็บเซิร์ฟเวอร์ nginx ใน container แล้วให้เราเข้าผ่าน http://localhost:8080
```
...
> **📝 บันทึกผล**: Container ID คือ: `fc5be861b646` และ IP Address คือ: `172.17.0.2`
...
1. `docker ps` กับ `docker ps -a` ต่างกันอย่างไร?
   ...
   > _คำตอบ_: ps = ดู ที่กำลังทำงาน
ps -a = ดู ทั้งหมด (all)

2. ขนาดของ `nginx:alpine` image คือเท่าไร?

   > _คำตอบ_: DISK USAGE   : 93.5 MB
CONTENT SIZE : 26.9 MB

3. flag `--rm` ใน `docker run` มีประโยชน์ในกรณีใด?

   > _คำตอบ_: flag --rm ใน docker run ใช้สำหรับลบ container อัตโนมัติเมื่อหยุดการทำงาน เหมาะสำหรับการใช้งานชั่วคราว เช่น การทดสอบหรือรันคำสั่งแบบครั้งเดียว เพื่อป้องกันไม่ให้มี container ค้างอยู่ในระบบ
4. `docker exec -it` ต่างจาก `docker run -it` อย่างไร?

   > _คำตอบ_: docker run -it ใช้สำหรับสร้างและรัน container ใหม่พร้อมเข้าใช้งานแบบ interactive ในขณะที่ docker exec -it ใช้สำหรับเข้าไปใช้งานภายใน container ที่กำลังทำงานอยู่แล้ว โดยไม่ต้องสร้างใหม่
...
```
Multi-stage Build คือการสร้าง Docker image แบบ “หลายขั้นตอน”
เพื่อให้ได้ image สุดท้ายที่ เล็ก สะอาด และไม่มีไฟล์ที่ไม่จำเป็น
```
---
...
> **📝 บันทึกผล**: ขนาดของ booking-backend:1.0 คือ: `550` MB 
...
```
คำสั่ง -e DB_PATH=/app/data/booking.db ใช้สำหรับกำหนดตัวแปรสภาพแวดล้อมให้กับ container เพื่อระบุตำแหน่งของไฟล์ฐานข้อมูล ส่วนคำสั่ง -v $(pwd)/test-data:/app/data ใช้สำหรับเชื่อมโฟลเดอร์จากเครื่อง host ไปยัง container ทำให้ข้อมูลสามารถถูกเก็บและใช้งานได้แม้ container จะถูกลบ
```
...
1. `booking-backend:1.0` คืออะไร?
   ...
   > _คำตอบ_: ชื่อและเวอร์ชันของ Docker image

2. ถ้าต้องการให้ ทดสอบ API ด้วย `curl http://localhost` ต้องทำอย่างไร?

   > _คำตอบ_: ต้องให้ container รันอยู่บนพอร์ตที่เราต้องการ เช่น 5000

docker run -d --name backend -p 5000:5000 booking-backend:1.0
ใช้ curl หรือ Invoke-RestMethod (PowerShell) เข้าถึง URL ของ server ที่รันใน container

ตัวอย่างเรียก root route:
...
ผลลัพธ์ควรเห็นข้อความตอบกลับจาก server เช่น:
...
> **📝 บันทึกผล**: booking-frontend:1.0 ขนาด `150` MB | booking-backend:1.0 ขนาด `120` MB
...
1. ขนาดของ `booking-frontend:1.0` เทียบกับ `booking-backend:1.0` ต่างกันอย่างไร? เพราะเหตุใด?
   ...
   > _คำตอบ_: booking-frontend:1.0 มักจะมีขนาด ใหญ่กว่า booking-backend:1.0 เพราะใน frontend เราต้องติดตั้ง Node.js, dependencies ของ React/Vite, และ build files ที่รวมถึงไฟล์ JavaScript, CSS, และ assets ต่าง ๆ ส่วน backend ใช้ Node.js + Express + SQLite + libraries เล็ก ๆ เท่านั้น ทำให้ image backend ขนาดเล็กกว่า

2. `location /api/` ใน nginx.conf ทำหน้าที่อะไร?

   > _คำตอบ_: ใน nginx config location /api/ ทำหน้าที่เป็น reverse proxy สำหรับ ส่ง request ที่มี path /api/ ไปยัง backend container ซึ่งอยู่ใน network เดียวกัน เช่น port 5000 ของ container backend

3. ทำไม Frontend Container ถึงใช้ชื่อ `backend` ในการ Proxy ได้?

   > _คำตอบ_: Docker network ทำให้ container สื่อสารด้วยชื่อ container ได้ ไม่ต้องใช้ IP จริง
---
...
สร้างไฟล์ `docker-compose.yml` ที่ Root Directory 
พร้อมเนื้อหาดังนี้:
...
1. docker-compose.yml คืออะไร มีประโยชน์อย่างไร?
   ...
   > _คำตอบ_: docker-compose.yml คือไฟล์ตั้งค่าที่ใช้กับ Docker Compose เพื่อกำหนดว่าโปรเจกต์ของเรามี container อะไรบ้าง แต่ละ container ใช้ image ไหน, port ไหน, volume อะไร, network ใดบ้าง และมีการเชื่อมต่อกันอย่างไร
ประโยชน์คือเราสามารถ รันหลาย container พร้อมกันได้ด้วยคำสั่งเดียว (docker-compose up) โดยไม่ต้องรันแต่ละ container แยกกันเอง ทำให้การจัดการระบบที่มีหลาย service เช่น frontend + backend + database ทำได้ง่ายและเป็นระเบียบ

2. `restart: unless-stopped` หมายความว่าอย่างไร?

   > _คำตอบ_: เป็นตัวเลือกของ Docker ให้ container รีสตาร์ทอัตโนมัติ

Docker จะพยายามเริ่ม container ใหม่ ถ้ามันเกิด crash หรือถูกปิดโดยไม่ตั้งใจ
จะ ไม่รีสตาร์ท ก็ต่อเมื่อผู้ใช้สั่ง docker stop หรือ docker-compose down เอง
นั่นช่วยให้ service ของเรา มีความเสถียรสูง แม้ container จะปิดไปโดยไม่คาดคิด

3. Named Volume `sqlite_data` ต่างจาก Bind Mount อย่างไร? และทำไมต้องใช้กับ SQLite?

   > _คำตอบ_: Bind Mount → เชื่อมโฟลเดอร์บน host (เครื่องเรา) เข้าไปใน container เช่น ./test-data:/app/data
Named Volume → Docker สร้าง storage พิเศษเก็บข้อมูลให้ container โดยไม่ผูกกับ path บน host

ข้อแตกต่างสำคัญ

Bind mount: ข้อมูลอยู่บน host → เห็นได้ง่าย แก้ไขตรง ๆ ได้ แต่ path อาจไม่เหมือนกันระหว่างเครื่อง
Named volume: Docker จัดการเอง → ย้าย container ได้ง่าย ข้อมูลไม่หายแม้ container จะถูกลบ

เหตุผลที่ใช้กับ SQLite

SQLite เป็น database แบบไฟล์เดียว (.db) ถ้า volume หายไป ไฟล์ database ก็หาย → ใช้ Named Volume หรือ Bind Mount เพื่อเก็บไฟล์ database ให้อยู่ถาวร
ป้องกันข้อมูลหายเวลาลบหรือ rebuild container
...
1. `docker compose ps` แสดงสถานะ Service อย่างไร?
   ...
   > _คำตอบ_: 
docker compose ps จะแสดง สถานะของแต่ละ Service/Container ที่ถูกสร้างจาก docker-compose.yml โดยบอกข้อมูลหลัก ๆ

2. IP Address ของแต่ละ Container ในผลจาก `docker network inspect` คืออะไร?

   > _คำตอบ_: เมื่อรัน docker network inspect booking-network จะได้ข้อมูล JSON ของ network แต่ละ Container จะมี IP Address ภายใน network

3. SQLite database file (`booking.db`) ถูกสร้างใน Path ใดภายใน Container?

   > _คำตอบ_:
ดังนั้น SQLite file จะถูกสร้างที่ /data/booking.db ภายใน backend container
Volume sqlite_data จะเก็บไฟล์นี้ไว้แม้ container จะถูก restart หรือ remove
