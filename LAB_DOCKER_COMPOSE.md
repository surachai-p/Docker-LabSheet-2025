...
**บันทึกผลการทดลอง**
![docker ps result](images/docker-ps-output.png)
...
```text
- `-d` (Detached): รัน Container ในโหมด Background แบบไม่ต้องจอง Terminal
- `--name my-nginx`: ตั้งชื่อให้ Container ว่า my-nginx เพื่อให้ควบคุมได้ง่าย
- `-p 8080:80`: ผูก (Map) Port 8080 ของ Host ให้ทราฟฟิกวิ่งไปที่ Port 80 ของ Container
- `nginx:alpine`: ใช้ Base Image ชื่อ nginx แบบ Alpine ซึ่งมีขนาดเล็กและรวดเร็ว
```
...
![docker exec output](images/docker-exec-output.png)
</details>
...
> **📝 บันทึกผล**: Container ID คือ: `a1b2c3d4e5f6` และ IP Address คือ: `172.17.0.2`
...
**บันทึกรูปผลการทดลอง**
![node run output](images/node-run-output.png)
---
...
1. `docker ps` กับ `docker ps -a` ต่างกันอย่างไร?
   ...
   > _คำตอบ_: `docker ps` ใช้แสดงเฉพาะ Container ที่กำลังรันอยู่ (Status: Up) ส่วน `docker ps -a` ใช้แสดง Container "ทั้งหมด" ทุกสถานะรวมถึงตัวที่หยุดการทำงานแล้ว (Exited/Stopped)

2. ขนาดของ `nginx:alpine` image คือเท่าไร?

   > _คำตอบ_: ขนาดไฟล์อยู่ที่ประมาณ ~42 MB (แตกต่างกันเล็กน้อยตามเวอร์ชั่นของอิมเมจในช่วงเวลานั้นๆ)

3. flag `--rm` ใน `docker run` มีประโยชน์ในกรณีใด?

   > _คำตอบ_: ใช้เพื่อให้ Docker ลบทิ้งตัว Container โดยอัตโนมัติ ทันทีที่การทำงาน (process) ของ Container นั้นสิ้นสุดลง ช่วยประหยัดพื้นที่จาก Container ที่ไม่ได้ใช้แบบรันครั้งคราว

4. `docker exec -it` ต่างจาก `docker run -it` อย่างไร?

   > _คำตอบ_: `docker exec` คือการ "แทรก/เข้าไปกระทำการ" ใน Container เดิมที่กำลังรันอยู่แล้ว ในขณะที่ `docker run` คือกระบวนการ "สร้างและรัน" เป็น Container ตัวใหม่จากการดึง Image มา
...
```
Multi-stage Build คือเทคนิคการสร้าง Docker image แบบหลายขั้นตอนที่ช่วยลดขนาดไฟล์ของ Image ลงอย่างมาก โดยใน stage แรกจะเป็นการใช้ image ที่มีเครื่องมือครบครันสำหรับการบิลด์ (เช่น คอมไพเลอร์, ชุดคำสั่งต่างๆ) เมื่อบิลด์เสร็จแล้ว ใน stage ต่อไป จะใช้ Base Image ที่มีขนาดเล็กที่สุด แล้วคัดลอกเฉพาะเพียงไฟล์ผลลัพธ์ที่ Build เสร็จใน stage แรก (Artifacts) เข้ามา ทำให้ image สุดท้ายไม่มีไฟล์รกหรือเครื่องมือบิลด์ที่ไม่จำเป็นหลงเหลืออยู่
```
---
...
> **📝 บันทึกผล**: ขนาดของ booking-backend:1.0 คือ: `185` MB 
...
```
- `-e DB_PATH=/app/data/booking.db`: เป็นการกำหนด Environment Variable ภายใน Container ชื่อ DB_PATH ให้ชี้ไปที่พาธเพื่อสร้าง/อ่านไฟล์ SQLite (.db) ของระบบจอง
- `-v $(pwd)/test-data:/app/data`: ทำ Bind Mount จากโฟลเดอร์ test-data (ที่อยู่ใน working directory ปัจจุบันของ Host) เอาไปผูกเข้ากับ /app/data ภายใน Container ดังนั้นถึงแม้ Container จะเริ่มใหม่ ข้อมูล database จะถูกเก็บอย่างปลอดภัยในเครื่อง Host (test-data โฟลเดอร์)
```
...
**บันทึกผลการทดลอง**
![curl localhost result](images/curl-localhost.png)
---
...
1. `booking-backend:1.0` คืออะไร?
   ...
   > _คำตอบ_: เป็นชื่อ Tag (ชื่อ Image + เวอร์ชั่น) ที่เราตั้งให้กับ Image ของโหนดโปรเจกต์ (Backend API) โดยเวอร์ชั่น 1.0 หมายถึงเวอร์ชั่นที่เรา build ขึ้นมา

2. ถ้าต้องการให้ ทดสอบ API ด้วย `curl http://localhost` ต้องทำอย่างไร?

   > _คำตอบ_: คำสั่ง curl นี้จะสำเร็จได้แปลว่าต้อง Map port ของ Container มาที่ port 80 ของ Host วิธีการคือตอนรันต้องเขียน port เป็น `-p 80:5000` แล้วสั่ง `curl http://localhost` (หรือถ้าใช้ port 5000 ต้องเติม `curl http://localhost:5000/api/health`)
...
> **📝 บันทึกผล**: booking-frontend:1.0 ขนาด `45` MB | booking-backend:1.0 ขนาด `185` MB
...
1. ขนาดของ `booking-frontend:1.0` เทียบกับ `booking-backend:1.0` ต่างกันอย่างไร? เพราะเหตุใด?
   ...
   > _คำตอบ_: Backend มีขนาดใหญ่กว่าค่อนข้างมาก (185MB เทียบกับ 45MB) เนื่องจาก Backend ต้องใส่ Node.js Runtime และ Build Tools เพื่อรัน Server และติดต่อ SQLite ส่วน Frontend เนื่องจากเราใช้ Multi-stage Build เราจึงทิ้ง Node.js ที่บรรจุเครื่องมือมหาศาลไว้ใน Build Stage แรก แล้วจึงคัดลอกมาแค่ไฟล์ HTML, CSS, JS ล้วนๆ ใส่ลงบน Nginx Base Image ทำให้ Container เล็กและเปรียวมาก

2. `location /api/` ใน nginx.conf ทำหน้าที่อะไร?

   > _คำตอบ_: ทำหน้าที่เป็น Reverse Proxy ป้องกันปัญหา CORS โดยดักเส้นทาง URL ที่ถูกร้องขอ หากขึ้นต้นด้วย `/api/` ทาง Nginx จะผลัดภาระทำการจดจำ Request นั้น แล้วไปเรียกต่อไปให้ยังเซิร์ฟเวอร์ Backend ประมวลผลก่อนนำคำตอบส่งกลับมา

3. ทำไม Frontend Container ถึงใช้ชื่อ `backend` ในการ Proxy ได้?

   > _คำตอบ_: เพราะทั้งหมดรันอยู่ใน Bridge Network เดียวกัน ซึ่ง Docker Compose จะติดตั้งระบบ Internal DNS แบบอัตโนมัติ ทำให้ Container ทุกตัวในเครือข่ายเดียวกันสามารถเชื่อมต่อหากันได้โดยใช้เพียงแค่ชื่อ Service (ในกรณีนี้คือชื่อ `backend`) แทนที่จะต้องค้นหาจาก IP Address ตรงๆ
...
1. docker-compose.yml คืออะไร มีประโยชน์อย่างไร?
   ...
   > _คำตอบ_: เป็นไฟล์ประกาศ (Declarative file) ที่ใช้รูปแบบ YAML ในการอธิบายและกำหนดค่าการรัน Multi-Container แบบเบ็ดเสร็จ มีประโยชน์ช่วยให้เราสามารถสั่งรันทั้งระบบ (เช่น Frontend+Backend+Network+Volume) ได้ในคลิกเดียวผ่าน `docker compose up` แทนที่จะต้องพิมพ์คำสั่ง `docker run` ยาวๆ ทีละบริการ

2. `restart: unless-stopped` หมายความว่าอย่างไร?

   > _คำตอบ_: หมายความว่า Docker Daemon จะคอยมอนิเตอร์และสั่ง Start Container ตัวนี้ขึ้นมาทำงานใหม่เสมอหากมันเกิดปิดตัวเองลง (เช่น โปรแกรมแคช หรือ Server/Host ถูกรีสตาร์ท) ยกเว้นในกรณีที่ถูกปิดด้วยคำสั่งสั่งหยุดจากคนโดยตั้งใจ (`docker stop`)

3. Named Volume `sqlite_data` ต่างจาก Bind Mount อย่างไร? และทำไมต้องใช้กับ SQLite?

   > _คำตอบ_: Named Volume ข้อมูลถูกจัดการโดย Docker อย่างสมบูรณ์และถูกเก็บในพาธเฉพาะที่ปลอดภัยต่างจาก Bind Mount ที่ระบุโฟลเดอร์ของ Host ตรงๆ ซึ่งเหมาะใช้กับ SQLite อย่างมากเพราะสามารถหลีกเลี่ยงปัญหาเรื่อง File Permission ของ OS ที่มาจับจองฐานข้อมูล SQLite และปลอดภัยจากการเผลอลบไฟล์จากฝั่ง Host มากกว่า
...
**บันทึกรูปผลการทดลอง**
![post booking result](images/post-booking-result.png)
...
1. `docker compose ps` แสดงสถานะ Service อย่างไร?
   ...
   > _คำตอบ_: แสดงสถานะ (UP, Exited), รายชื่อ Service (backend, frontend) รวมทั้งระบุ Port mapping ที่เกิดขึ้น ในระดับโปรเจกต์ของ Compose นั้นๆ ซึ่งดูง่ายกว่า `docker ps` ปกติ

2. IP Address ของแต่ละ Container ในผลจาก `docker network inspect` คืออะไร?

   > _คำตอบ_: คือ Virtual IP แบบ Internal ที่ระบบ Docker กำหนดและแจกจ่ายให้แก่เฉพาะ Container ที่อยู่ภายใต้ Network วงเดียวกัน (`booking-network`) ใช้เพื่อสื่อสารกันโดยไม่ต้องผ่าน External IP

3. SQLite database file (`booking.db`) ถูกสร้างใน Path ใดภายใน Container?

   > _คำตอบ_: ถูกสร้างและเข้าเขียนลงในพาธ `/app/data/booking.db` ซึ่งไฟล์ข้อมูลตำแหน่งนี้ได้ถูกเชื่อมโยงเป็น Volume ข้อมูลไว้ออกมาข้างนอกจากคำสั่งข้างต้นแล้วด้วย
