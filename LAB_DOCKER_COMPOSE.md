...
```text
อธิบายคำสั่งที่นี่
docker run => คำสั่งหลักในการ สร้าง (Create) และ รัน (Start) Container ตัวใหม่
-d => ย่อมาจาก Detached mode สั่งให้ Container ทำงานอยู่เบื้องหลัง (Background) ทำให้เรายังใช้หน้าจอ Terminal ต่อไปได้โดยที่โปรแกรมไม่หยุดทำงาน
--name my-nginx => เป็นการตั้งชื่อให้กับ Container นี้ว่า "my-nginx" เพื่อให้ง่ายต่อการเรียกใช้หรือสั่งหยุด (ถ้าไม่ระบุ Docker จะสุ่มชื่อให้เอง)
-p 8080:80 => คือการทำ Port Mapping (เชื่อมต่อพอร์ต)8080 (ตัวหน้า): พอร์ตบนเครื่องคอมพิวเตอร์ของเรา (Host) 80 (ตัวหลัง): พอร์ตภายในของ Container(ถ้าเราเข้าเว็บผ่าน localhost:8080 มันจะส่งข้อมูลไปที่พอร์ต 80 ของ Nginx ใน Container)
nginx:alpine => คือชื่อของ Image ที่เราจะนำมาใช้ nginx คือชื่อซอฟต์แวร์ :alpine คือ Tag หรือเวอร์ชันที่เน้นความเล็กและประหยัดพื้นที่
```
...
> **📝 บันทึกผล**: Container ID คือ: `ad32c55e0aa8` และ IP Address คือ: `172.17.0.2`
...
1. `docker ps` กับ `docker ps -a` ต่างกันอย่างไร?
   ...
   > _คำตอบ_: docker ps => แสดงเฉพาะ Container ที่ กำลังรันอยู่ เท่านั้น สถานะของ Container คือ Up ...
   docker ps -a => แสดงทุก Container ที่เคยสร้างไว้ในเครื่อง สถานะของ Container คือ Up ..., Exited (...), Created
...
3. flag `--rm` ใน `docker run` มีประโยชน์ในกรณีใด?
   ...
   > _คำตอบ_: flag --rm (ย่อมาจาก Remove) มีไว้เพื่อ "ใช้แล้วทิ้ง" มันมีประโยชน์สูงสุดในกรณีที่เราต้องการรัน Container เพื่อทำงานบางอย่างให้เสร็จ แล้วไม่อยากให้ตัว Container นั้นค้างเป็นขยะอยู่ในเครื่องหลังจากจบงาน

4. `docker exec -it` ต่างจาก `docker run -it` อย่างไร?

   > _คำตอบ_: docker exec -it => ใช้สั่งงาน container ที่มีอยู่แล้ว ซึ่งจะทำงานต่อจากสถานะปัจจุบัน เมื่อพิมพ์ exit Container ยังทำงานต่อ ตามปกติ ใช้ตอนจะมุดเข้าไปเช็คไฟล์/แก้ไขงาน
   docker run -it => สร้าง Container ใหม่ขึ้นมาทำงาน โดยเริ่มต้นจากศูนย์ (Fresh Start) เมื่อพิมพ์ exit Container จะ หยุดทำงาน (Stop) ใช้สำหรับเริ่มรัน App ครั้งแรก
...
6. ต้องการลบ image ชื่อ nginx:alpine จะต้องเขียนคำสั่งอย่างไร หากมีหลายขั้นตอนที่ต้องเขียน ให้เขียนลำดับคำสั่งให้ถูกต้อง
   ```bash
   เขียนคำสั่งที่นี่ 
   docker ps -a #เช็คว่ามี Container ไหนรัน nginx:alpine อยู่บ้าง
   docker stop my-nginx #หยุดการทำงานของ Container (ถ้ามันยังรันอยู่)
   docker rm my-nginx #ลบ Container นั้นทิ้ง เพราะถ้าไม่ลบ Container ทิ้ง Docker จะล็อก Image ไว้ไม่ให้เราลบ
   ```
7. เมื่อลบ image แล้วให้ทำการ pull image กลับมาเหมือนเดิม พร้อมทำการรัน container
   ```bash
   เขียนคำสั่งที่นี่
   1. docker pull nginx:alpine => การ Pull Image กลับมา (แบบแมนนวล)หากต้องการแค่ดาวน์โหลด Image มาเก็บไว้เฉยๆ ยังไม่ต้องการรัน
   2.docker run -d --name my-nginx -p 8080:80 nginx:alpine => การสั่งรัน Container จะ Pull ให้อัตโนมัติ
   ```
---
...
```
ตอบคำถามที่นี่ 
Multi-stage Build คือเทคนิคการเขียน Dockerfile ที่ช่วยให้สามารถ "แยกขั้นตอนการประกอบร่าง (Build)" ออกจาก "ขั้นตอนการใช้งานจริง (Run)" เพื่อให้ได้ Docker Image สุดท้ายที่มีขนาดเล็กและปลอดภัยที่สุด
```
---
...
> **📝 บันทึกผล**: ขนาดของ booking-backend:1.0 คือ: `183` MB 
...
```
อธิบายคำสั่งที่นี่
-e (Environment) => กำหนดพาธ "ภายใน Container" ว่าแอปจะไปอ่าน/เขียนไฟล์ Database ที่ไหน (ในที่นี้คือ /app/data/booking.db)
-v (Volume) => เชื่อมพาธ "เครื่องเรา" ($(pwd)/test-data) เข้ากับพาธ "ใน Container" (/app/data) เพื่อให้ไฟล์ Database ถูกเก็บไว้ที่เครื่องเราถาวร ไม่หายไปพร้อม Container
```
...
```
![alt text](image-4.png)![alt text](image-4-2.png)
---
...
1. `booking-backend:1.0` คืออะไร?
   ...
   > _คำตอบ_: booking-backend:1.0 => คือ Docker Image ที่ทำหน้าที่เป็นต้นแบบ (Template) สำหรับรันระบบหลังบ้าน (Backend) ของแอปพลิเคชันจองที่พัก 
   booking-backend => (Repository Name) คือชื่อของโปรเจกต์หรือตัวแอปพลิเคชัน (ในที่นี้คือ Backend ของระบบ Booking)
   :1.0 => (Tag) คือเลขเวอร์ชัน (Version) ของ Image นั้นๆ เพื่อให้ระบุได้ว่ากำลังใช้งานตัวไหนอยู่
2. ถ้าต้องการให้ ทดสอบ API ด้วย `curl http://localhost` ต้องทำอย่างไร?

   > _คำตอบ_: 1.ต้อง Map Port 80 ตอนรัน Container ใช้พารามิเตอร์ -p 80:5000 เพื่อให้เรียกผ่าน localhost ปกติ (Port 80) แล้ววิ่งไปหาแอปข้างใน (Port 5000)
   2.คำสั่งรัน: docker run -p 80:5000 booking-backend:1.0
   3.คำสั่งทดสอบ: curl http://localhost
...
> **📝 บันทึกผล**: booking-frontend:1.0 ขนาด `26.1` MB | booking-backend:1.0 ขนาด `183` MB
...
1. ขนาดของ `booking-frontend:1.0` เทียบกับ `booking-backend:1.0` ต่างกันอย่างไร? เพราะเหตุใด?
   ...
   > _คำตอบ_: Frontend (26.1 MB) => "เล็กและเบา" เพราะใช้ Nginx เป็นตัวรัน ซึ่งทำหน้าที่แค่ส่งไฟล์ภาพและตัวอักษร (Static Files) ให้ Browser เท่านั้น ไม่ต้องมีโปรแกรมช่วยคำนวณอะไรซับซ้อน
   Backend (183 MB) => "ใหญ่และหนัก" เพราะต้องแบก Node.js Runtime (ตัวรันภาษา JS) และโฟลเดอร์ node_modules ที่รวม Library ต่างๆ ไว้ใช้ประมวลผลข้อมูล รวมถึงมีพวกเครื่องมือช่วยคอมไพล์ (Build Tools) ติดมาด้วย

2. `location /api/` ใน nginx.conf ทำหน้าที่อะไร?

   > _คำตอบ_: หน้าที่ของ location /api/ => 1.เป็นด่านหน้าคอยดักจับทุก Request ที่มีคำว่า /api/ เพื่อแยกออกจากไฟล์หน้าเว็บปกติ
   2.ส่งต่อข้อมูล ใช้ proxy_pass โยนคำขอนั้นไปให้ Container ฝั่ง Backend ประมวลผลต่อทันที
   3.แก้ปัญหา CORS ทำให้ Browser มองว่าหน้าบ้านและหลังบ้านมาจากที่เดียวกัน จึงไม่โดนบล็อกความปลอดภัย

3. ทำไม Frontend Container ถึงใช้ชื่อ `backend` ในการ Proxy ได้?

   > _คำตอบ_: Docker มีระบบ Embedded DNS ที่คอยจับคู่ "ชื่อ Container" เข้ากับ "เลข IP" ให้เรา มีการอัปเดตอัตโนมัติแม้ IP ของ Container จะเปลี่ยนไปตอน Restart แต่เรายังเรียกชื่อเดิมได้เสมอโดยไม่ต้องแก้โค้ด ฟีเจอร์นี้จะทำงานก็ต่อเมื่อทั้งสอง Container เชื่อมต่ออยู่ใน Docker Network เดียวกันเท่านั้น
...
1. docker-compose.yml คืออะไร มีประโยชน์อย่างไร?
   ...
   > _คำตอบ_: docker-compose.yml คือไฟล์ที่ใช้กำหนดโครงสร้างและตั้งค่าการทำงานของ Multi-container Docker applications (แอปที่มีหลายตู้คอนเทนเนอร์) โดยรวบรวมคำสั่ง docker run ยาวๆ มาเขียนเป็นไฟล์เดียวในรูปแบบภาษา YAML 
   ประโยชน์ได้แก่ 1.รันทุกอย่างในคำสั่งเดียว โดยใช้คำสั่ง docker-compose up -d ระบบจะสร้างและรันทุกคอนเทนเนอร์ให้พร้อมกันทันที
   2.จัดการ Network และ Volume อัตโนมัติ ระบบจะสร้าง Network วงเดียวกันให้ทุก Service คุยกันด้วยชื่อ (เช่น backend) ได้เลย และจองพื้นที่เก็บข้อมูล (Volumes) ให้ตามที่ตั้งค่าไว้ในไฟล์
   3.ช่วยให้การย้ายโปรเจกต์ไปรันที่เครื่องเพื่อนหรือบน Server ทำได้ง่ายและแม่นยำ เพราะการตั้งค่าทุกอย่าง (Port, Environment, Image) ถูกบันทึกไว้ในไฟล์นี้หมดแล้ว

2. `restart: unless-stopped` หมายความว่าอย่างไร?

   > _คำตอบ_: restart: unless-stopped => คือการตั้งค่า นโยบายการเริ่มทำงานใหม่ (Restart Policy) ของ Container มีการรันอัตโนมัติ เมื่อ Docker Engine เริ่มทำงาน (เช่น หลังเปิดคอมพิวเตอร์ใหม่) Container นี้จะถูกสั่งให้ Start ขึ้นมาเองทันที หากแอปภายในเกิด Error หรือ Container ดับไปเองโดยไม่ตั้งใจ Docker จะ Restart ให้กลับมาทำงานใหม่เสมอ และข้อยกเว้นเดียวคือถ้าเราเป็นคนสั่ง docker stop ด้วยตัวเอง Docker จะไม่แอบสั่งรันมันขึ้นมาใหม่จนกว่าเราจะสั่ง Start อีกครั้ง
3. Named Volume `sqlite_data` ต่างจาก Bind Mount อย่างไร? และทำไมต้องใช้กับ SQLite?

   > _คำตอบ_: Named Volume มีประสิทธิภาพสูงกว่าและเสถียรกว่าบน Windows/Mac เพราะลดปัญหาเรื่องสิทธิ์การเข้าถึงไฟล์ (Permission) และความขัดแย้งของระบบไฟล์ การใช้กับ SQLite เพื่อป้องกันข้อมูลสูญหาย ลดโอกาสไฟล์พัง และสามารถย้ายเครื่องได้สะดวกโดยไม่ต้องกังวลเรื่อง Path ของโฟลเดอร์ในแต่ละเครื่องที่อาจจะไม่เหมือนกัน
---
...
1. `docker compose ps` แสดงสถานะ Service อย่างไร?
   ...
   > _คำตอบ_: docker compose ps => ทำหน้าที่แสดงสถานะการทำงานของทุก Service ที่ถูกระบุไว้ในไฟล์ docker-compose.yml ของโปรเจกต์  โดยแสดง NAME & IMAGE คือการ แสดงชื่อคอนเทนเนอร์และ Image ที่ใช้ (เช่น booking-frontend และ booking-frontend:1.0)
   STATUS บอกสถานะปัจจุบันว่ากำลังรันอยู่ (Running/Up), หยุดทำงาน (Exited), หรือกำลังเริ่ม (Starting)
   PORTS แสดงการจับคู่พอร์ตระหว่างเครื่องเรากับคอนเทนเนอร์ (เช่น 0.0.0.0:3000->80/tcp)

2. IP Address ของแต่ละ Container ในผลจาก `docker network inspect` คืออะไร?

   > _คำตอบ_: หมายเลข IP Address คือที่อยู่เสมือนภายในระบบเครือข่ายของ Docker (Docker Bridge Network) เพื่อให้แต่ละ Container คุยกันได้

3. SQLite database file (`booking.db`) ถูกสร้างใน Path ใดภายใน Container?

   > _คำตอบ_: ถูกสร้างใน Path /app/data/booking.db ภายใน Container
