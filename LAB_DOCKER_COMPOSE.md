...
```
[คำสั่งเต็ม: docker run -d --name my-nginx -p 8080:80 nginx:alpine

docker run 🏃‍♂️

ความหมาย: เป็นคำสั่งหลักสำหรับ "สร้างและเปิดใช้งาน" Container ใหม่

เปรียบเทียบ: เหมือนการสั่ง "สร้างและเปิดร้านใหม่" ในห้าง

-d (ย่อมาจาก --detach) 👻

ความหมาย: สั่งให้ Container รันอยู่เบื้องหลัง (Background) ทำให้เราสามารถพิมพ์คำสั่งอื่นๆ ใน Terminal ต่อได้เลยโดยไม่ต้องรอให้ Container ปิดการทำงาน

เปรียบเทียบ: ให้ร้านเปิดทำการไปเรื่อยๆ โดยที่เรา (ผู้จัดการ) ไม่ต้องไปยืนเฝ้าหน้าร้านตลอดเวลา สามารถไปทำธุระอื่นต่อได้

--name my-nginx 🏷️

ความหมาย: ตั้งชื่อให้กับ Container นี้ว่า "my-nginx" (ถ้าเราไม่ใส่ Docker จะสุ่มชื่อแปลกๆ มาให้)

เปรียบเทียบ: การขึ้นป้ายชื่อร้านว่า "my-nginx" เพื่อให้เราจำง่ายและเรียกใช้ถูกร้านในอนาคต

-p 8080:80 (ย่อมาจาก --publish) 🚪

ความหมาย: การเชื่อม Port (ช่องทางสื่อสาร) ตัวเลขซ้าย 8080 คือ Port ของเครื่องคอมพิวเตอร์เรา (Host) ส่วนตัวเลขขวา 80 คือ Port ภายใน Container

เปรียบเทียบ: สร้างประตูเชื่อม! โดยให้ลูกค้าที่เดินมาทาง ประตูห้างหมายเลข 8080 ทะลุเข้าไปเจอ ประตูร้านหมายเลข 80 (ซึ่งเป็นประตูมาตรฐานที่เว็บเซิร์ฟเวอร์ Nginx เปิดรอรับลูกค้าอยู่)

nginx:alpine 📦

ความหมาย: ชื่อของ Image (แผ่นเกม/พิมพ์เขียว) ที่เราจะเอามาสร้าง Container โดยใช้แอปชื่อ nginx และเลือกเวอร์ชัน (Tag) ที่ชื่อ alpine ซึ่งเป็นระบบปฏิบัติการ Linux ขนาดจิ๋ว ทำให้โหลดไวและเบาเครื่องมาก

เปรียบเทียบ: เลือกใช้ "แฟรนไชส์ร้าน Nginx รุ่นกะทัดรัด (alpine)" มาเป็นรูปแบบในการเปิดร้านนี้]
...
> **📝 บันทึกผล**: Container ID คือ: `bd7f5c8bece8` และ IP Address คือ: `172.17.0.2`
...
[PS C:\Users\Lenovo\Desktop\docker\Docker-LabSheet-2025\booking-app-demo-2025> docker run -it --rm node:20-alpine sh
Unable to find image 'node:20-alpine' locally
20-alpine: Pulling from library/node
ed2fdcee5269: Pull complete
7ad115895a6a: Pull complete
9d10d4687fae: Pull complete
a0163922be89: Download complete
1e93025935e1: Download complete
...
/ # node -e "console.log('Hello from Docker! Running in:', process.env.HOSTNAME)"
Hello from Docker! Running in: 02b39302ec41
/ # exit]
```
---
...
1. `docker ps` กับ `docker ps -a` ต่างกันอย่างไร?
   ...
   > _คำตอบ_: ลองนึกถึงตอนที่เราดูแลห้างสรรพสินค้าครับ คำสั่ง docker ps ปกติจะแสดงเฉพาะร้าน (Container) ที่กำลัง "เปิดทำการอยู่" (สถานะ Up) ในขณะนั้น

ตัวอักษร -a ย่อมาจากคำว่า "all" คุณคิดว่าถ้านำมาต่อท้าย จะทำให้ผลลัพธ์บนหน้าจอเปลี่ยนไปอย่างไรครับ? จะมีร้านสถานะไหนโผล่เพิ่มขึ้นมาให้เราเห็นบ้าง?

2. ขนาดของ `nginx:alpine` image คือเท่าไร?

   > _คำตอบ_: เราไม่จำเป็นต้องเดาหรือจำตัวเลขนี้ครับ Docker มีคำสั่งสำหรับดูรายชื่อ Image (แผ่นเกม) ทั้งหมดที่เราโหลดเก็บไว้ในเครื่อง ซึ่งตารางผลลัพธ์จะมีคอลัมน์ SIZE บอกขนาดไว้อย่างชัดเจน

ลองดูข้อมูลจากภาคทฤษฎีหัวข้อ 1.7 คำสั่งที่ใช้ในการ "แสดงรายชื่อ Image ทั้งหมด" ในเครื่องของเราคือคำสั่งอะไรครับ? (ซึ่งคำสั่งนี้สามารถนำไปใช้เป็นคำตอบของข้อ 5 ได้พอดีเลยครับ)

3. flag `--rm` ใน `docker run` มีประโยชน์ในกรณีใด?

   > _คำตอบ_:ลองเปรียบเทียบกับการตั้ง "บูธจัดกิจกรรมชั่วคราว" ในห้างดูครับ บูธพวกนี้พอจัดงานเสร็จปุ๊บ (ทำงานเสร็จ/ปิดการทำงาน) เจ้าหน้าที่จะรื้อถอนและเคลียร์พื้นที่ออกไปทันทีโดยอัตโนมัติ ไม่ปล่อยโครงสร้างทิ้งไว้ให้เกะกะพื้นที่ห้าง 🧹

ในโลกของ Docker ถ้าเราไม่ใส่ --rm เวลา Container หยุดทำงาน มันจะกลายสภาพเป็น Container ที่ตายแล้ว (Exited) แต่ยังกินพื้นที่อยู่ในเครื่อง (ซึ่งเราจะเห็นได้จากการพิมพ์ docker ps -a)

จากการเปรียบเทียบนี้ คุณคิดว่าถ้าเราใส่ --rm เข้าไปตอนสั่งรัน จะเกิดอะไรขึ้นหลังจากที่เรากดหยุด (Stop) Container ตัวนั้นครับ?
4. `docker exec -it` ต่างจาก `docker run -it` อย่างไร?

   > _คำตอบ_: run = สร้างร้านขึ้นมาใหม่

exec  = ทำงานกับร้านที่เปิดอยู่แล้ว

ดังนั้น docker run -it จะเหมือนการสั่งให้ "สร้างร้านใหม่ขึ้นมา 1 ร้าน แล้วเราก็เดินเข้าไปข้างในทันที"

ส่วน docker exec -it จะหมายถึงอะไรครับ? ลองอธิบายโดยใช้การเปรียบเทียบเรื่องการเดินเข้าร้านดูครับ

5. ต้องการดูว่าในเครื่องมี image อะไรอยู่บ้าง จะเขียนคำสั่งอย่างไร
   ```bash
   เขียนคำสั่งที่นี่ docker images
   ```
6. ต้องการลบ image ชื่อ nginx:alpine จะต้องเขียนคำสั่งอย่างไร หากมีหลายขั้นตอนที่ต้องเขียน ให้เขียนลำดับคำสั่งให้ถูกต้อง
   ```bash
   เขียนคำสั่งที่นี่
   # 1. หยุดการทำงานของ Container ก่อน
docker stop my-nginx

# 2. ลบ Container ออกจากระบบ
docker rm my-nginx

# 3. ลบ Image ทิ้ง (rmi ย่อมาจาก remove image)
...
# 2. สั่งรัน Container ใหม่ พร้อม map port และตั้งชื่อ
docker run -d --name my-nginx -p 8080:80 nginx:alpine
   ```
---
...
```
ตอบคำถามที่นี่
[Multi-stage Build คือการแบ่งขั้นตอนการสร้าง Docker Image ไว้ในไฟล์เดียว (โดยใช้คำสั่ง FROM หลายครั้ง)

สเตปแรก (Build): ใช้ลงเครื่องมือและแพ็กโค้ด (ไฟล์จะหนัก)

สเตปสุดท้าย (Run): ก๊อปปี้เฉพาะ "แอปที่พร้อมใช้งาน" จากสเตปแรกมารัน

สรุปข้อดี: ทำให้ Image ตัวสุดท้ายมี ขนาดเล็กมาก และ ปลอดภัย เพราะทิ้งไฟล์ขยะและเครื่องมือหนักๆ ไว้ในสเตปแรกทั้งหมดครับ]
```
---
...
> **📝 บันทึกผล**: ขนาดของ booking-backend:1.0 คือ: `183MB` MB 
...
```
อธิบายคำสั่งที่นี่
[1. -e DB_PATH=/app/data/booking.db (ตั้งค่า Environment Variable)

-e ย่อมาจาก Environment: เปรียบเหมือน "การเขียนป้ายคำสั่งแปะไว้ให้พนักงานในร้าน"

ความหมาย: เรากำลังบอกตัวโค้ดแอป Backend ว่า "เฮ้! ถ้าจะหาไฟล์ฐานข้อมูล (DB_PATH) ให้เข้าไปหาหรือสร้างไฟล์ชื่อ booking.db ในโฟลเดอร์ /app/data ของ Container นะ"

2. -v $(pwd)/test-data:/app/data (การทำ Volume แบบ Bind Mount)
*

-v ย่อมาจาก Volume: เปรียบเหมือน "การสร้างท่อส่งของเชื่อมระหว่างในร้านกับนอกร้าน"

ฝั่งซ้าย $(pwd)/test-data: คือโฟลเดอร์ในเครื่องคอมพิวเตอร์ของคุณ (Host)

ฝั่งขวา /app/data: คือโฟลเดอร์ภายใน Container

ความหมาย: เรากำลังสั่งเชื่อมโฟลเดอร์ทั้งสองฝั่งเข้าด้วยกัน อะไรก็ตามที่ถูกสร้างหรือแก้ไขใน /app/data (ใน Container) จะมาโผล่ที่ test-data (ในเครื่องคอมพิวเตอร์ของคุณ) แบบเรียลไทม์!]
```
...
1. `booking-backend:1.0` คืออะไร?
   ...
   > _คำตอบ_: แนวคิด: มันคือ ชื่อและเวอร์ชันของ Docker Image ที่เราสร้างขึ้นมาครับ

คำอธิบาย: * booking-backend คือ ชื่อของ Image (Repository name)

1.0 คือ แท็ก (Tag) ซึ่งมักจะใช้บอก "เวอร์ชัน" ของแอปพลิเคชันนั้นๆ ครับ

2. ถ้าต้องการให้ ทดสอบ API ด้วย `curl http://localhost` ต้องทำอย่างไร?

   > _คำตอบ_: แนวคิด: ปกติแล้วเวลาเราพิมพ์ URL เว็บไซต์โดยไม่ใส่ตัวเลขพอร์ตด้านหลัง (เช่น http://localhost เฉยๆ) ระบบจะถือว่าเรากำลังเรียกไปที่ พอร์ต 80 (พอร์ตมาตรฐานของ HTTP) เสมอครับ

คำอธิบาย: ตอนนี้เรารันด้วย -p 5000:5000 (เอาพอร์ต 5000 ของเครื่องเรา ไปต่อกับ 5000 ของ Container) ถ้าเราอยากให้มันเรียกผ่านพอร์ต 80 ของเครื่องเราได้ เราต้องเปลี่ยนคำสั่งตอนรัน docker run ตรงส่วนการเชื่อมพอร์ต (Port Mapping) เป็นแบบนี้ครับ:
👉 -p 80:5000 (เอาพอร์ต 80 ของเครื่องเรา ทะลุไปหาพอร์ต 5000 ของ Container)
...
> **📝 บันทึกผล**: booking-frontend:1.0 ขนาด `92 26` MB | booking-backend:1.0 ขนาด `698 183` MB
...
1. ขนาดของ `booking-frontend:1.0` เทียบกับ `booking-backend:1.0` ต่างกันอย่างไร? เพราะเหตุใด?
   ...
   > _คำตอบ_: booking-frontend:1.0 จะมีขนาด เล็กกว่า booking-backend:1.0 มากครับ
2. `location /api/` ใน nginx.conf ทำหน้าที่อะไร?

   > _คำตอบ_: ทำหน้าที่เป็น Reverse Proxy (ตัวส่งต่อคำขอ) ครับ

3. ทำไม Frontend Container ถึงใช้ชื่อ `backend` ในการ Proxy ได้?

   > _คำตอบ_: เพราะ Container ทั้งสองตัวถูกกำหนดให้เชื่อมต่ออยู่ใน Docker Network วงเดียวกัน (เช่นวงที่ชื่อว่า booking-network) ครับ
...
1. docker-compose.yml คืออะไร มีประโยชน์อย่างไร?
   ...
   > _คำตอบ_: .คือไฟล์ Configuration (เขียนด้วยรูปแบบ YAML) ที่ใช้กำหนดรายละเอียดการทำงานของ Container หลายๆ ตัวไว้ในที่เดียว ประโยชน์คือช่วยให้เราสามารถรันและจัดการแอปพลิเคชันที่มีหลายส่วน (เช่น Frontend, Backend, Database) ได้พร้อมกันผ่านคำสั่งเดียว (docker compose up) แทนที่จะต้องมาพิมพ์คำสั่ง docker run ยาวๆ ทีละตัวครับ


2. `restart: unless-stopped` หมายความว่าอย่างไร?

   > _คำตอบ_: เป็นคำสั่งที่บอกให้ Docker พยายามเปิด Container นี้ขึ้นมาทำงานใหม่โดยอัตโนมัติเสมอ (เช่น ในกรณีที่แอปพัง หรือเครื่องเซิร์ฟเวอร์ถูกรีสตาร์ท) ยกเว้น ว่าเราจะเป็นคนสั่งหยุดมันด้วยตัวเอง (ผ่านคำสั่ง docker stop หรือ docker compose stop) ครับ

3. Named Volume `sqlite_data` ต่างจาก Bind Mount อย่างไร? และทำไมต้องใช้กับ SQLite?

   > _คำตอบ_: ความต่าง: Bind Mount เป็นการผูกโฟลเดอร์ในเครื่องเราเข้ากับ Container โดยตรง (ต้องระบุ Path ชัดเจน) ส่วน Named Volume เป็นพื้นที่ที่ Docker สร้างและจัดการให้เองโดยอัตโนมัติ (ไม่ต้องสนว่าไฟล์จริงอยู่ตรงไหนบนเครื่องเรา) ทำให้ย้ายไปรันเครื่องอื่นได้ง่ายกว่า

ทำไมต้องใช้กับ SQLite: เพราะ SQLite เก็บข้อมูลเป็นไฟล์ (.db) การใช้ Named Volume จะช่วยการันตีว่าไฟล์ฐานข้อมูลนี้จะถูกเก็บรักษาไว้อย่างปลอดภัยและไม่สูญหายแม้ Container จะถูกลบหรือสร้างใหม่ แถมยังทำงานได้เสถียรกว่าการอ้างอิง Path บนเครื่อง Host ครับ
---
...
> **📝 บันทึกผลการทดลอง**:
> - IP ของ frontend: `172.18.0.3/16`
> - IP ของ backend: `172.18.0.2/16`
...
1. `docker compose ps` แสดงสถานะ Service อย่างไร?
   ...
   > _คำตอบ_:แสดงผลเป็นตารางสรุปข้อมูลของ Container ทั้งหมดในโปรเจกต์ โดยจะบอกรายละเอียดหลักๆ คือ ชื่อ Container (Name), สถานะการทำงานปัจจุบัน (Status เช่น Up, Exited, หรือ healthy), และการตั้งค่าพอร์ต (Ports) ที่เชื่อมต่อระหว่างเครื่อง Host กับ Container ครับ

2. IP Address ของแต่ละ Container ในผลจาก `docker network inspect` คืออะไร?

   > _คำตอบ_: คือหมายเลข IP จำลอง (Internal IP) ที่ระบบ Docker สร้างและแจกจ่ายให้กับแต่ละ Container เพื่อให้พวกมันสามารถสื่อสารและส่งข้อมูลหากันได้เองภายในวงเครือข่าย (Network) เดียวกัน โดยไม่จำเป็นต้องออกไปใช้ระบบเน็ตเวิร์คหลักของเครื่อง Host ครับ

3. SQLite database file (`booking.db`) ถูกสร้างใน Path ใดภายใน Container?

   > _คำตอบ_: ถูกสร้างและเก็บไว้ที่ Path /app/data/booking.db ภายใน Backend Container ครับ (ซึ่งโฟลเดอร์ /app/data นี้ได้ถูกเชื่อมต่อกับ Named Volume sqlite_data เอาไว้ เพื่อให้ข้อมูลการจองห้องพักไม่สูญหายแม้จะลบ Container ทิ้งไปแล้วครับ
---
