...
```
<img width="757" height="729" alt="image" src="https://github.com/user-attachments/assets/a76cf7e3-f673-420a-b11c-773986213485" />
...
--- <img width="1919" height="1018" alt="image" src="https://github.com/user-attachments/assets/c02c13cb-c065-4eea-86d7-8d06a7f8143f" />
...
```
<img width="1068" height="974" alt="image" src="https://github.com/user-attachments/assets/70f6150d-eb3c-411d-ae76-a7353d3d306f" />


เปิด Browser ไปที่ **http://localhost:8080** ควรเห็นหน้า `Welcome to nginx!`
<img width="924" height="533" alt="image" src="https://github.com/user-attachments/assets/ce8a9c6b-521d-4fe8-82a5-051df3b8fb05" />


**อธิบาย แต่ละส่วนของคำสั่ง docker run -d --name my-nginx -p 8080:80 nginx:alpine**
```text
อธิบายคำสั่งที่นี่
- docker run : คำสั่งสำหรับสร้างและรัน Container ใหม่ขึ้นมา
- -d : ย่อมาจาก Detached mode คือการสั่งให้ Container รันทำงานอยู่เบื้องหลัง (Background) ทำให้เราสามารถพิมพ์คำสั่งอื่นต่อใน Terminal ได้โดยไม่ถูกค้าง
- --name my-nginx : เป็นการตั้งชื่อให้กับ Container นี้ว่า "my-nginx" เพื่อให้จดจำและเรียกใช้งานได้ง่าย
- -p 8080:80 : ย่อมาจาก Port Mapping คือการเชื่อมพอร์ต (Map Port) จากเครื่องของเรา (Host) ที่พอร์ต 8080 ให้ทะลุเข้าไปหาพอร์ต 80 ซึ่งเป็นพอร์ตที่ Nginx ทำงานอยู่ภายใน Container 
- nginx:alpine : ระบุชื่อ Image (nginx) และ Tag หรือเวอร์ชัน (alpine) ที่จะนำมาใช้สร้าง Container ซึ่งเวอร์ชัน alpine จะมีขนาดเล็กและทำงานได้เร็ว
```
...
**บันทึกผลการรันคำสั่ง docker exec -it และ ls /usr/share/nginx/html**
   ...
   บันทึกรูปภาพที่นี่
   ```
<img width="812" height="95" alt="image" src="https://github.com/user-attachments/assets/0fcd820d-06aa-4e19-84f8-2ddfb2cb0ac9" />
...
> **📝 บันทึกผล**: Container ID คือ: `ae773eba35c0` และ IP Address คือ: `172.17.0.2`
...
```
<img width="931" height="700" alt="image" src="https://github.com/user-attachments/assets/cbf939e4-9e91-4e50-a8da-0c21d820e880" />
...
1. `docker ps` กับ `docker ps -a` ต่างกันอย่างไร?
   ...
   > _คำตอบ_: docker ps ใช้สำหรับดูเฉพาะ Container ที่ กำลังทำงานอยู่ (สถานะ Up) ในขณะนั้น ส่วน docker ps -a ใช้สำหรับดู Container ทั้งหมด ที่มีอยู่ในระบบ รวมถึงตัวที่หยุดทำงานไปแล้ว (สถานะ Exited) ด้วย

2. ขนาดของ `nginx:alpine` image คือเท่าไร?

   > _คำตอบ_: มีขนาดประมาณ 40-45 MB (สามารถดูตัวเลขที่แน่นอนของเครื่องได้จากการรันคำสั่ง docker images แล้วดูที่คอลัมน์ SIZE ซึ่งปกติจะอยู่ที่ราวๆ 41 - 42.6 MB)
   
3. flag `--rm` ใน `docker run` มีประโยชน์ในกรณีใด?

   > _คำตอบ_: มีประโยชน์สำหรับการรัน Container เพื่อใช้งานแบบ ชั่วคราว เพราะเมื่อเราออกจากระบบ (exit) หรือหยุดการทำงาน ตัว Container จะถูกลบทิ้งให้อัตโนมัติทันที ทำให้ไม่กลายเป็นขยะตกค้างในระบบและไม่เปลืองพื้นที่

4. `docker exec -it` ต่างจาก `docker run -it` อย่างไร?

   > _คำตอบ_: docker run คือการ สร้าง Container ตัวใหม่ ขึ้นมารันการทำงาน ส่วน docker exec คือการ เข้าไปสั่งงานภายใน Container ตัวเดิม ที่กำลังรันทำงานอยู่แล้วโดยไม่ได้สร้างใหม่.
...
7. เมื่อลบ image แล้วให้ทำการ pull image กลับมาเหมือนเดิม พร้อมทำการรัน container
   ```bash
   เขียนคำสั่งที่นี่
  # 1. ดึง Image กลับมาจาก Docker Hub
docker pull nginx:alpine

# 2. รัน Container กลับขึ้นมาใหม่
docker run -d --name my-nginx -p 8080:80 nginx:alpine
   ```
---
...
ตอบคำถามที่นี่
```
> Multi-stage Build คือเทคนิคการเขียน Dockerfile ที่แบ่งการทำงานออกเป็นหลายๆ ขั้นตอน (Stage) โดยมักจะใช้ Stage แรกในการ Build โค้ด (ซึ่งต้องใช้เครื่องมือหนักๆ เช่น compiler) จากนั้นใน Stage สุดท้าย จะทำการคัดลอกเฉพาะ "ไฟล์ที่พร้อมใช้งานจริง" ไปใส่ใน Image ตัวใหม่ ผลลัพธ์คือจะได้ Docker Image สุดท้ายที่มีขนาดเล็กกะทัดรัด ปลอดภัย และไม่มีไฟล์ขยะหรือเครื่องมือ Build ตกค้างอยู่
---
...
```
อธิบายคำสั่งที่นี่
-e DB_PATH=/app/data/booking.db: คือการกำหนด Environment Variable ให้กับ Container เพื่อบอกตัวโปรแกรม Node.js ว่าให้ไปสร้างหรือเรียกใช้ไฟล์ฐานข้อมูล SQLite ที่ตำแหน่ง /app/data/booking.db ภายใน Container
  -v $(pwd)/test-data:/app/data: คือการทำ Bind Mount (Volume) เพื่อเชื่อมโยงโฟลเดอร์ test-data ในเครื่องคอมพิวเตอร์ของเรา (Host) เข้ากับโฟลเดอร์ /app/data ใน Container มีประโยชน์เพื่อให้ข้อมูลในฐานข้อมูลไม่หายไปเมื่อ Container ถูกลบ (Data Persistence) และช่วยให้เราสามารถจัดการไฟล์จากเครื่องหลักได้โดยตรง
```
...
**บันทึกผลการทดลอง**
<img width="1259" height="279" alt="image" src="https://github.com/user-attachments/assets/fb6e1628-b1fa-463b-98fe-fbc0e2b1f21a" />
```
บันทึกรูปผลการรัน curl http://localhost:5000
```
<img width="1919" height="992" alt="image" src="https://github.com/user-attachments/assets/3b54f281-d973-4544-a1f4-41c820ffc82e" />
...
1. `booking-backend:1.0` คืออะไร?
   ...
   > _คำตอบ_: คือชื่อของ Docker Image (Repository Name) ที่ชื่อว่า booking-backend และมีหมายเลขเวอร์ชัน (Tag) คือ 1.0 ซึ่งเป็น Image สำหรับส่วน Backend ที่เราได้ทำการ Build เตรียมเอาไว้ใช้งานค่ะ
   
2. ถ้าต้องการให้ ทดสอบ API ด้วย `curl http://localhost` ต้องทำอย่างไร?

   > _คำตอบ_: ต้องแก้ไขคำสั่งตอนรัน Container (docker run) ในส่วนของการจับคู่พอร์ต (Port Mapping) จากเดิม -p 5000:5000 ให้เปลี่ยนเป็น -p 80:5000 ค่ะ เนื่องจากคำสั่ง curl http://localhost จะเรียกไปที่พอร์ต 80 (พอร์ตมาตรฐานของ HTTP) เป็นค่าเริ่มต้น การเปลี่ยนแบบนี้จะทำให้พอร์ต 80 ของเครื่องเรา (Host) ส่งข้อมูลทะลุไปหาพอร์ต 5000 ใน Container ได้อย่างถูกต้องค่ะ
...
> **📝 บันทึกผล**: booking-frontend:1.0 ขนาด `92.9` MB | booking-backend:1.0 ขนาด `698` MB
...
1. ขนาดของ `booking-frontend:1.0` เทียบกับ `booking-backend:1.0` ต่างกันอย่างไร? เพราะเหตุใด?
   ...
   > _คำตอบ_: booking-frontend:1.0 มีขนาดเล็กกว่ามาก (92.9 MB เทียบกับ 698 MB) สาเหตุเพราะตอนสร้าง Frontend เราใช้เทคนิค Multi-stage Build ซึ่งจะดึงมาเฉพาะไฟล์เว็บ (HTML/CSS/JS) ที่สร้างเสร็จแล้วไปใส่ใน Nginx (Alpine) ที่มีขนาดเล็กกะทัดรัด ส่วนเครื่องมือหนักๆ ที่ใช้ตอน Build (เช่น Node.js) จะถูกทิ้งไปหมดเลย ต่างจาก Backend ที่ต้องเก็บสภาพแวดล้อมทั้งหมดเอาไว้สำหรับรันโปรแกรมตลอดเวลา

2. `location /api/` ใน nginx.conf ทำหน้าที่อะไร?

   > _คำตอบ_: .ทำหน้าที่เป็น Reverse Proxy ค่ะ คือคอยดักจับว่าถ้ามีใครเรียกใช้งานเส้นทาง (URL) ที่ขึ้นต้นด้วย /api/ ตัว Nginx จะไม่ค้นหาไฟล์ในตัวเอง แต่จะทำการส่งต่อคำขอ (Forward Request) นั้นทะลุไปให้โปรแกรมฝั่ง Backend Container เป็นคนจัดการแทน

3. ทำไม Frontend Container ถึงใช้ชื่อ `backend` ในการ Proxy ได้?

   > _คำตอบ_: เพราะทั้ง Frontend และ Backend ถูกจับให้อยู่ในเครือข่ายเดียวกัน (Docker Network) ซึ่ง Docker มีระบบ Internal DNS ของตัวเอง มันจึงสามารถจำชื่อ Container หรือ Service (เช่น คำว่า backend) และแปลงเป็น IP Address เพื่อให้ Container คุยกันเองได้อัตโนมัติ โดยที่เราไม่ต้องไปจำเลข IP เลยค่ะ
...
1. docker-compose.yml คืออะไร มีประโยชน์อย่างไร?
   ...
   > _คำตอบ_: คือไฟล์ที่ใช้กำหนดโครงสร้างและตั้งค่าการทำงานของ Container หลายๆ ตัวให้ทำงานร่วมกันเป็นระบบเดียว ประโยชน์คือช่วยให้เราสามารถสั่งรัน (Start), หยุด (Stop) หรือจัดการระบบทั้งหมดได้ด้วยคำสั่งเดียว แทนที่จะต้องสั่งทีละ Container ช่วยลดความผิดพลาดและจัดการเรื่อง Network/Volume ได้ง่ายขึ้น

2. `restart: unless-stopped` หมายความว่าอย่างไร?

   > _คำตอบ_: หมายความว่าให้ Docker สั่งรัน Container นี้ใหม่โดยอัตโนมัติหากมันหยุดทำงานหรือเครื่องคอมพิวเตอร์ Restart ยกเว้นในกรณีที่เราเป็นคนสั่งหยุด Container นั้นด้วยตัวเอง (Manual Stop)

3. Named Volume `sqlite_data` ต่างจาก Bind Mount อย่างไร? และทำไมต้องใช้กับ SQLite?

   > _คำตอบ_: Named Volume ถูกจัดการโดย Docker โดยตรง (เก็บไว้ในที่เฉพาะของ Docker) ซึ่งมีประสิทธิภาพและความปลอดภัยสูงกว่า Bind Mount (ที่เชื่อมกับโฟลเดอร์ในเครื่องเราโดยตรง) สำหรับ SQLite เราต้องใช้ Volume เพื่อให้ไฟล์ฐานข้อมูล .db ยังคงอยู่ไม่หายไปเมื่อ Container ถูกลบหรืออัปเดตใหม่ และช่วยป้องกันปัญหาเรื่องสิทธิ์การเข้าถึงไฟล์ (File Permission) ที่มักพบใน Bind Mount บนบางระบบปฏิบัติการค่ะ
...
**บันทึกรูปผลการทดลอง**
```
<img width="1134" height="1019" alt="image" src="https://github.com/user-attachments/assets/8f950d85-ba9d-40a7-b473-b6f483dfa2bf" />

บันทึกรูปที่นี่
```
<img width="1246" height="1015" alt="image" src="https://github.com/user-attachments/assets/3539162d-a4a1-4317-8c6c-5a8c45169d4a" />
...
> **📝 บันทึกผลการทดลอง**:
> - IP ของ frontend: `172.18.0.3`
> - IP ของ backend: `172.18.0.2`
...
1. `docker compose ps` แสดงสถานะ Service อย่างไร?
   ...
   > _คำตอบ_: แสดงรายชื่อ Container ทั้งหมดในโปรเจกต์, สถานะการทำงานปัจจุบัน (เช่น Up, Running, Healthy), ระยะเวลาที่สร้าง และการจับคู่พอร์ต (Port Mapping) ระหว่างเครื่อง Host กับ Container

2. IP Address ของแต่ละ Container ในผลจาก `docker network inspect` คืออะไร?

   > _คำตอบ_: (อ้างอิงจากผลการทดลองของคุณ) Frontend คือ 172.18.0.3 และ Backend คือ 172.18.0.2 ซึ่งเป็นเลข IP ภายในที่ใช้สื่อสารกันใน booking-network

3. SQLite database file (`booking.db`) ถูกสร้างใน Path ใดภายใน Container?

   > _คำตอบ_: ถูกสร้างไว้ที่ Path /app/data/booking.db ภายใน Backend Container ซึ่งเชื่อมต่อกับ Volume ภายนอกเพื่อเก็บรักษาข้อมูล
