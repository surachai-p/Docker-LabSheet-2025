...
> **📝 บันทึกผล**: Container ID คือ: `8af271ec89e0` และ IP Address คือ: `172.17.0.2`
Container ID = มันคือรหัสเฉพาะ Unique Identifier ที่ Docker สุ่มขึ้นมาเพื่อใช้ระบุตัวตนของ Container แต่ละตัวที่กำลังรันอยู่ ไม่ให้ซ้ำกัน

IP Address = ที่อยู่ประจำตัว ของอุปกรณ์ทุกชนิดที่เชื่อมต่อกับเครือข่ายอินเทอร์เน็ต
...
1. `docker ps` กับ `docker ps -a` ต่างกันอย่างไร?
   ...
   > _คำตอบ_: docker ps แสดงเฉพาะ Container ที่ กำลังรันอยู่ ในตอนนั้นเท่านั้น
              docker ps -a ย่อมาจาก --all แสดง Container ทั้งหมด ที่เคยสร้างไว้ในเครื่อง ไม่ว่าจะกำลังรันอยู่ หรือหยุดทำงานไปแล้ว
...
3. flag `--rm` ใน `docker run` มีประโยชน์ในกรณีใด?
   ...
   > _คำตอบ_: ใช้สำหรับ สั่งให้ลบ Container ทิ้งทันทีที่มันทำงานเสร็จหรือเราสั่งหยุด

4. `docker exec -it` ต่างจาก `docker run -it` อย่างไร?

   > _คำตอบ_: docker run -it คือการ สร้างตัวใหม่และเริ่มงาน สร้าง Container ใหม่จาก Image แล้วเปิดเข้าไปใช้งานทันที
              docker exec -it คือการ มุดเข้าไปในตัวที่รันอยู่แล้ว ใช้เพื่อสั่งงานหรือเข้าไปดูข้างใน Container ที่เปิดค้างไว้อยู่ก่อนแล้ว โดยไม่สร้างตัวใหม่ขึ้นมาเพิ่ม

5. ต้องการดูว่าในเครื่องมี image อะไรอยู่บ้าง จะเขียนคำสั่งอย่างไร
   ```bash
   เขียนคำสั่งที่นี่ ใช้คำสั่ง docker images
   ```
6. ต้องการลบ image ชื่อ nginx:alpine จะต้องเขียนคำสั่งอย่างไร หากมีหลายขั้นตอนที่ต้องเขียน ให้เขียนลำดับคำสั่งให้ถูกต้อง
   ```bash
   เขียนคำสั่งที่นี่ ใช้คำสั่ง 1. docker ps -a , 2. docker rm -f <container_id_หรือ_name> , 3. docker rmi nginx:alpine
   ```
7. เมื่อลบ image แล้วให้ทำการ pull image กลับมาเหมือนเดิม พร้อมทำการรัน container
   ```bash
   เขียนคำสั่งที่นี่ ใช้คำสั่ง docker run -d -p 8080:80 --name my-nginx nginx:alpine
   ```
---
...
```
ตอบคำถามที่นี่ เทคนิคการเขียน Dockerfile โดยแบ่งการทำงานออกเป็นหลาย ขั้นตอน ในไฟล์เดียว เพื่อแยกสภาพแวดล้อมตอน Build ออกจากตอน Run
```
---
...
```
อธิบายคำสั่งที่นี่ 
1. -e DB_PATH=/app/data/booking.db
มันคือ การตั้งค่า Environment Variable ภายใน Container

2. -v $(pwd)/test-data:/app/data
มันคือ การทำ Bind Mount (Volume) หรือการ ผูกโฟลเดอร์ เข้าหากัน
```
...
 เปิดไม่ขึ้น เพราะทำการลบแล้วตามคำสั่งก่อนหน้านี้ 
```
---
...
1. `booking-backend:1.0` คืออะไร?
   ...
   > _คำตอบ_: คือ Image Name และ เวอร์ชัน ของตัวโปรแกรมที่เรา Build ขึ้นมา

2. ถ้าต้องการให้ ทดสอบ API ด้วย `curl http://localhost` ต้องทำอย่างไร?

   > _คำตอบ_: ต้องทำการ Port Forwarding โดยใช้ flag -p ตอนสั่งรัน Container ครับ
...
1. ขนาดของ `booking-frontend:1.0` เทียบกับ `booking-backend:1.0` ต่างกันอย่างไร? เพราะเหตุใด?
   ...
   > _คำตอบ_: booking-backend จะมีขนาดใหญ่กว่า booking-frontend ประมาณ 662MB เทียบกับ 92MB

2. `location /api/` ใน nginx.conf ทำหน้าที่อะไร?

   > _คำตอบ_: ทำหน้าที่เป็น Reverse Proxy

3. ทำไม Frontend Container ถึงใช้ชื่อ `backend` ในการ Proxy ได้?

   > _คำตอบ_: เพราะมีการใช้ Docker Embedded DNS ภายใน Network เดียวกัน
...
1. docker-compose.yml คืออะไร มีประโยชน์อย่างไร?
   ...
   > _คำตอบ_: คือไฟล์ที่ใช้ กำหนดโครงสร้างและควบคุม Container หลายๆ ตัวพร้อมกัน 

2. `restart: unless-stopped` หมายความว่าอย่างไร?

   > _คำตอบ_: คือนโยบายการสั่งให้ Container เริ่มทำงานใหม่โดยอัตโนมัติ หากเกิดข้อผิดพลาดจนเครื่องดับ หรือแม้แต่ตอนที่พี่ Restart ใหม่

3. Named Volume `sqlite_data` ต่างจาก Bind Mount อย่างไร? และทำไมต้องใช้กับ SQLite?

   > _คำตอบ_: ความต่าง Bind Mount  คือการผูกโฟลเดอร์ในเครื่อง Mac เข้ากับ Container โดยตรง จัดการไฟล์ได้ง่ายจาก FinderNamed Volume คือการให้ Docker เป็นคนจัดการพื้นที่เก็บข้อมูลให้เองในจุดที่ปลอดภัย ซึ่งจะไม่มีปัญหาเรื่อง Permission ระหว่าง macOS กับ Linux ภายใน Container ทำไมต้องใช้กับ SQLite เพราะ SQLite เป็นไฟล์ Database ที่ไวต่อการเขียนข้อมูลพร้อมกัน (Locking) การใช้ Named Volume จะมีประสิทธิภาพ สูงกว่าและเสถียรกว่าบน Docker Desktop ลดโอกาสที่ไฟล์ booking.db จะเสียหาย เมื่อเทียบกับการใช้ Bind Mount ครับ
...
1. `docker compose ps` แสดงสถานะ Service อย่างไร?
   ...
   > _คำตอบ_: แสดงรายการ Container ทั้งหมดที่ถูกจัดการภายใต้ไฟล์ docker-compose.yml นั้นๆ

2. IP Address ของแต่ละ Container ในผลจาก `docker network inspect` คืออะไร?

   > _คำตอบ_: IP Address ที่ Docker Assign ให้กับแต่ละ Container ภายใน Virtual Network เช่น 172.18.0.2, 172.18.0.3 เพื่อให้ Container สามารถสื่อสารกันเองได้โดยตรงผ่านระบบเครือข่ายจำลองของ Docker 

3. SQLite database file (`booking.db`) ถูกสร้างใน Path ใดภายใน Container?

   > _คำตอบ_: ถูกสร้างไว้ที่ Path /app/data/booking.db
