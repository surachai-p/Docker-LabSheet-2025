...
👉 คำสั่งสำหรับ “สร้าง + รัน container ใหม่”

ถ้ายังไม่มี image → มันจะ pull มาให้อัตโนมัติ
แล้วก็ start container ทันที
🔹 -d (detached mode)

👉 รันแบบ เบื้องหลัง

terminal จะไม่ค้าง
container ทำงานอยู่ แต่เราใช้ command ต่อได้

📌 ถ้าไม่ใส่ -d → จะเห็น log วิ่งเต็มจอ

🔹 --name my-nginx

👉 ตั้งชื่อ container

ปกติ Docker จะสุ่มชื่อให้ (งง ๆ เช่น happy_penguin)
ใส่ชื่อเองจะเรียกใช้ง่าย เช่น:
...
<พอร์ตเครื่องเรา>:<พอร์ตใน container>

กรณีนี้คือ:

8080 = พอร์ตบนเครื่องเรา (localhost)
80 = พอร์ตใน container (nginx ใช้)

📌 ความหมาย:
👉 เวลาเข้า http://localhost:8080
➡️ จะถูกส่งไปที่ nginx ใน container (port 80)
...
nginx = ชื่อ image (เว็บเซิร์ฟเวอร์)
alpine = version ที่เล็ก (เบา เร็ว)

📌 ถ้าไม่มีในเครื่อง:
→ Docker จะไปโหลดจาก Docker Hub
...
> **📝 บันทึกผล**: Container ID คือ: 2d36e01994e6
IP Address คือ: 172.17.0.2
...
1. `docker ps` กับ `docker ps -a` ต่างกันอย่างไร?
   ...
   > _คำตอบ_: docker ps แสดงเฉพาะ container ที่กำลังทำงานอยู่ (สถานะ Up)
docker ps -a แสดง container ทั้งหมด ไม่ว่าจะกำลังทำงาน (Up) หรือหยุดแล้ว (Exited, Created)

2. ขนาดของ `nginx:alpine` image คือเท่าไร?

   > _คำตอบ_: ประมาณ 5–7 MB (เล็กมาก เพราะใช้ Alpine Linux)

3. flag `--rm` ใน `docker run` มีประโยชน์ในกรณีใด?

   > _คำตอบ_: ใช้สำหรับลบ container อัตโนมัติเมื่อหยุดการทำงาน
เหมาะกับการรันแบบชั่วคราว เช่น ทดสอบคำสั่ง หรือ run script สั้น ๆ เพื่อไม่ให้มี container ค้างในระบบ

4. `docker exec -it` ต่างจาก `docker run -it` อย่างไร?

   > _คำตอบ_: docker run -it ใช้สร้างและรัน container ใหม่ พร้อมเข้า interactive shell

docker exec -it ใช้เข้าไปใน container ที่กำลังทำงานอยู่แล้ว เพื่อรันคำสั่งหรือเปิด shell เพิ่ม
...
```
คือการแบ่งการ build Docker image ออกเป็นหลายขั้น (stage)
โดยใช้ stage แรกสำหรับ build และติดตั้งเครื่องมือ
แล้วคัดลอกเฉพาะไฟล์ที่จำเป็นไปยัง stage สุดท้าย
ทำให้ image มีขนาดเล็ก สะอาด และไม่มี build tools ที่ไม่จำเป็น
```
---
...
> **📝 บันทึกผล**![alt text](image-5.png): ขนาดของ booking-backend:1.0 คือ: `57.2` MB 
...
-e ย่อมาจาก environment variable
ใช้สำหรับกำหนด ตัวแปรแวดล้อม (environment variable) ให้กับ Container
ในที่นี้คือกำหนดตัวแปรชื่อ DB_PATH ให้ Container รู้ว่า ไฟล์ฐานข้อมูล SQLite จะอยู่ที่ /app/data/booking.db
ตัว Container หรือแอปใน Container จะอ่านค่า DB_PATH นี้เพื่อเชื่อมต่อกับฐานข้อมูล
2️⃣ -v $(pwd)/test-data:/app/data
-v ย่อมาจาก volume mount
ใช้สำหรับ เชื่อมต่อโฟลเดอร์/ไฟล์จากเครื่อง Host เข้ากับ Container

Syntax:

-v <host_path>:<container_path>
ในตัวอย่างนี้:
$(pwd)/test-data → โฟลเดอร์ test-data ในเครื่องคุณ (Host)
/app/data → โฟลเดอร์ใน Container
ผลลัพธ์:
ทุกไฟล์ใน test-data บนเครื่อง Host จะสามารถเข้าถึงได้จาก /app/data ใน Container
ถ้า Container สร้างไฟล์ใหม่ เช่นฐานข้อมูล booking.db ก็จะไปอยู่ใน test-data ของ Host ด้วย
ทำให้ข้อมูลไม่หายแม้ Container ถูกลบ
```
...
**บันทึกผลการทดลอง**![alt text](image-6.png) ![alt text](image-7.png)
```
...
1. `booking-backend:1.0` คืออะไร?
   ...
   > _คำตอบ_: booking-backend:1.0 คือ ชื่อและเวอร์ชันของ Docker image

booking-backend → ชื่อ Image (บอกว่าเป็น Backend ของระบบ Booking)
1.0 → Tag หรือเวอร์ชันของ Image

ใช้เวลารัน Container เช่น

docker run booking-backend:1.0

→ จะสร้าง Container จาก Image ตัวนี้

สรุป: เป็นตัวแทนของ แอป Backend พร้อมรันได้ใน Container

2. ถ้าต้องการให้ ทดสอบ API ด้วย `curl http://localhost` ต้องทำอย่างไร?

   > _คำตอบ_: ต้องทำให้ Container เปิด Port ของ Backend และเชื่อมกับเครื่อง Host

รัน Container พร้อมระบุ Port Mapping:
docker run -p 5000:5000 booking-backend:1.0
-p <host_port>:<container_port>
ในตัวอย่างนี้: Port 5000 ของ Host → Port 5000 ของ Container
ตรวจสอบว่า Container รันอยู่:
docker ps
ทดสอบ API ด้วย curl เช่น:
curl http://localhost:5000/api/bookings
จะส่งคำขอ HTTP ไปยัง Backend Container ผ่าน Port 5000
...
> **📝 บันทึกผล**: บันทึกผล: booking-frontend:1.0 ขนาด 24.85 MB | booking-backend:1.0 ขนาด 174.16 MB
...
1. ขนาดของ `booking-frontend:1.0` เทียบกับ `booking-backend:1.0` ต่างกันอย่างไร? เพราะเหตุใด?
   ...
   > _คำตอบ_: booking-frontend:1.0 ขนาดประมาณ 25 MB
booking-backend:1.0 ขนาดประมาณ 174 MB
ความแตกต่าง: Backend ใหญ่กว่า Frontend ประมาณ 7 เท่า

สาเหตุ:

Backend มักมี runtime, libraries, framework, Node.js หรือภาษาอื่นๆ, database client และ dependency จำนวนมาก
Frontend เป็นแค่ static files (HTML, CSS, JS, images) และ Nginx สำหรับ serve ไฟล์ จึงมีขนาดเล็ก

2. `location /api/` ใน nginx.conf ทำหน้าที่อะไร?

   > _คำตอบ_: เป็น directive ของ Nginx สำหรับกำหนด URL path ที่เริ่มด้วย /api/
โดยปกติจะใช้ Proxy Request ไปยัง Backend Container
ตัวอย่างใน Nginx:
location /api/ {
    proxy_pass http://backend:5000/;
}
หมายความว่า ทุก request ที่ URL เริ่มด้วย /api/ จะถูกส่งไปที่ Backend (Port 5000) แทนที่จะให้ Nginx serve static files

💡 สรุป: ช่วยให้ Frontend (Static) และ Backend (API) ทำงานร่วมกันใน Container เดียวกันหรือหลาย Container ได้

3. ทำไม Frontend Container ถึงใช้ชื่อ `backend` ในการ Proxy ได้?

   > _คำตอบ_: เพราะ Docker Compose หรือ Docker Network ให้ Container สามารถสื่อสารกันด้วยชื่อ Container หรือ service name
...
services:
  ...
  backend:
    ...
    ...
จาก Frontend Container สามารถใช้ URL เช่น http://backend:5000 เพื่อเข้าถึง Backend ได้โดยตรง
Docker จะจัดการ DNS ภายใน Network ของ Container ให้ชื่อ backend ชี้ไปยัง IP ของ Backend Container

💡 สรุป: ไม่จำเป็นต้องใช้ localhost หรือ IP จริง, ใช้ชื่อ service/container ก็เพียงพอ
...
1. docker-compose.yml คืออะไร มีประโยชน์อย่างไร?
   ...
   > _คำตอบ_: docker-compose.yml เป็น ไฟล์ configuration ของ Docker Compose
ใช้สำหรับ กำหนดหลาย Container (services) ให้รันพร้อมกัน
ระบุได้ว่าแต่ละ service ใช้ image, build context, port, volume, environment variable อะไรบ้าง
ประโยชน์:
รันหลาย Container พร้อมกันด้วยคำสั่งเดียว: docker compose up
จัดการ network ระหว่าง Container อัตโนมัติ
เก็บค่า configuration ของโปรเจกต์เป็นไฟล์เดียว ทำให้แชร์กับทีมได้ง่าย

2. `restart: unless-stopped` หมายความว่าอย่างไร?

   > _คำตอบ_: เป็น policy การ restart ของ Container
ความหมาย:
Container จะ รีสตาร์ทอัตโนมัติ ถ้าเกิด error หรือ Docker Host รีบูต
ยกเว้น ผู้ใช้สั่ง docker stop ด้วยมือ Container จะไม่เริ่มอัตโนมัติ
ตัวอย่างอื่น ๆ:
no → ไม่ restart อัตโนมัติ
always → restart ทุกครั้งไม่สนใจ stop ด้วยมือ

3. Named Volume `sqlite_data` ต่างจาก Bind Mount อย่างไร? และทำไมต้องใช้กับ SQLite?

   > _คำตอบ_: ประเภท	อธิบาย
Named Volume	Docker สร้าง storage ภายใน Docker managed location เช่น /var/lib/docker/volumes/...
Container สามารถใช้หลายครั้ง, portable, ไม่ต้องสนใจ path ของ Host
Bind Mount	ใช้โฟลเดอร์หรือไฟล์ของ Host จริง ๆ เช่น C:/Users/.../data
สะดวกในการแก้ไขไฟล์จาก Host, แต่ path ขึ้นกับ OS และอาจมี permission issue

ทำไม SQLite ต้องใช้ Named Volume:

SQLite คือ ไฟล์ database ที่อยู่ใน filesystem
ถ้าไม่ mount volume, ข้อมูลจะ หายไปเมื่อ Container ถูกลบ
Named Volume ทำให้ SQLite database คงอยู่แม้ Container ถูกลบ และง่ายต่อการ backup/restore
...
**บันทึกรูปผลการทดลอง**![alt text](image-8.png)
```
บันทึกรูปที่นี่
```
...
> **📝 บันทึกผลการทดลอง**:
>booking-frontend: 172.19.0.3/16
>booking-backend: 172.19.0.2/16
...
1. `docker compose ps` แสดงสถานะ Service อย่างไร?
   ...
   > _คำตอบ_: แสดง รายการ Container ทั้งหมด ที่ถูกสร้างจาก Docker Compose project ปัจจุบัน
ข้อมูลที่เห็นจะมี:
Name → ชื่อ Container
Service → ชื่อ Service ตาม docker-compose.yml
Command → คำสั่งที่รันใน Container
State / Status → สถานะปัจจุบัน เช่น Up, Exited, Restarting
Ports → Port mapping ระหว่าง Host และ Container

2. IP Address ของแต่ละ Container ในผลจาก `docker network inspect` คืออะไร?

   > _คำตอบ_: เมื่อรัน docker network inspect <network_name> จะเห็นรายละเอียด network ของ Compose project
แต่ละ Container จะมี IP Address ภายใน Docker Network
ใช้สื่อสารระหว่าง Container เช่น Frontend → Backend ผ่าน http://<IP>:5000
ตัวอย่างผลลัพธ์:
"Containers": {
    "booking-backend-1": {
...
3. SQLite database file (`booking.db`) ถูกสร้างใน Path ใดภายใน Container?
   ...
   > _คำตอบ_: ตาม .env และ volume mount:
DB_PATH=/app/data/booking.db
Path ภายใน Container คือ:
/app/data/booking.db
Volume หรือ Named Volume (เช่น sqlite_data) ทำให้ไฟล์นี้ คงอยู่แม้ Container ถูกลบ
