...
* **`docker run`**
    คือคำสั่งหลักสำหรับ "สร้างและเปิดใช้งาน" (Create and Start) Container ใหม่จากอิมเมจ (Image) ที่เราระบุ
* **`-d`** (ย่อมาจาก `--detach`)
    เป็นการสั่งให้ Container รันอยู่เบื้องหลัง (Background) ทำให้เมื่อกด Enter รันคำสั่งไปแล้ว Terminal ของเราจะถูกคืนค่ากลับมาให้พิมพ์คำสั่งอื่นต่อไปได้ทันที โดยไม่ต้องรอให้ Container หยุดทำงาน
* **`--name my-nginx`**
    เป็นการตั้งชื่อให้กับ Container ตัวนี้ว่า `my-nginx` ครับ การตั้งชื่อจะช่วยให้เราจัดการ Container ได้ง่ายขึ้นมาก (เช่น เวลาจะสั่งหยุดก็พิมพ์แค่ `docker stop my-nginx`) หากเราไม่ใส่พารามิเตอร์นี้ Docker จะสุ่มชื่อแปลกๆ มาให้เราเอง
* **`-p 8080:80`** (ย่อมาจาก `--publish`)
     
    ส่วนนี้คือการทำ **Port Mapping** หรือการเชื่อมโยงพอร์ตระหว่างเครื่องคอมพิวเตอร์ของเรา (Host) กับตัว Container ครับ รูปแบบคือ `Host_Port:Container_Port`
    * **`8080` (ตัวหน้า):** คือ Port บนเครื่องคอมพิวเตอร์ของเรา
    * **`80` (ตัวหลัง):** คือ Port ภายใน Container ที่โปรแกรม Nginx เปิดให้บริการอยู่ตามปกติ
    * *ความหมายรวม:* เมื่อคุณเปิด Browser แล้วเข้าเว็บไปที่ `http://localhost:8080` คำขอของคุณจะถูกส่งทะลุเข้าไปยัง Port `80` ของ Nginx ที่อยู่ใน Container ทันที
* **`nginx:alpine`**
    คือชื่อของ Docker Image ที่เราต้องการนำมารันครับ โดยแบ่งเป็น 2 ส่วนคั่นด้วยเครื่องหมาย colon (`:`)
    * **`nginx`**: คือชื่ออิมเมจหลัก (โปรแกรม Web Server)
    * **`alpine`**: คือ Tag (เวอร์ชันหรือรูปแบบย่อย) ซึ่ง `alpine` หมายถึงการนำระบบปฏิบัติการ Alpine Linux มาเป็นฐานในการสร้างอิมเมจนี้ ข้อดีคือมันมีขนาดเล็กและเบามากๆ ทำให้ดาวน์โหลดและรันได้ไว
...
> **📝 บันทึกผล**: Container ID คือ: `8256e03de1cb` และ IP Address คือ: `172.17.0.2`
...
1. `docker ps` กับ `docker ps -a` ต่างกันอย่างไร?
   ...
   > _คำตอบ_: ต่างกันตรงที่สถานะของ Container ที่แสดงให้เห็นครับ:

* **`docker ps`** : แสดงเฉพาะ Container ที่ **กำลังทำงานอยู่ (Up/Running)** ในขณะนั้น
* **`docker ps -a`** : แสดง Container **ทั้งหมด** ทั้งตัวที่กำลังทำงานอยู่ และตัวที่ **หยุดทำงานไปแล้ว (Exited/Stopped)** ครับ

2. ขนาดของ `nginx:alpine` image คือเท่าไร?

   > _คำตอบ_: ขนาดของอิมเมจ `nginx:alpine` จะอยู่ที่ประมาณ **40-45 MB** (ขนาดพื้นที่จริงเมื่อดึงมาอยู่บนเครื่องแล้ว) แต่ในขั้นตอนการดาวน์โหลดหรือ Pull (Compressed size) จะใช้ปริมาณข้อมูลเพียงแค่ประมาณ **15-20 MB** เท่านั้นครับ

สาเหตุที่มันมีขนาดเล็กมากเมื่อเทียบกับ `nginx` แท็กปกติ (ซึ่งมักจะมีขนาด 140-190 MB ขึ้นไป) เป็นเพราะมันถูกสร้างขึ้นบนฐานของ **Alpine Linux** ซึ่งเป็นระบบปฏิบัติการที่ถูกปรับแต่งมาให้เล็กกะทัดรัดและใช้ทรัพยากรน้อยที่สุด เพื่อให้เหมาะกับการใช้งานใน Container โดยเฉพาะ

3. flag `--rm` ใน `docker run` มีประโยชน์ในกรณีใด?

   > _คำตอบ_: `--rm` ใช้สั่งให้ Docker **ลบ Container ทิ้งอัตโนมัติทันทีที่มันหยุดทำงาน** ครับ

**ประโยชน์:** เหมาะสำหรับรันงานชั่วคราว หรือใช้งานคำสั่งแค่ครั้งเดียวจบ เพื่อป้องกันไม่ให้มี Container ขยะตกค้างและกินพื้นที่ในเครื่องครับ

4. `docker exec -it` ต่างจาก `docker run -it` อย่างไร?

   > _คำตอบ_: ต่างกันที่ **เป้าหมายการทำงาน** ครับ:

* **`docker run -it`** : ใช้สำหรับ **"สร้าง Container ใหม่"** ขึ้นมาจากอิมเมจ แล้วเข้าไปใช้งานหน้าจอ Terminal ของ Container นั้นทันที 
* **`docker exec -it`** : ใช้สำหรับ **"เข้าไปใน Container ที่กำลังทำงานอยู่แล้ว"** เพื่อรันคำสั่ง ตรวจสอบไฟล์ หรือแก้คอนฟิกต่างๆ โดยไม่ได้สร้าง Container ขึ้นมาใหม่

**จำง่ายๆ:**
* `run` = สร้างบ้านหลังใหม่ แล้วเดินเข้าไป
* `exec` = เปิดประตูเข้าไปในบ้านที่สร้างเสร็จและมีคนอยู่แล้ว
...
* **หยุดการทำงาน:** `docker stop <ชื่อคอนเทนเนอร์>`
* **ลบตัวคอนเทนเนอร์:** `docker rm <ชื่อคอนเทนเนอร์>`
* **คำสั่ง:** `docker rmi nginx:alpine`
