...
```text
อธิบายคำสั่งที่นี่
คำสั่ง docker run -d --name my-nginx -p 8080:80 nginx:alpine ใช้สำหรับสร้างและรัน container nginx โดย docker run คือสั่งให้สร้างและรัน container, -d คือให้ทำงานเบื้องหลัง, --name my-nginx คือการตั้งชื่อ container, -p 8080:80 คือเชื่อม port 8080 ของเครื่องเราไปที่ port 80 ใน container และ nginx:alpine คือ image ที่ใช้ ซึ่งเป็นเวอร์ชันเล็กโหลดไว
```
...
> **📝 บันทึกผล**: Container ID คือ:`676a29b84e58` และ IP Address คือ: `172.17.0.2`
...
1. `docker ps` กับ `docker ps -a` ต่างกันอย่างไร?
   ...
   > _คำตอบ_: docker ps กับ docker ps -a ต่างกันคือ docker ps จะดูเฉพาะ container ที่กำลังรันอยู่ ส่วน docker ps -a จะดูได้ทั้งหมดทั้งที่รันอยู่และที่หยุดไปแล้ว

2. ขนาดของ `nginx:alpine` image คือเท่าไร?

   > _คำตอบ_: ขนาดของ nginx:alpine จะประมาณ 20-30 MB เป็น image ขนาดเล็ก โหลดเร็ว

3. flag `--rm` ใน `docker run` มีประโยชน์ในกรณีใด?

   > _คำตอบ_: --rm ใช้สำหรับลบ container อัตโนมัติหลังจากหยุดการทำงาน เหมาะกับเอาไว้ทดลองหรือใช้งานครั้งเดียว

4. `docker exec -it` ต่างจาก `docker run -it` อย่างไร?

   > _คำตอบ_: docker exec -it คือใช้เข้าไปใน container ที่มันรันอยู่แล้ว ส่วน docker run -it คือใช้สร้าง container ใหม่แล้วเข้าไปใช้งานเลย
...
6. ต้องการลบ image ชื่อ nginx:alpine จะต้องเขียนคำสั่งอย่างไร หากมีหลายขั้นตอนที่ต้องเขียน ให้เขียนลำดับคำสั่งให้ถูกต้อง
   ```bash
   เขียนคำสั่งที่นี่
   ```docker ps -a          # ดู container ก่อน
docker stop my-nginx  # หยุด container (ถ้ามี)
...
```
ตอบคำถามที่นี่
```Multi-stage Build คือการแบ่งการสร้าง Docker เป็นหลายขั้นตอน 
แล้วเอาแค่ไฟล์ที่จำเป็นมาใช้ ทำให้ image เล็กลง
---
...
```
อธิบายคำสั่งที่นี่
```บอก Container ว่าไฟล์ฐานข้อมูลอยู่ตรงไหน แล้วก็เชื่อมโฟลเดอร์ในเครื่องกับ Container เพื่อเก็บไฟล์ไม่ให้หาย
...
1. `booking-backend:1.0` คืออะไร?
   ...
   > _คำตอบ_: booking-backend:1.0 คือ Docker image ของ backend ที่สร้างขึ้นในแลปนี้ 
โดย booking-backend เป็นชื่อ และ 1.0 เป็นเวอร์ชัน

2. ถ้าต้องการให้ ทดสอบ API ด้วย `curl http://localhost` ต้องทำอย่างไร?

   > _คำตอบ_: ถ้าต้องการทดสอบ API ด้วย curl http://localhost 
ต้องตั้งค่า port mapping ให้ container ใช้ port 80 
เช่น -p 80:5000 หรือ -p 80:3000 แล้วจึงใช้คำสั่ง curl http://localhost ได้
...
> **📝 บันทึกผล**: 
booking-frontend:1.0              09a75ed82cf2       92.9MB         26.1MB
booking-backend:1.0               b848c96f5352        698MB          183MB
...
1. ขนาดของ `booking-frontend:1.0` เทียบกับ `booking-backend:1.0` ต่างกันอย่างไร? เพราะเหตุใด?
   ...
   > _คำตอบ_: frontend เล็กกว่า เพราะเป็นแค่ไฟล์เว็บ backend ใหญ่กว่า เพราะมี node_modules

2. `location /api/` ใน nginx.conf ทำหน้าที่อะไร?

   > _คำตอบ_: ใช้ส่ง request ไป backend

3. ทำไม Frontend Container ถึงใช้ชื่อ `backend` ในการ Proxy ได้?

   > _คำตอบ_: เพราะอยู่ network เดียวกัน เลยเรียกชื่อ container ได้
...
1. docker-compose.yml คืออะไร มีประโยชน์อย่างไร?
   ...
   > _คำตอบ_: คือไฟล์ที่ใช้จัดการหลาย container พร้อมกัน ทำให้สั่งรันทีเดียวได้ทั้งระบบ

2. `restart: unless-stopped` หมายความว่าอย่างไร?

   > _คำตอบ_: ถ้า container หยุดเอง จะรันใหม่อัตโนมัติ ยกเว้นเราสั่ง stop เอง
3. Named Volume `sqlite_data` ต่างจาก Bind Mount อย่างไร? และทำไมต้องใช้กับ SQLite?

   > _คำตอบ_: Named Volume จะให้ Docker จัดการให้ ส่วน Bind Mount คือใช้โฟลเดอร์ในเครื่องเรา
ใช้กับ SQLite เพื่อให้ไฟล์ database ไม่หายเมื่อ container ถูกลบ
---
...
1. `docker compose ps` แสดงสถานะ Service อย่างไร?
   ...
   > _คำตอบ_: ใช้ดูว่า Service แต่ละอันกำลังรันอยู่หรือหยุด เช่น ขึ้นว่า Up คือรันอยู่ ขึ้นว่า Exited คือหยุดแล้ว

2. IP Address ของแต่ละ Container ในผลจาก `docker network inspect` คืออะไร?

   > _คำตอบ_: เป็นเลข IP ที่ Container แต่ละอันได้ในเครือข่าย Docker ใช้ให้ Container คุยกันเอง
3. SQLite database file (`booking.db`) ถูกสร้างใน Path ใดภายใน Container?

   > _คำตอบ_: ไฟล์จะอยู่ในโฟลเดอร์ของแอปใน Container เช่น /app/booking.db หรือในโฟลเดอร์ที่เราเชื่อมกับเครื่องเรา
