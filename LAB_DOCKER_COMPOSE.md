...
> **📝 บันทึกผล**: 
[บันทึกภาพหน้าจอของผลลัพธ์การทดลอง](img/image.png)
...
```text
อธิบายคำสั่งที่นี่
docker run คำสั่งหลักสำหรับ สร้างและเริ่ม container ใหม่ จาก image
-d รัน container แบบ เบื้องหลัง (background)
  ถ้าไม่ใส่ → จะเห็น log ติดอยู่ใน terminal
  ใส่แล้ว → terminal ใช้งานต่อได้เลย
--name my-nginx ตั้งชื่อ container ว่า my-nginx
  -p 8080:80 (port mapping)
เชื่อม port ระหว่าง เครื่องเรา ↔ container
8080 = port บนเครื่องเรา (host)
80 = port ภายใน container (Nginx ใช้ port 80)
  nginx:alpine คือ image ที่ใช้
nginx = official Nginx image
alpine = version ที่ใช้ Alpine Linux (เบา เร็ว ขนาดเล็ก)
```
...
> **📝 บันทึกผล**: Container ID คือ: `a41521d41070` และ IP Address คือ: `172.17.0.2`
...
```
[docker container](img/image3.png)
---
...
1. `docker ps` กับ `docker ps -a` ต่างกันอย่างไร?
   ...
   > _คำตอบ_: docker ps แสดงเฉพาะ container ที่กำลังรันอยู่ docker ps -a แสดง container ทั้งหมด 

2. ขนาดของ `nginx:alpine` image คือเท่าไร?

   > _คำตอบ_: ประมาณ 20–30 MB (ขึ้นอยู่กับ version) เล็กกว่า nginx ปกติที่อาจ >100 MB

3. flag `--rm` ใน `docker run` มีประโยชน์ในกรณีใด?

   > _คำตอบ_: ใช้เมื่อต้องการให้ container ถูกลบทิ้งอัตโนมัติหลังหยุดทำงาน

4. `docker exec -it` ต่างจาก `docker run -it` อย่างไร?

   > _คำตอบ_: docker run -it  สร้าง container ใหม่ + เข้าไปใช้งานทันที docker exec -it เข้าไปใช้งานใน container ที่กำลังรันอยู่แล้ว
...
6. ต้องการลบ image ชื่อ nginx:alpine จะต้องเขียนคำสั่งอย่างไร หากมีหลายขั้นตอนที่ต้องเขียน ให้เขียนลำดับคำสั่งให้ถูกต้อง
   ```bash
   เขียนคำสั่งที่นี่
   1. หยุด container (ถ้ามีใช้งานอยู่)
  docker stop my-nginx

  2. ลบ container ก่อน (สำคัญ)
  docker rm my-nginx
...
```
ตอบคำถามที่นี่
Multi-stage Build คือเทคนิคใน Docker ที่ใช้ หลาย stage (หลายขั้นตอน) ภายใน Dockerfile เดียว เพื่อให้ได้ image ที่ เล็กลง สะอาดขึ้น และปลอดภัยมากขึ้น
```
---
...
```
อธิบายคำสั่งที่นี่
-e DB_PATH=...: คือการส่ง "ที่อยู่ไฟล์" เข้าไปบอก App ใน Container ว่าให้ไปอ่าน/เขียน Database ที่ไหน

-v $(pwd)/...: คือการ "แชร์โฟลเดอร์" จากเครื่องคุณเข้าไปใน Container เพื่อให้ไฟล์ Database ไม่หายไปแม้จะสั่งลบ Container ทิ้ง (Data Persistence)
```
...
1. `booking-backend:1.0` คืออะไร?
   ...
   > _คำตอบ_: คือ Image Name และ Tag (ชื่อและเวอร์ชันของโปรเจกต์)  ที่เราสร้างขึ้นเพื่อใช้เป็นต้นแบบในการสั่งรันContainer

2. ถ้าต้องการให้ ทดสอบ API ด้วย `curl http://localhost` ต้องทำอย่างไร?

   > _คำตอบ_: ต้องเพิ่มพารามิเตอร์ -p 80:80 (หรือพอร์ตที่ App ใช้ เช่น -p 80:3000) ในคำสั่ง docker run เพื่อเชื่อมต่อพอร์ตของเครื่องเราเข้ากับพอร์ตภายใน Container
...
1. ขนาดของ `booking-frontend:1.0` เทียบกับ `booking-backend:1.0` ต่างกันอย่างไร? เพราะเหตุใด?
   ...
   > _คำตอบ_: Frontend เล็กกว่า เพราะเหลือแค่ไฟล์ Static (HTML/JS) รันบน Nginx ส่วน Backend ใหญ่กว่า เพราะต้องแบกทั้ง Runtime (เช่น Node.js) และ Library (node_modules) ทั้งหมดไว้

2. `location /api/` ใน nginx.conf ทำหน้าที่อะไร?

   > _คำตอบ_: เป็น Reverse Proxy คอยดัก Request ที่จะคุยกับฐานข้อมูล แล้วส่งต่อไปให้เครื่อง Backend ทำงานแทน เพื่อเลี่ยงปัญหา CORS และให้ใช้ Domain เดียวกันได้

3. ทำไม Frontend Container ถึงใช้ชื่อ `backend` ในการ Proxy ได้?

   > _คำตอบ_: เพราะ Docker Internal DNS ครับ เมื่อคอนเทนเนอร์อยู่ใน Network เดียวกัน มันจะมองเห็นและเรียกหากันด้วย "ชื่อ" ได้ทันทีโดยไม่ต้องใช้เลข IP
...
1. docker-compose.yml คืออะไร มีประโยชน์อย่างไร?
   ...
   > _คำตอบ_: คือไฟล์ที่ใช้กำหนดค่าเพื่อรันหลายคอนเทนเนอร์พร้อมกัน ประโยชน์คือช่วยให้จัดการทั้ง Service, Network และ Volume ทั้งหมดได้ด้วยคำสั่งเดียว แทนที่จะต้องมานั่งพิมพ์ docker run ทีละตัว

2. `restart: unless-stopped` หมายความว่าอย่างไร?

   > _คำตอบ_: คือการตั้งค่าให้คอนเทนเนอร์เริ่มทำงานใหม่เองโดยอัตโนมัติถ้ามันเกิด Error (Crash) หรือเครื่อง Server รีสตาร์ท ยกเว้นกรณีเดียวคือเราเป็นคนสั่งหยุด (Stop) คอนเทนเนอร์นั้นด้วยตัวเอง

3. Named Volume `sqlite_data` ต่างจาก Bind Mount อย่างไร? และทำไมต้องใช้กับ SQLite?

   > _คำตอบ_: Named Volume จะให้ Docker เป็นคนจัดการพื้นที่เก็บข้อมูลให้ (ปลอดภัยและประสิทธิภาพดีกว่า) ส่วน Bind Mount เราต้องระบุโฟลเดอร์ในเครื่องเอง ที่ใช้กับ SQLite เพราะช่วยป้องกันปัญหาเรื่อง Permission ของไฟล์ และลดโอกาสที่ไฟล์ Database จะพังเวลาแชร์ข้ามระบบปฏิบัติการ
...
1. `docker compose ps` แสดงสถานะ Service อย่างไร?
   ...
   > _คำตอบ_: คำสั่ง docker compose ps ใช้แสดง สถานะของ service (container) ที่อยู่ใน compose นั้น เช่น
  ชื่อ service / container
  สถานะ (Up, Exited, Restarting)
  port ที่เปิดใช้งาน
  command ที่รัน

2. IP Address ของแต่ละ Container ในผลจาก `docker network inspect` คืออะไร?

   > _คำตอบ_: คือ IP ภายใน network ของ Docker (internal network) ที่ container ใช้สื่อสารกันเอง

3. SQLite database file (`booking.db`) ถูกสร้างใน Path ใดภายใน Container?

   > _คำตอบ_: ขึ้นอยู่กับที่กำหนดในแอป แต่โดยทั่วไปจะอยู่ใน path เช่น:
  /app/booking.db
  หรือ
  /usr/src/app/booking.db
