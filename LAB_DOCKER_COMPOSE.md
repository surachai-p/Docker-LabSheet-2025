...
```text
อธิบายคำสั่งที่นี่
dockr run -d คือสั่งให้ docker รันทำงานโดยที่ -d จะเป็นการให้ docker ทำงานเบื่องหลังโดยไม่สนว่าเราจะปิดหรือเปิด terminal --name เป็นการตั้งชื่อในที่นี้ชื่อว่า my-nginx -p คือกำหนด port 8080 เป็น public port ภายนอกเวลา clinet เข้ามา 80 เป็น private port ภายใน container ซึ่ง docker จะทำการ forwarding port เข้าหากัน  nginx:alpine คือโหลด nginx เวอร์ชั่นที่พัฒนาให้มีขนาดเล็กแล้วมาจาก Registy ของ docker 
...
> **📝 บันทึกผล**: Container ID คือ: `fe9cd3ca1294` และ IP Address คือ: `172.17.0.2`
...
1. `docker ps` กับ `docker ps -a` ต่างกันอย่างไร?
   ...
   > _คำตอบ_: ps ทำการดู container ทีทำงานอยู่แต่ -a ทำการดู container ทั้งหมดที่อยูาในเครื่องรวมตัวที่หยุดทำงานด้วย 

2. ขนาดของ `nginx:alpine` image คือเท่าไร?

   > _คำตอบ_: nginx เวอร์ชั่นที่ทำการย่อให้เล็กทำให้มาเพื่อการทำงานใน container 

3. flag `--rm` ใน `docker run` มีประโยชน์ในกรณีใด?

   > _คำตอบ_: ลบ container อัตโนมัติทันทีที่เมื่อ container หยุดทำงาน 

4. `docker exec -it` ต่างจาก `docker run -it` อย่างไร?

   > _คำตอบ_: docker exec -it เป็นการเข้าไปที่ kernel ของ container นัน้ ๆ โดยจะใช้คำสังเป็น linux และ docker run -it สร้าง container ใหม่ แล้วเข้าไป ส่วน docker exec เข้าอันที่มีอยู่แล้ว
...
7. เมื่อลบ image แล้วให้ทำการ pull image กลับมาเหมือนเดิม พร้อมทำการรัน container
   ```bash
   # ขั้นตอนที่ 1 — pull image กลับมา
docker pull nginx:alpine
...
เพื่อแยกพวกไฟล์ต่าง ๆ ที่ไม่จำเป็นการในการ bulild image ออกไปทำให้ ขนาด container มีขนาดเล็กลง 
```
---
...
1. `booking-backend:1.0` คืออะไร?
   ...
   > _คำตอบ_:  Docker Image ของ backend ที่ถูก build มาจาก Dockerfile ของ backend 

2. ถ้าต้องการให้ ทดสอบ API ด้วย `curl http://localhost` ต้องทำอย่างไร?

   > _คำตอบ_:# ขั้นตอนที่ 1 — build image
docker build -t booking-backend:1.0 ./backend

# ขั้นตอนที่ 2 — รัน container
docker run -d --name booking-backend -p 5001:5000 booking-backend:1.0
...
1. ขนาดของ `booking-frontend:1.0` เทียบกับ `booking-backend:1.0` ต่างกันอย่างไร? เพราะเหตุใด?
   ...
   > _คำตอบ_: เพราะ backend มีการดึง image ของ npm ที่เป็น run-time ทำให้มีขนาดไฟล์ที่ใหญ่กว่า front-end ที่มีแค่ไฟล์โค้ด และ nginx 

2. `location /api/` ใน nginx.conf ทำหน้าที่อะไร?

   > _คำตอบ_: location /api/ ทำหน้าที่เป็น Reverse Proxy โดยเมื่อ client ส่ง request มาที่ URL ที่ขึ้นต้นด้วย /api/ nginx จะ forward request นั้นไปยัง backend container ที่ http://backend:5000 แทน

3. ทำไม Frontend Container ถึงใช้ชื่อ `backend` ในการ Proxy ได้?

   > _คำตอบ_: Frontend container ใช้ชื่อ backend ในการ proxy ได้ เพราะทั้งสอง container อยู่ใน Docker Network เดียวกัน (booking-network) ซึ่ง Docker จะทำหน้าที่เป็น DNS อัตโนมัติ โดยแปลงชื่อ container เป็น IP address ให้เลย
---
...
1. docker-compose.yml คืออะไร มีประโยชน์อย่างไร?
   ...
   > _คำตอบ_: docker compose คือไฟล์ที่เขียนเหมือน blueprint เป็นการกำหนด space ของ image ไว้ว่าอยากได้อะไรทำให้เวลามี image เยอะ ๆ ใช้แค่ docker compose up -d ได้เลย 

2. `restart: unless-stopped` หมายความว่าอย่างไร?

   > _คำตอบ_: ให้ Docker restart container อัตโนมัติ ทุกครั้งที่ container หยุดทำงาน เช่น crash หรือเครื่อง reboot

3. Named Volume `sqlite_data` ต่างจาก Bind Mount อย่างไร? และทำไมต้องใช้กับ SQLite?

   > _คำตอบ_: ใช้ Named Volume กับ SQLite เพราะต้องการให้ ไฟล์ booking.db อยู่รอด แม้ container จะถูกลบหรือ restart และให้ Docker จัดการ path ให้อัตโนมัติโดยไม่ต้องกังวลว่า path บน host
...
> **📝 บันทึกผลการทดลอง**:
> - IP ของ frontend: `172.20.0.2/16`
> - IP ของ backend: `172.20.0.3/16`
...
1. `docker compose ps` แสดงสถานะ Service อย่างไร?
   ...
   > _คำตอบ_: แสดงสถานะของทุก service ใน compose ได้แก่ ชื่อ container, image ที่ใช้, สถานะ (Up/Exited), และ port mapping ที่ map จาก host เข้า container

2. IP Address ของแต่ละ Container ในผลจาก `docker network inspect` คืออะไร?

   > _คำตอบ_: IP ภายใน Docker Network ที่ Docker แจกให้แต่ละ container อัตโนมัติ ทำให้ container ที่อยู่ใน network เดียวกันสามารถติดต่อกันได้โดยใช้ ชื่อ container แทน IP โดยไม่ต้องพึ่ง port forwarding 

3. SQLite database file (`booking.db`) ถูกสร้างใน Path ใดภายใน Container?

   > _คำตอบ_: ไฟล์ booking.db ถูกสร้างที่ /app/data/booking.db ภายใน container และถูก mount ด้วย Named Volume sqlite_data ทำให้ข้อมูลไม่หายแม้ container จะถูกลบ
