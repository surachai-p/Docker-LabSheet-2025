...
1. docker run
ใช้สำหรับ “สร้าง + รัน Container” ใหม่จาก Image

2. -d (detached mode)
รันแบบ background (เบื้องหลัง)
ไม่ต้องค้างหน้าจอ terminal

3. --name my-nginx
ตั้งชื่อ Container ว่า my-nginx
เอาไว้เรียกใช้ง่าย เช่น docker stop my-nginx

4. -p 8080:80
map port ระหว่างเครื่องเรา กับ Container
8080 (เครื่องเรา) → 80 (ใน container)

เข้าเว็บผ่าน http://localhost:8080
จะไปที่ Nginx ใน container


---5. nginx:alpine
คือ Image ที่ใช้
nginx = web server
alpine = version เบา (เล็ก เร็ว ใช้ resource น้อย)
...
docker exec = เข้าไป “ดู/แก้ไข/สั่งงาน” ข้างใน container ได้เหมือนเครื่องจริง
```
docker exec -it my-nginx sh
            │  │
...
> **📝 บันทึกผล**: Container ID คือ: `0ea365709fa1 ` และ IP Address คือ: `172.17.0.2`
...
1. `docker ps` กับ `docker ps -a` ต่างกันอย่างไร?
   ...
   > _คำตอบ_: docker ps จะแสดงเฉพาะ container ที่กำลัง running อยู่เท่านั้น
   ส่วน docker ps -a จะแสดง container ทั้งหมด ไม่ว่าจะ running, stoppedหรือ exited

2. ขนาดของ `nginx:alpine` image คือเท่าไร?

   > _คำตอบ_: โดยประมาณจะอยู่ที่ ~20–30 MB (ขึ้นอยู่กับเวอร์ชัน) ซึ่งเล็กกว่า nginx ปกติมาก เพราะใช้ Alpine Linux

3. flag `--rm` ใน `docker run` มีประโยชน์ในกรณีใด?

   > _คำตอบ_: ใช้เพื่อลบ container อัตโนมัติเมื่อหยุดทำงาน (stop)
เหมาะกับงานชั่วคราว เช่น run script, test หรือ job ที่ไม่ต้องการเก็บ container ไว้
4. `docker exec -it` ต่างจาก `docker run -it` อย่างไร?

   > _คำตอบ_:exec = เข้า container เดิม
             run = สร้าง container ใหม่

5. ต้องการดูว่าในเครื่องมี image อะไรอยู่บ้าง จะเขียนคำสั่งอย่างไร
   ```bash
   เขียนคำสั่งที่นี่
   ```# ดูรายการ image ทั้งหมดในเครื่อง
docker images
6. ต้องการลบ image ชื่อ nginx:alpine จะต้องเขียนคำสั่งอย่างไร หากมีหลายขั้นตอนที่ต้องเขียน ให้เขียนลำดับคำสั่งให้ถูกต้อง
   ```bash
   เขียนคำสั่งที่นี่
   # ลบ image nginx:alpine (ถ้ามี container ใช้อยู่ ต้องลบก่อน)

# ดู container ทั้งหมด
docker ps -a

# ลบ container ที่ใช้ image นี้ (ตัวอย่างใช้ container_id)
...
---คือเทคนิคในการเขียน Dockerfile โดยแบ่งการ build ออกเป็นหลาย “stage” (หลายขั้นตอน) เพื่อให้ได้ image สุดท้ายที่เล็กและสะอาดขึ้น
...
อธิบายคำสั่งที่นี่
```
ตั้ง Environment Variable ใน container

ให้แอปใน container รู้ว่าไฟล์ฐานข้อมูลอยู่ที่ /app/data/booking.db

-v $(pwd)/test-data:/app/data
Mount โฟลเดอร์ จาก host เข้าไปใน container

เชื่อมต่อ ./test-data (host) กับ /app/data (container)
```bash
...
1. `booking-backend:1.0` คืออะไร?
   ...
   > _คำตอบ_: ชื่อและแท็ก (tag) ของ Docker image ที่ใช้ระบุเวอร์ชัน

2. ถ้าต้องการให้ ทดสอบ API ด้วย `curl http://localhost` ต้องทำอย่างไร?

   > _คำตอบ_:  ต้อง map พอร์ตของ container ไปยังพอร์ต 80 ของ host เพื่อให้ curl http://localhost ทำงานได้.
...
> **📝 บันทึกผล**: booking-frontend:1.0 ขนาด `34.7` MB | booking-backend:1.0 ขนาด `398` MB
...
1. ขนาดของ `booking-frontend:1.0` เทียบกับ `booking-backend:1.0` ต่างกันอย่างไร? เพราะเหตุใด?
   ...
   > _คำตอบ_: Frontend เล็กกว่า เพราะเป็น static files (nginx) ส่วน backend ใหญ่กว่าเพราะมี Node.js + dependencies

2. `location /api/` ใน nginx.conf ทำหน้าที่อะไร?

   > _คำตอบ_: เป็น reverse proxy ส่ง request ที่ขึ้นต้นด้วย /api/ ไปยัง backend container

3. ทำไม Frontend Container ถึงใช้ชื่อ `backend` ในการ Proxy ได้?

   > _คำตอบ_: เพราะอยู่ใน Docker network เดียวกัน Docker DNS จะ resolve ชื่อ container เป็น IP อัตโนมัติ
---
...
1. docker-compose.yml คืออะไร มีประโยชน์อย่างไร?
   ...
   > _คำตอบ_:  เป็นไฟล์สำหรับ กำหนดและรัน multi-container Docker applications โดยใช้คำสั่งเดียวในการจัดการ container ทั้งหมด

2. `restart: unless-stopped` หมายความว่าอย่างไร?

   > _คำตอบ_: เป็นนโยบายการ restart container อัตโนมัติเมื่อ container หยุดทำงาน โดยจะ restart ยกเว้นถูกหยุดด้วยคำสั่ง docker stop.

3. Named Volume `sqlite_data` ต่างจาก Bind Mount อย่างไร? และทำไมต้องใช้กับ SQLite?

   > _คำตอบ_:  Named volume และ bind mount ต่างกันที่การจัดการตำแหน่งเก็บข้อมูลและความสะดวกในการใช้งาน
---
...
บันทึกรูปที่นี่
```
![alt text](<สกรีนช็อต 2026-03-11 220020.png>)
---
...
1. `docker compose ps` แสดงสถานะ Service อย่างไร?
   ...
   > _คำตอบ_: แสดงตารางสถานะ service (NAME, IMAGE, SERVICE, STATUS, PORTS)

2. IP Address ของแต่ละ Container ในผลจาก `docker network inspect` คืออะไร?

   > _คำตอบ_:IP private ใน Docker network ใช้สื่อสารระหว่าง container (ควรใช้ container name แทน)

3. SQLite database file (`booking.db`) ถูกสร้างใน Path ใดภายใน Container?

   > _คำตอบ_: ตามค่า DB_PATH environment variable เช่น /app/data/booking.db
---
