...
```text
อธิบายคำสั่งที่นี่
อธิบายคำสั่ง docker run -d --name my-nginx -p 8080:80 nginx:alpine
คำสั่งนี้คือการสั่งให้ Docker สร้างและรันคอนเทนเนอร์ใหม่จาก Image โดยมีความหมายในแต่ละส่วนดังนี้:

docker run: คำสั่งหลักที่ใช้สำหรับ สร้าง (Create) และ เริ่มต้นทำงาน (Start) คอนเทนเนอร์ใหม่

-d (Detached mode): สั่งให้คอนเทนเนอร์ทำงานอยู่เบื้องหลัง (Background) โดยไม่ยึดหน้าจอ Terminal ทำให้เราสามารถพิมพ์คำสั่งอื่นต่อได้

--name my-nginx: การกำหนด ชื่อเฉพาะ ให้กับคอนเทนเนอร์ (ในที่นี้คือ my-nginx) เพื่อให้ง่ายต่อการเรียกใช้งานหรือสั่งหยุดทำงานในภายหลัง

-p 8080:80 (Port Mapping): การเชื่อมต่อพอร์ตระหว่างเครื่อง Host และ Container

8080 (ตัวหน้า): พอร์ตบนเครื่องคอมพิวเตอร์ของเรา (Host Port)

80 (ตัวหลัง): พอร์ตภายในคอนเทนเนอร์ที่โปรแกรม Nginx ใช้งาน (Container Port)

สรุปคือ เข้าใช้งานผ่านหน้าเว็บด้วย localhost:8080

nginx:alpine: ชื่อของ Image ที่นำมาใช้เป็นต้นแบบ

nginx: คือซอฟต์แวร์เว็บเซิร์ฟเวอร์

alpine: คือเวอร์ชันที่เป็น Linux ขนาดเล็กพิเศษ (Lightweight) ช่วยให้ดาวน์โหลดเร็วและประหยัดทรัพยากรเครื่อง
```
...
> **📝 บันทึกผล**: Container ID คือ: `40a0ddad69d0` และ IP Address คือ: `172.17.0.2`
...
1. `docker ps` กับ `docker ps -a` ต่างกันอย่างไร?
   ...
   > _คำตอบ_: docker ps จะโชว์เฉพาะ Container ที่ กำลังทำงานอยู่ (Running) เท่านั้น แต่ docker ps -a (All) จะโชว์ Container ทั้งหมด ที่มีอยู่ในเครื่อง ไม่ว่าจะกำลังรันอยู่หรือหยุดทำงาน (Exited) ไปแล้วก็ตาม

2. ขนาดของ `nginx:alpine` image คือเท่าไร?

   > _คำตอบ_: ประมาณ 5 MB - 10 MB (เลขดูได้จากคำสั่ง docker images ในคอลัมน์ SIZE) ซึ่งสาเหตุที่มันเล็กเพราะใช้ Alpine Linux เป็นฐานครับ

3. flag `--rm` ใน `docker run` มีประโยชน์ในกรณีใด?

   > _คำตอบ_: มีประโยชน์มากเวลาเราต้องการรัน Container ขึ้นมา ใช้งานชั่วคราว (เช่น เข้าไปเช็คเวอร์ชันโปรแกรม หรือทดลองพิมพ์คำสั่ง) เพราะพอเราเลิกใช้และ exit ออกมา Docker จะ ลบ Container นั้นทิ้งให้ทันที ทำให้เครื่องไม่รกครับ

4. `docker exec -it` ต่างจาก `docker run -it` อย่างไร?

   > _คำตอบ_: docker run -it: คือการ "สร้างตู้ใหม่" แล้วกระโดดเข้าไปข้างใน (เริ่มจากศูนย์) docker exec -it: คือการ "มุดเข้าไปในตู้เดิม" ที่กำลังรันอยู่แล้ว เพื่อสั่งงานเพิ่มหรือเข้าไปดูข้อมูลข้างใน
...
```
คือเทคนิคการเขียน Dockerfile โดยแบ่งขั้นตอนการทำงานออกเป็นหลายช่วง (Stages) เพื่อแยกส่วนการ Build (ที่ต้องใช้เครื่องมือเยอะและไฟล์ใหญ่) ออกจากส่วนการ Run (ที่ต้องการความเบาและปลอดภัย) ผลลัพธ์คือจะได้ Docker Image ที่มีขนาดเล็กที่สุด เพราะมีเฉพาะไฟล์ที่จำเป็นต้องใช้รันแอปจริงๆ เท่านั้น
```
---
...
```
-e DB_PATH=/app/data/booking.db: คือการกำหนด Environment Variable เพื่อบอกตัวแปรในโปรแกรมว่าให้เก็บไฟล์ฐานข้อมูลไว้ที่พาธไหนภายใน Container

-v $(pwd)/test-data:/app/data: คือการทำ Volume Mapping เพื่อเชื่อมโฟลเดอร์ในเครื่องเรา (test-data) เข้ากับใน Container เพื่อให้ข้อมูลไม่หายเมื่อลบคอนเทนเนอร์ (Data Persistence)
```
...
1. `booking-backend:1.0` คืออะไร?
   ...
   > _คำตอบ_: คือ Docker Image ที่เราสร้างขึ้น (Build) เพื่อเป็นต้นแบบในการรัน Backend ของระบบจอง โดยภายในจะประกอบด้วยสภาพแวดล้อม (OS), โปรแกรม Node.js, ไฟล์โค้ด server.js, และ Library ต่างๆ ที่จำเป็นต้องใช้ในการรันแอปพลิเคชัน ส่วนเลข :1.0 คือการระบุเวอร์ชัน (Tag) ของ Image นั้นๆ

2. ถ้าต้องการให้ ทดสอบ API ด้วย `curl http://localhost` ต้องทำอย่างไร?

   > _คำตอบ_: ต้องทำการ Map Port 80 ของเครื่องเรา เข้ากับ Port ของแอปพลิเคชันใน Container (เช่น Port 5000) ตอนสั่งรัน โดยใช้ Flag -p 80:5000 เนื่องจาก HTTP ปกติจะทำงานบน Port 80 เป็นค่าเริ่มต้น ทำให้เราสามารถเรียกใช้งานผ่าน localhost ได้โดยไม่ต้องระบุเลขพอร์ตตามท้าย
...
1. ขนาดของ `booking-frontend:1.0` เทียบกับ `booking-backend:1.0` ต่างกันอย่างไร? เพราะเหตุใด?
   ...
   > _คำตอบ_: ขนาดของ Frontend (92.9MB) เล็กกว่า Backend (698MB) อย่างมาก เพราะ Frontend ใช้เทคนิค Multi-stage Build ซึ่งในขั้นตอนสุดท้ายจะเหลือเพียงไฟล์ Static (HTML/JS/CSS) และ Nginx ตัวจิ๋วเท่านั้น ส่วน Backend มีขนาดใหญ่เพราะต้องรวมเอาเครื่องมือ Build Tools (เช่น Python, GCC) และ Node Modules ทั้งหมดไว้เพื่อรันระบบฐานข้อมูล

2. `location /api/` ใน nginx.conf ทำหน้าที่อะไร?

   > _คำตอบ_: ทำหน้าที่เป็น Reverse Proxy โดยจะคอยดักฟัง Request ที่ส่งมายังพาธ /api/ แล้วทำการส่งต่อ (Forward) คำสั่งนั้นไปยัง Container ของ Backend ที่พอร์ต 5000 อัตโนมัติ ช่วยให้เราสามารถเรียกใช้งาน API ผ่าน URL เดียวกันกับหน้าเว็บได้เลยโดยไม่ติดปัญหาเรื่อง CORS

3. ทำไม Frontend Container ถึงใช้ชื่อ `backend` ในการ Proxy ได้?

   > _คำตอบ_: เพราะเมื่อเราใช้ Docker Compose คอนเทนเนอร์ทั้งหมดจะถูกดึงเข้ามาอยู่ในระบบเครือข่ายเดียวกัน (Docker Network) ซึ่ง Docker จะมีระบบ Internal DNS ในตัว ทำให้คอนเทนเนอร์สามารถคุยกันผ่าน "ชื่อ Service" (เช่น backend) แทนการใช้เลข IP Address ที่เปลี่ยนแปลงได้ตลอดเวลา
...
1. docker-compose.yml คืออะไร มีประโยชน์อย่างไร?
   ...
   > _คำตอบ_: คือไฟล์สำหรับ กำหนดค่า (Configuration) และจัดการรัน Container หลายๆ ตัวพร้อมกันในคำสั่งเดียว ประโยชน์คือช่วยลดความยุ่งยากในการพิมพ์คำสั่ง docker run ยาวๆ ทีละตัว ช่วยจัดการเรื่องเครือข่าย (Network) และที่เก็บข้อมูล (Volume) ให้เชื่อมต่อกันโดยอัตโนมัติ ทำให้การ Deploy ระบบที่มีทั้ง Frontend และ Backend ทำได้รวดเร็วและแม่นยำ

2. `restart: unless-stopped` หมายความว่าอย่างไร?

   > _คำตอบ_: หมายความว่าให้ Docker พยายามรัน Container ขึ้นมาใหม่โดยอัตโนมัติ เสมอหากมันพังหรือเครื่องคอมพิวเตอร์ถูกรีสตาร์ท "ยกเว้น" กรณีเดียวคือเราเป็นคนสั่ง docker stop ด้วยตัวเองเท่านั้น คอนเทนเนอร์ถึงจะไม่พยายามรันกลับขึ้นมาใหม่ครับ

3. Named Volume `sqlite_data` ต่างจาก Bind Mount อย่างไร? และทำไมต้องใช้กับ SQLite?

   > _คำตอบ_: ความต่าง: Named Volume (เช่น sqlite_data) คือพื้นที่ที่ Docker จัดการให้เองภายในระบบ (ปลอดภัยและประสิทธิภาพสูงกว่า) ส่วน Bind Mount คือการเจาะจงโฟลเดอร์ในเครื่องเรา (เช่น $(pwd)/data)เหตุผลที่ใช้กับ SQLite: เพราะ SQLite เก็บข้อมูลทั้งหมดเป็น "ไฟล์เดียว" หากเราไม่ใช้ Volume เมื่อลบ Container ทิ้ง ข้อมูลการจองทั้งหมดจะหายไป การใช้ Named Volume จึงช่วยรักษาข้อมูลให้คงอยู่ (Data Persistence) และป้องกันปัญหาเรื่อง Permission ของไฟล์ที่มักจะเกิดกับ Bind Mount บน Windows/Mac ครับ
...
1. `docker compose ps` แสดงสถานะ Service อย่างไร?
   ...
   > _คำตอบ_: แสดงรายการคอนเทนเนอร์ทั้งหมดที่ถูกจัดการภายใต้โปรเจกต์นั้น โดยระบุชื่อคอนเทนเนอร์ (Name), คำสั่งที่ใช้รัน (Command), สถานะการทำงานปัจจุบัน (Status) เช่น Running หรือ Exited, และการจับคู่พอร์ต (Ports) ระหว่างเครื่อง Host กับ Container ทำให้เราตรวจสอบได้ทันทีว่าระบบส่วนไหนทำงานปกติหรือพังอยู่

2. IP Address ของแต่ละ Container ในผลจาก `docker network inspect` คืออะไร?

   > _คำตอบ_: คือ Internal IP Address (หมายเลขไอพีภายใน) ที่ Docker Assign ให้กับแต่ละคอนเทนเนอร์โดยเฉพาะภายใน Network วงนั้น (เช่น 172.18.0.x) เพื่อใช้ในการสื่อสารกันเองระหว่างคอนเทนเนอร์ ซึ่ง IP นี้จะเข้าถึงได้จากภายในเครื่อง Host หรือคอนเทนเนอร์ในวงเดียวกันเท่านั้น ไม่สามารถเข้าถึงจากอินเทอร์เน็ตภายนอกโดยตรงได้

3. SQLite database file (`booking.db`) ถูกสร้างใน Path ใดภายใน Container?

   > _คำตอบ_: ถูกสร้างไว้ที่พาธ /app/data/booking.db ตามที่เรากำหนดไว้ในตัวแปรสภาพแวดล้อม DB_PATH ในไฟล์ docker-compose.yml และพาธนี้ยังถูกเชื่อมต่อกับ Named Volume ที่ชื่อ sqlite_data เพื่อให้ไฟล์ฐานข้อมูลถูกเก็บรักษาไว้ถาวรแม้จะมีการลบคอนเทนเนอร์ทิ้ง
