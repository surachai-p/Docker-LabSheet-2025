...
-d             → รันแบบ background ไม่ต้องค้าง terminal
--name my-nginx → ตั้งชื่อ Container ว่า my-nginx
-p 8080:80     → Map port 8080 ของเครื่อง → port 80 ของ Container
nginx          → ชื่อ Image ที่จะใช้
:alpine        → Tag ของ Image ที่เบาและเล็ก
...
> **📝 บันทึกผล**: Container ID คือ: `e3f51b5d90e2` และ IP Address คือ: `172.17.0.2`
...
1. `docker ps` กับ `docker ps -a` ต่างกันอย่างไร?
   ...
   > _คำตอบ_: docker ps แสดงเฉพาะ Container ที่กำลังรันอยู่ docker ps -a แสดง Container ทั้งหมดรวมถึงที่หยุดทำงานแล้วด้วย

2. ขนาดของ `nginx:alpine` image คือเท่าไร?

   > _คำตอบ_: ประมาณ 50 MB ซึ่งเบากว่า nginx:latest ที่มีขนาดประมาณ 180 MB

3. flag `--rm` ใน `docker run` มีประโยชน์ในกรณีใด?

   > _คำตอบ_: ใช้เมื่อต้องการให้ Container ถูกลบอัตโนมัติทันทีหลังหยุดทำงาน เหมาะกับงานที่รันครั้งเดียวแล้วไม่ต้องการเก็บ Container      ไว้ เช่น docker run --rm nginx:alpine

4. `docker exec -it` ต่างจาก `docker run -it` อย่างไร?

   > _คำตอบ_: docker exec -it ใช้เข้าไปใน Container ที่กำลังรันอยู่แล้ว
        docker run -it ใช้สร้าง Container ใหม่แล้วเข้าไปใน Container นั้นเลย
...
6. ต้องการลบ image ชื่อ nginx:alpine จะต้องเขียนคำสั่งอย่างไร หากมีหลายขั้นตอนที่ต้องเขียน ให้เขียนลำดับคำสั่งให้ถูกต้อง
   ```bash
   ขั้นตอนที่ 1 หยุด Container ที่ใช้ image นี้อยู่ก่อน docker stop my-nginx 
   ขั้นตอนที่ 2 ลบ Container docker rm my-nginx ขั้นตอนที่ 3 ลบ Image docker rmi nginx:alpine
   ```
7. เมื่อลบ image แล้วให้ทำการ pull image กลับมาเหมือนเดิม พร้อมทำการรัน container
   ```bash
   ขั้นตอนที่ 1 Pull image กลับมา docker pull nginx:alpineขั้นตอนที่ 2 รัน Container
    docker run -d --name my-nginx -p 8080:80 nginx:alpine
   ```
---
...
```
คือเทคนิคการเขียน Dockerfile ที่แบ่งการ build ออกเป็น หลายขั้นตอน เพื่อให้ Image สุดท้ายมีขนาดเล็กที่สุด
```
---
...
```
-e ย่อมาจาก Environment Variable คือการส่งค่าตัวแปรเข้าไปใน Container -e DB_PATH=/app/data/booking.db บอกให้ Container รู้ว่า database file อยู่ที่ /app/data/booking.db
-v ย่อมาจาก Volume คือการ Mount โฟลเดอร์จากเครื่องเราเข้าไปใน Container  $(pwd)/test-data : โฟลเดอร์ในเครื่องเรา  
/app/data : โฟลเดอร์ในContainer
```
...
1. `booking-backend:1.0` คืออะไร?
   ...
   > _คำตอบ_:คือ ชื่อ:เวอร์ชัน ของ Image ที่เราสร้างเองจาก Dockerfile 

2. ถ้าต้องการให้ ทดสอบ API ด้วย `curl http://localhost` ต้องทำอย่างไร?

   > _คำตอบ_: ต้องรู้ก่อนว่า Container รันอยู่ที่ port อะไร แล้วใส่ให้ตรง 
   ดูว่า Container รันอยู่ที่ port อะไร docker ps แล้วก็ curl ตาม port ที่ Map ไว้ เช่น ถ้า Map -p 5000:3001 curl http://localhost:5000
   สั้นๆ คือ port ซ้ายของ -p คือ port ที่ใช้ curl 
...
1. ขนาดของ `booking-frontend:1.0` เทียบกับ `booking-backend:1.0` ต่างกันอย่างไร? เพราะเหตุใด?
   ...
   > _คำตอบ_: Frontend เล็กกว่า เพราะ build เสร็จแล้วได้แค่ไฟล์ static อย่าง HTML, CSS, JS แล้วให้ nginx เสิร์ฟ ไม่ต้องพก Node.js      ติดไปด้วย
   Backend ใหญ่กว่า เพราะต้องพก Node.js runtime, dependencies ใน node_modules และ packages ต่างๆ ติดไปด้วยตลอดเวลา

2. `location /api/` ใน nginx.conf ทำหน้าที่อะไร?

   > _คำตอบ_: ทำหน้าที่เป็น Proxy คือรับ request ที่ขึ้นต้นด้วย /api/ แล้วส่งต่อไปยัง Backend Container Frontend และ Backend คนละ Container กัน แต่ Browser เห็นเป็น URL เดียวกัน เพราะ nginx เป็นตัวกลางส่ง request ให้ครับ

3. ทำไม Frontend Container ถึงใช้ชื่อ `backend` ในการ Proxy ได้?

   > _คำตอบ_: เพราะอยู่ใน Network เดียวกัน ตอนรัน Container ทั้งสองใช้ --network booking-network เหมือนกัน Docker จึงให้ Container คุยกันได้โดยใช้ ชื่อ Container แทน IP
...
1. docker-compose.yml คืออะไร มีประโยชน์อย่างไร?
   ...
   > _คำตอบ_: คือไฟล์ที่ใช้กำหนดและรัน Container หลายตัวพร้อมกันในคำสั่งเดียว

2. `restart: unless-stopped` หมายความว่าอย่างไร?

   > _คำตอบ_: คือการตั้งค่าให้ Container รีสตาร์ทอัตโนมัติ ยกเว้นกรณีที่เราสั่งหยุดเอง

3. Named Volume `sqlite_data` ต่างจาก Bind Mount อย่างไร? และทำไมต้องใช้กับ SQLite?

   > _คำตอบ_: เพราะ SQLite เก็บข้อมูลทั้งหมดไว้ในไฟล์ .db ไฟล์เดียว ถ้าไม่ใช้ Volume ข้อมูลจะหายทันทีเมื่อลบ Container Named Volume จึงการันตีว่าข้อมูลจะอยู่ครบแม้ Container จะถูกลบหรือรีสตาร์ท
...
1. `docker compose ps` แสดงสถานะ Service อย่างไร?
   ...
   > _คำตอบ_:  แสดงรายการ Service ทั้งหมดใน docker-compose.yml พร้อมสถานะ
        เช่น ชื่อ Service, Status (Running/Stopped), Port ที่ใช้งาน

2. IP Address ของแต่ละ Container ในผลจาก `docker network inspect` คืออะไร?

   > _คำตอบ_: frontend: 172.18.0.3
        backend:  172.18.0.2

3. SQLite database file (`booking.db`) ถูกสร้างใน Path ใดภายใน Container?

   > _คำตอบ_: /app/data/booking.db
        (กำหนดจาก -e DB_PATH=/app/data/booking.db ตอนรัน Container)
