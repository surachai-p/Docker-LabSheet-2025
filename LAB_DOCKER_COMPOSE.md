...
> **📝 บันทึกผล**: Container ID คือ: `b5b3e01a4087a8fe3d6e2d46eb94772150b661ddb5e78fa51dafb46934989eeb` 
  และ IP Address คือ: `172.17.0.2`
...
1. `docker ps` กับ `docker ps -a` ต่างกันอย่างไร?
   ...
   > _คำตอบ_: docker ps แสดงเฉพาะ Container ที่กำลังรันอยู่ (Status = Up)
docker ps -a แสดง Container ทั้งหมด รวมถึงที่หยุดทำงานแล้ว (Status = Exited)

2. ขนาดของ `nginx:alpine` image คือเท่าไร?

   > _คำตอบ_: nginx:alpine image คือ 26.9 MB (Content Size)

3. flag `--rm` ใน `docker run` มีประโยชน์ในกรณีใด?

   > _คำตอบ_: ใช้สำหรับ ลบ Container อัตโนมัติหลังออกจาก Container เหมาะกับการทดลองหรือรันชั่วคราว ไม่ต้องเก็บ Container ไว้

4. `docker exec -it` ต่างจาก `docker run -it` อย่างไร?

   > _คำตอบ_: docker run -it = สร้าง Container ใหม่และเข้า interactive shell
docker exec -it = เข้าไปทำงาน ใน Container ที่รันอยู่แล้ว โดยไม่สร้างใหม่
...
6. ต้องการลบ image ชื่อ nginx:alpine จะต้องเขียนคำสั่งอย่างไร หากมีหลายขั้นตอนที่ต้องเขียน ให้เขียนลำดับคำสั่งให้ถูกต้อง
   ```bash
   เขียนคำสั่งที่นี่
   # หยุด Container ที่ใช้ image นี้ (ถ้ามี)
    docker ps -a
...
```
ตอบคำถามที่นี่
คือ เทคนิคในการเขียน Dockerfile ที่ใช้ หลายขั้นตอน (stages) ในการสร้าง image เพื่อลดขนาดไฟล์และแยก build environment กับ runtime environment ออกจากกัน
```
---
...
> **📝 บันทึกผล**: ขนาดของ booking-backend:1.0 คือ: `188` MB 
...
```
อธิบายคำสั่งที่นี่
-e DB_PATH=/app/data/booking.db -v $(pwd)/test-data:/app/data

กำหนด environment variable (DB_PATH) ให้ container รู้ว่าไฟล์ SQLite อยู่ที่ไหน
แชร์โฟลเดอร์ test-data บน host ไปยัง /app/data ใน container → ข้อมูลใน database ไม่หายแม้ container ถูกลบ และแก้ไขบน host → container เห็นทันที
```
...
1. `booking-backend:1.0` คืออะไร?
   ...
   > _คำตอบ_: คือ ชื่อ (repository) และ tag/version ของ Docker image ที่เราสร้างขึ้นจาก Dockerfile สำหรับรัน backend ของระบบจองห้องพัก

2. ถ้าต้องการให้ ทดสอบ API ด้วย `curl http://localhost` ต้องทำอย่างไร?

   > _คำตอบ_: ต้องแน่ใจว่า container รันอยู่และ port ถูก map กับ host เช่น -p 5000:5000 จากนั้นใช้คำสั่ง curl http://localhost:5000/api/health หรือ curl http://localhost:5000/api-docs เพื่อทดสอบว่า backend ตอบสนอง API ได้ปกติ
