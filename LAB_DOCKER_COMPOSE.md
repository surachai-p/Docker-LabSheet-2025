...
```
คำอธิบายส่วนประกอบของคำสั่ง docker run -d --name my-nginx -p 8080:80 nginx:alpine
1. docker run: คำสั่งหลักในการสร้างและเริ่มต้นการทำงานของ Container ใหม่
2. -d (Detached): สั่งให้ Container ทำงานอยู่เบื้องหลัง (Background mode) เพื่อให้เรายังใช้ Terminal ต่อได้
3. --name my-nginx: การกำหนดชื่อให้ Container เพื่อให้ง่ายต่อการเรียกใช้งานหรือจัดการในภายหลัง
4. -p 8080:80 (Port Mapping): การเชื่อมโยงพอร์ตระหว่างเครื่อง Host (8080) เข้ากับพอร์ตภายใน Container (80)
5. nginx:alpine (Image:Tag): ชื่อ Image ที่นำมาใช้งาน โดยระบุเวอร์ชัน "alpine" ซึ่งมีขนาดเล็กและประหยัดทรัพยากร
---
...
> **📝 บันทึกผล**: Container ID คือ: `54828db1c21e` และ IP Address คือ: `172.17.0.2`
...
1. `docker ps` กับ `docker ps -a` ต่างกันอย่างไร?
   ...
   > _คำตอบ_: docker ps จะแสดงเฉพาะ Container ที่กำลัง รันอยู่ (Running) เท่านั้น ส่วน docker ps -a จะแสดง Container ทั้งหมด ที่มีอยู่ในเครื่อง รวมถึงตัวที่หยุดทำงานไปแล้ว (Exited) ด้วย
...
3. flag `--rm` ใน `docker run` มีประโยชน์ในกรณีใด?
   ...
   > _คำตอบ_: มีประโยชน์ในการสั่งให้ Docker ลบ Container ทิ้งโดยอัตโนมัติ ทันทีที่ Container นั้นหยุดทำงาน ช่วยลดการสะสมของ Container ที่ไม่ได้ใช้งานแล้วในเครื่อง เหมาะสำหรับงานที่รันชั่วคราวหรือการทดสอบ
...
```
Multi-stage Build คือ เทคนิคใน Dockerfile ที่ช่วยให้เราสามารถแบ่งการทำงานออกเป็นหลายขั้นตอน (Stages) โดยใช้คำสั่ง FROM หลายครั้งในไฟล์เดียว
หลักการทำงาน:
1. Stage แรก (Build Stage): ใช้สำหรับเตรียมสภาพแวดล้อมในการ Compile หรือ Build source code (เช่น การลง Tooling, Compilers หรือ Dependencies ต่างๆ) ซึ่งมักจะมีขนาดใหญ่
2. Stage สุดท้าย (Final Stage): จะทำการคัดลอก (Copy) เฉพาะไฟล์ที่จำเป็นจริงๆ (เช่น ไฟล์ Binary หรือไฟล์ที่ Compile เสร็จแล้ว) มาจาก Stage แรก เพื่อรันแอปพลิเคชัน
ข้อดี:
- ช่วยให้ Image ที่ได้มีขนาดเล็กมาก (Minimal Image) เพราะไม่มีไฟล์ขยะหรือ Tooling ที่ใช้แค่ตอน Build ติดไปด้วย
- เพิ่มความปลอดภัย เพราะใน Production Image จะไม่มี Source code หรือเครื่องมือที่ Hacker อาจนำไปใช้ต่อได้
- จัดการขั้นตอนการสร้างได้จบใน Dockerfile เดียว (Single Source of Truth)
```
---
...
```
คำอธิบายส่วนประกอบของคำสั่ง -e และ -v

1. -e DB_PATH=/app/data/booking.db (Environment Variable):
- เป็นการกำหนด "ตัวแปรสภาพแวดล้อม" (Environment Variable) ชื่อ DB_PATH ให้กับแอปพลิเคชันภายใน Container
- ในที่นี้เป็นการบอกแอปฯ ว่าให้ไปเรียกใช้งานไฟล์ฐานข้อมูลที่ตำแหน่ง /app/data/booking.db ภายในระบบไฟล์ของ Container
2. -v $(pwd)/test-data:/app/data (Volume Binding):
- เป็นการทำ "Bind Mount" เพื่อเชื่อมโยง (Link) โฟลเดอร์ระหว่างเครื่อง Host และ Container
- $(pwd)/test-data: คือตำแหน่งโฟลเดอร์ในเครื่องปัจจุบันของคุณ (Host Path)
- :/app/data: คือตำแหน่งโฟลเดอร์ปลายทางภายใน Container (Container Path)
- ประโยชน์: ข้อมูลที่อยู่ในโฟลเดอร์ test-data บนเครื่องเราจะถูกส่งเข้าไปปรากฏใน /app/data ของ Container ทันที และหากแอปฯ ใน Container เขียนข้อมูลใหม่ลงไป ข้อมูลนั้นก็จะถูกบันทึกกลับมาที่เครื่องเราด้วย (Data Persistence)
```
...
1. `booking-backend:1.0` คืออะไร?
   ...
   > _คำตอบ_: คือ Image Name และ Tag ที่เราต้องการนำมาสร้างเป็น Container โดย booking-backend คือชื่อของ Image (Repository) และ 1.0 คือหมายเลขเวอร์ชัน (Tag) เพื่อระบุเจาะจงว่าให้ใช้ Image ชุดที่ Build ไว้ในเวอร์ชัน 1.0

2. ถ้าต้องการให้ ทดสอบ API ด้วย `curl http://localhost` ต้องทำอย่างไร?

   > _คำตอบ_: ต้องทำการ Port Mapping พอร์ต 80 ของเครื่อง Host เข้ากับพอร์ตที่แอปพลิเคชันรันอยู่ภายใน Container (สมมติว่าเป็นพอร์ต 5000) โดยการเพิ่ม Flag -p 80:5000 ในคำสั่ง docker run เพื่อให้เราสามารถเรียกใช้งานผ่าน URL มาตรฐานของ HTTP (พอร์ต 80) ได้โดยไม่ต้องระบุเลขพอร์ตต่อท้าย
...
1. ขนาดของ `booking-frontend:1.0` เทียบกับ `booking-backend:1.0` ต่างกันอย่างไร? เพราะเหตุใด?
   ...
   > _คำตอบ_: โดยทั่วไป booking-frontend:1.0 จะมีขนาด เล็กกว่ามาก (หากใช้ Nginx:alpine) เมื่อเทียบกับ booking-backend:1.0 เพราะ Frontend มักถูก Build เป็นไฟล์ Static (HTML/JS/CSS) แล้วนำไปวางบน Web Server ขนาดจิ๋ว ส่วน Backend ต้องรวมเอา Runtime (เช่น Node.js), Dependencies (node_modules) และชุดคำสั่งประมวลผลต่างๆ ไว้ภายใน ทำให้มีขนาดใหญ่กว่า

2. `location /api/` ใน nginx.conf ทำหน้าที่อะไร?

   > _คำตอบ_: ทำหน้าที่เป็น Reverse Proxy เพื่อกำหนดเส้นทาง (Routing) ว่าหากมีการเรียกใช้งาน URL ที่ขึ้นต้นด้วย /api/ ให้ Nginx ส่งต่อ (Forward) Request นั้นไปยัง Server หรือ Container ปลายทางที่ระบุไว้ (เช่น Backend Service) แทนที่จะหาไฟล์ในเครื่องตัวเอง

3. ทำไม Frontend Container ถึงใช้ชื่อ `backend` ในการ Proxy ได้?

   > _คำตอบ_: เพราะทั้งสอง Container เชื่อมต่ออยู่ใน Docker Network (Custom Bridge) วงเดียวกัน ซึ่ง Docker มีระบบ Embedded DNS ที่ช่วยให้ Container สามารถค้นหาและสื่อสารกันได้โดยใช้ "ชื่อ Container" (Container Name) แทนการใช้ IP Address ที่เปลี่ยนแปลงบ่อย
...
1. docker-compose.yml คืออะไร มีประโยชน์อย่างไร?
   ...
   > _คำตอบ_: คือไฟล์ในรูปแบบ YAML ที่ใช้สำหรับ กำหนดและรันแอปพลิเคชันที่มีหลาย Container (Multi-container applications) พร้อมกัน มีประโยชน์คือช่วยให้เราสามารถจัดการ Infrastructure ทั้งหมด (Service, Network, Volume) ได้ด้วยคำสั่งเดียว (docker-compose up) แทนการพิมพ์คำสั่ง docker run ยาวๆ หลายครั้ง ช่วยลดความผิดพลาดและทำให้การตั้งค่าระบบเป็นมาตรฐานเดียวกัน

2. `restart: unless-stopped` หมายความว่าอย่างไร?

   > _คำตอบ_: เป็นนโยบายการเริ่มทำงานใหม่ของ Container โดย Docker จะ สั่งให้ Container รันใหม่โดยอัตโนมัติ หากมันหยุดทำงานผิดปกติ (Crash) หรือเมื่อมีการรีสตาร์ทเครื่อง Host ยกเว้น ในกรณีที่ผู้ใช้เป็นคนสั่งหยุด Container นั้นด้วยตัวเอง (ด้วยคำสั่ง docker stop) Docker จะไม่พยายามรันมันขึ้นมาใหม่จนกว่าเราจะสั่ง Start อีกครั้ง

3. Named Volume `sqlite_data` ต่างจาก Bind Mount อย่างไร? และทำไมต้องใช้กับ SQLite?

   > _คำตอบ_: Named Volume คือพื้นที่จัดเก็บข้อมูลที่ Docker จัดการให้โดยเฉพาะในพื้นที่ส่วนตัวของ Docker เอง (Docker internal area) ต่างจาก Bind Mount ที่เราต้องระบุ Path ในเครื่องเราเอง ข้อดีคือมีความปลอดภัยและประสิทธิภาพสูงกว่าในการเขียนอ่านข้อมูล ส่วนเหตุผลที่ต้องใช้กับ SQLite เพราะ SQLite เป็นฐานข้อมูลแบบไฟล์เดียวที่ต้องการความเสถียรและความเร็วในการเข้าถึงข้อมูล (I/O) สูง การใช้ Named Volume จะช่วยป้องกันปัญหาเรื่องสิทธิ์การเข้าถึงไฟล์ (Permissions) ที่มักพบใน Bind Mount บนบางระบบปฏิบัติการ
...
1. `docker compose ps` แสดงสถานะ Service อย่างไร?
   ...
   > _คำตอบ_: จะแสดงรายการ Services ทั้งหมดที่ถูกกำหนดไว้ในไฟล์ docker-compose.yml พร้อมระบุสถานะการทำงานปัจจุบัน (เช่น Running, Exited), ชื่อ Container, พอร์ตที่กำลังใช้งาน (Ports) และสถานะความพร้อมของระบบ (Health status) ทำให้เราตรวจสอบภาพรวมของโปรเจกต์ได้ง่ายกว่าการใช้ docker ps ปกติ

2. IP Address ของแต่ละ Container ในผลจาก `docker network inspect` คืออะไร?

   > _คำตอบ_: Internal IP Address ที่ Docker กำหนดให้กับแต่ละ Container ภายในวง Network นั้นๆ (เช่น 172.18.0.2) เพื่อใช้ในการสื่อสารกันเองภายในระบบ โดย IP นี้จะถูกจัดเก็บอยู่ในส่วนของ Containers ภายในผลลัพธ์ของคำสั่ง inspect

3. SQLite database file (`booking.db`) ถูกสร้างใน Path ใดภายใน Container?

   > _คำตอบ_: ถูกสร้างใน Path /app/data/booking.db (ตามที่กำหนดไว้ใน Environment Variable DB_PATH และมีการเชื่อมต่อกับ Volume เพื่อเก็บข้อมูลให้คงอยู่ถาวร)
