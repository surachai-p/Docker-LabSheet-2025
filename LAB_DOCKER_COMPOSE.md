...
> **📝 บันทึกผล**: 
![alt text](images/เช็คเวอร์ชั่น.png)
---
...
docker run คือ การสั่งให้ Docker สร้าง (Create) และเริ่มทำงาน (Start) คอนเทนเนอร์ใหม่
-d คือ การสั่งให้คอนเทนเนอร์ทำงานอยู่เบื้องหลัง (Background) เพื่อให้เรายังใช้หน้าจอ Terminal ต่อได้
--name my-nginx คือ ตั้งชื่อให้คอนเทนเนอร์ว่า "my-nginx" 
-p 8080:80 คือ การเชื่อมต่อพอร์ต Host : Container ในที่นี้คือเข้าผ่านเครื่องเราที่ 8080 เพื่อไปหา Nginx ข้างในที่ 80
nginx:alpine คือ ระบุ Image ที่จะใช้ โดยใช้ nginx เวอร์ชั่น alpine
...
**บันทึกผลการรันคำสั่ง docker exec -it และ ls /usr/share/nginx/html**
   ...
   ![alt text](images/บันทึกผลการรัน.png)
</details>
...
> **📝 บันทึกผล**: Container ID คือ: `c30accc985df` และ IP Address คือ: `172.17.0.2`
...
1. `docker ps` กับ `docker ps -a` ต่างกันอย่างไร?
   ...
   > _คำตอบ_: docker ps คำสั่งนี้จะแสดงเฉพาะคอนเทนเนอร์ที่กำลังทำงานอยู่เท่านั้น ส่วน docker ps -a คำสั่งนี้จะแสดงคอนเทนเนอร์ ทุกตัวที่มีอยู่ในเครื่อง ไม่ว่าสถานะจะเป็นอย่างไรก็ตาม
...
3. flag `--rm` ใน `docker run` มีประโยชน์ในกรณีใด?
   ...
   > _คำตอบ_: การใส่ --rm จะทำให้ Docker ลบคอนเทนเนอร์ทิ้งทันทีอัตโนมัติเมื่อมันหยุดทำงาน

4. `docker exec -it` ต่างจาก `docker run -it` อย่างไร?

   > _คำตอบ_: docker run -it คำสั่งนี้ใช้เพื่อสร้างและเริ่มคอนเทนเนอร์ใหม่จาก Image ส่วน docker exec -it คำสั่งนี้ใช้เพื่อรันคำสั่งใหม่เพิ่มเติมเข้าไปในคอนเทนเนอร์ที่ กำลังทำงานอยู่ 
...
6. ต้องการลบ image ชื่อ nginx:alpine จะต้องเขียนคำสั่งอย่างไร หากมีหลายขั้นตอนที่ต้องเขียน ให้เขียนลำดับคำสั่งให้ถูกต้อง
   ```bash
   docker ps -a
   docker stop <container_id_หรือ_ชื่อคอนเทนเนอร์>
   ```
...
```
คือ การเขียน Dockerfile ที่ช่วยให้ Image มีขนาดเล็กลงมากและปลอดภัยขึ้นมาก โดยจะแบ่งการทำงานออกเป็นหลายขั้นตอนในไฟล์เดียว
```
---
...
```
1. -e DB_PATH=/app/data/booking.db เป็นการกำหนดค่าตัวแปรไว้ในคอนเทนเนอร์ เพื่อให้ Application ที่รันอยู่ข้างในดึงไปใช้งาน
2. -v $(pwd)/test-data:/app/data เป็นการทำ Bind Mount คือการเอาโฟลเดอร์บนเครื่องเราไป "แปะทับ" หรือ "เชื่อม" กับโฟลเดอร์ในคอนเทนเนอร์
```
...
**บันทึกผลการทดลอง**
![alt text](images/Hotel_booking_API.png)
...
1. `booking-backend:1.0` คืออะไร?
   ...
   > _คำตอบ_: คือ Identifier ของ Container Image ที่ข้างในบรรจุโปรแกรม Backend ของระบบจอง Booking เอาไว้ พร้อมใช้งานทันทีไม่ว่าจะเอาไปใส่ที่คอมพิวเตอร์เครื่องไหนก็ตาม

2. ถ้าต้องการให้ ทดสอบ API ด้วย `curl http://localhost` ต้องทำอย่างไร?

   > _คำตอบ_: ถ้าต้องการทดสอบ API ด้วย curl http://localhost ให้ได้ผลลัพธ์นั้น เครื่องจะต้องมี Web Server หรือ API Service กำลังรันอยู่ที่เครื่องตัวเอง ผ่าน Port 80
...
1. ขนาดของ `booking-frontend:1.0` เทียบกับ `booking-backend:1.0` ต่างกันอย่างไร? เพราะเหตุใด?
   ...
   > _คำตอบ_: เพราะ Frontend ส่วนใหญ่มักใช้ nginx:alpine ซึ่งเป็น Linux ขนาดจิ๋วที่มีเฉพาะสิ่งที่จำเป็นในการรันเว็บเซิร์ฟเวอร์เท่านั้น ขนาดเริ่มต้นจึงอยู่ที่ประมาณ 20-40MB เท่านั้น
   แต่ Backend มักจะใช้ Base Image ที่มี Runtime ของภาษานั้นๆ แบบเต็มชุด เพื่อให้รันโค้ดได้ ซึ่งมักจะพ่วงเอา Library ของระบบปฏิบัติการมาด้วยจำนวนมาก ขนาดเริ่มต้นจึงดีดไปที่ 400-600MB 

2. `location /api/` ใน nginx.conf ทำหน้าที่อะไร?

   > _คำตอบ_: location /api/ ทำหน้าที่เป็นตัวดักจับและจัดการเส้นทาง มันจะคอยตรวจดูว่า ถ้ามี Request ใดๆ ที่ส่งมายัง Server แล้ว URL ขึ้นต้นด้วย /api/ เช่น http://localhost/api/users Nginx จะทำตามคำสั่งที่ระบุไว้ในบล็อกนั้นทันที

3. ทำไม Frontend Container ถึงใช้ชื่อ `backend` ในการ Proxy ได้?

   > _คำตอบ_: เพราะ Docker มีระบบ DNS Server อยู่ภายในตัว เมื่อทำการรัน Container ด้วยการระบุชื่อ เช่น --name backend ตัว Docker จะเอาชื่อนั้นไปบันทึกคู่กับ IP Address ของ Container ไว้ในฐานข้อมูลภายในทันที
...
1. docker-compose.yml คืออะไร มีประโยชน์อย่างไร?
   ...
   > _คำตอบ_: docker-compose.yml คือ ไฟล์ที่ใช้สำหรับ Define และ Run แอปพลิเคชันที่ประกอบด้วย Docker Container หลายๆ ตัวพร้อมกัน โดยใช้ฟอร์แมตไฟล์แบบ YAML โดยประโยชน์หลักของ docker-compose.yml คือสามารถจัดการหลาย Container ได้ภายในคำสั่งเดียว ซึ่งระบบจะรันทุกอย่างให้ตามลำดับที่ถูกต้อง แทนที่จะต้องพิมพ์ docker run แยกกันสำหรับ Frontend , Backend และ Database 

2. `restart: unless-stopped` หมายความว่าอย่างไร?

   > _คำตอบ_: คือ การเริ่มใหม่ใน Docker ที่กำหนดให้ Container สามารถกลับคืนมาทำงานได้โดยอัตโนมัติในสถานการณ์ต่าง ๆ เช่น ถ้า Container หยุดเพราะ Error มันจะ Restart ใหม่ให้ทันที

3. Named Volume `sqlite_data` ต่างจาก Bind Mount อย่างไร? และทำไมต้องใช้กับ SQLite?

   > _คำตอบ_: Named Volume คือพื้นที่ที่ Docker จัดการให้โดยตรง ทำให้เขียนข้อมูล SQLite ได้เร็วและเสถียรกว่า Bind Mount ที่ต้องดึงไฟล์ผ่าน OS หลักซึ่งมักเกิดปัญหา Database is Locked และที่ต้องใช้เป็น SQLite เพราะ SQLite ไม่ใช่ Service ที่ต้องรันแยกเหมือน MySQL หรือ PostgreSQL แต่มันสามารถติดตั้งและโปรแกรมเองสามารถอ่านและเขียนได้โดยตรง ทำให้ไม่ต้องเสียเวลาตั้งค่า Username , Password หรือจัดการ Port ให้ยุ่งยาก
...
1. `docker compose ps` แสดงสถานะ Service อย่างไร?
   ...
   > _คำตอบ_: คำสั่ง docker compose ps คือเครื่องมือสื่อสารหลักที่บอกว่า Container ของเราตอนนี้เป็นอย่างไรบ้าง มันจะสรุปสถานะของทุก Service ที่ระบุไว้ในไฟล์ docker-compose.yml ของโปรเจกต์นั้นๆ ออกมาเป็นตาราง

2. IP Address ของแต่ละ Container ในผลจาก `docker network inspect` คืออะไร?

   > _คำตอบ_: ในผลลัพธ์ของคำสั่ง docker network inspect ค่า IP Address ที่เห็นคือ ที่อยู่เครือข่ายจำลอง ที่ Docker สร้างขึ้นมาเพื่อให้ Container แต่ละตัวใช้คุยกันเองภายในเครือข่ายของ Docker ครับ

3. SQLite database file (`booking.db`) ถูกสร้างใน Path ใดภายใน Container?

   > _คำตอบ_: /app/booking.db
