...
``` 1️⃣ docker run
คำสั่งพื้นฐานของ Docker ใช้ สร้างและรัน container ใหม่ จาก image
ทุก container ที่เราจะรันจะเริ่มด้วย docker run
2️⃣ -d
ย่อมาจาก detached mode
หมายถึงรัน container เบื้องหลัง (background)
ถ้าไม่ใส่ -d container จะรัน ต่อหน้า terminal และ log ของ container จะแสดงทันที
3️⃣ --name my-nginx
ตั้ง ชื่อให้ container เป็น my-nginx
ทำให้เรียกใช้งาน, stop, restart, remove container ได้ง่ายขึ้น เช่น:
docker stop my-nginx
docker start my-nginx
docker rm my-nginx
ถ้าไม่ตั้งชื่อ Docker จะสร้างชื่ออัตโนมัติแบบสุ่ม เช่น happy_morse
4️⃣ -p 8080:80
เป็นการ map port ของ host กับ container
รูปแบบ: -p <host_port>:<container_port>
ในตัวอย่าง:
8080 → port ที่เครื่องเรา (host) จะเข้าถึง
80 → port ภายใน container ของ nginx
หมายความว่า ถ้าเราเปิด http://localhost:8080 → จะเข้าถึง nginx ใน container ได้

💡 หมายเหตุ:

ถ้า host port 8080 ใช้งานอยู่ ต้องเปลี่ยนเป็นพอร์ตอื่น เช่น 8081:80
ถ้า container ใช้ port ที่ซ้ำกับ container อื่น จะรันไม่สำเร็จ
5️⃣ nginx:alpine
image ที่ใช้รัน container
nginx → image หลักของ Nginx จาก Docker Hub
alpine → tag ของ image เป็นเวอร์ชัน ขนาดเล็ก (minimal)
ถ้า image ยังไม่มี Docker จะดึงจาก Docker Hub อัตโนมัติ
...
> **📝 บันทึกผล**: Container ID คือ: `ca3eeeea8faf2a5d3df85b1b9575e1ef82e4cd9520ccedc9e782a1cce5f4567e` และ IP Address คือ: `172.17.0.2`
...
1. `docker ps` กับ `docker ps -a` ต่างกันอย่างไร?
   ...
   > _คำตอบ_: 1️⃣ docker ps
แสดง เฉพาะ container ที่กำลังรันอยู่ตอนนี้ (running)
Container ที่ หยุดไปแล้ว จะไม่แสดง
ตัวอย่าง:
$ docker ps
CONTAINER ID   IMAGE        NAMES
ca3eeeea8faf   nginx:alpine my-nginx

✅ แสดงเฉพาะ container ที่กำลังทำงาน

2️⃣ docker ps -a
แสดง ทุก container ทั้งที่กำลังรัน และที่ หยุดแล้ว (stopped, exited)
จะเห็นทั้ง container เก่าที่เคยรันและ container ใหม่ที่เพิ่งสร้าง
ตัวอย่าง:
$ docker ps -a
CONTAINER ID   IMAGE        NAMES       STATUS
ca3eeeea8faf   nginx:alpine my-nginx    Up 5 minutes
b1f2d3c4e5f6   ubuntu       test-ubuntu Exited (0) 2 hours ago
STATUS จะบอกว่า container กำลังทำงานอยู่หรือหยุดแล้ว
🔹 สรุปง่าย ๆ
คำสั่ง	แสดง container
docker ps	เฉพาะ container ที่ running
docker ps -a	ทุก container ทั้ง running และ stopped

💡 เคล็ดลับ:

ถ้าจะลบ container ที่หยุดแล้ว ใช้:
docker rm <container_id>
หรือลบ ทั้งหมดที่หยุดแล้ว:
docker container prune

2. ขนาดของ `nginx:alpine` image คือเท่าไร?

   > _คำตอบ_: ขนาดของ nginx:alpine
ประมาณ 6–7 MB (เล็กมาก)
นี่เป็นเหตุผลที่ใช้ tag alpine ซึ่งเป็น เวอร์ชัน lightweight ของ Linux
จะต่างจาก nginx:latest (เวอร์ชันปกติ) ที่มีขนาด 20–25 MB ขึ้นไป

3. flag `--rm` ใน `docker run` มีประโยชน์ในกรณีใด?

   > _คำตอบ_: Flag --rm ในคำสั่ง docker run มีประโยชน์หลักคือ "ลบ Container อัตโนมัติ" ทันทีที่ Container นั้นหยุดทำงาน (Stopped) ช่วยประหยัดพื้นที่ดิสก์และจัดการความจำเพาะของ Container ที่เป็นงานชั่วคราว (Temporary/Ephemeral) ไม่ให้เหลือซากทิ้งไว้ โดยเฉพาะในกรณีการทดสอบแอปพลิเคชัน การทำงานแบบ script หรือการ run คำสั่งเพียงครั้งเดียว

4. `docker exec -it` ต่างจาก `docker run -it` อย่างไร?

   > _คำตอบ_: docker run -it
สร้าง container ใหม่ จาก image แล้วรันทันที
-i → interactive (เปิด stdin)
-t → allocate terminal (สร้าง pseudo-TTY ให้สามารถพิมพ์คำสั่งได้)
ใช้เมื่อคุณต้องการ เริ่ม container ใหม่ และเข้าไปใช้งาน shell หรือรันโปรแกรม

docker exec -it
เข้าไปใน container ที่กำลังรันอยู่แล้ว
ไม่สร้าง container ใหม่
ใช้สำหรับ รันคำสั่งหรือ shell ใน container ที่กำลังทำงาน
...
7. เมื่อลบ image แล้วให้ทำการ pull image กลับมาเหมือนเดิม พร้อมทำการรัน container
   ```bash
   เขียนคำสั่งที่นี่
   ``` # 1. ดึง image nginx:alpine จาก Docker Hub
docker pull nginx:alpine

# 2. รัน container ชื่อ my-nginx, map port 8080 ของ host ไปยัง port 80 ของ container
docker run -d --name my-nginx -p 8080:80 nginx:alpine

# 3. ตรวจสอบว่า container รันอยู่
docker ps
---
...
ตอบคำถามที่นี่
```
---  Multi-stage build คือเทคนิคใน Dockerfile ที่ใช้คำสั่ง FROM หลายครั้ง เพื่อแยกขั้นตอนการ build (เช่น compile code, ติดตั้ง dependencies) ออกจากขั้นตอนการ run จริง ช่วยลดขนาด Docker image สุดท้ายได้อย่างมหาศาล (เพราะลบ build tools ทิ้งไป) และเพิ่มความปลอดภัยโดยทิ้ง source code ไว้ข้างหลัง
...
> **📝 บันทึกผล**: ขนาดของ booking-backend:1.0 คือ: `booking-backend:1.0   83bd6bd58b7c        698MB
 ` 
---
...
อธิบายคำสั่งที่นี่
```
-e DB_PATH=/app/data/booking.db → กำหนด ตัวแปร environment ให้ container รู้ว่า database อยู่ที่ไหน
-v $(pwd)/test-data:/app/data → mount folder host → container เพื่อเก็บข้อมูลหรือ share files
...
1. `booking-backend:1.0` คืออะไร?
   ...
   > _คำตอบ_: เป็น template / blueprint ของ container
เมื่อคุณรัน docker run booking-backend:1.0 → Docker จะสร้าง container จาก image นี้
container คือ instance ของ image ที่กำลังรันอยู่บนเครื่อง

2. ถ้าต้องการให้ ทดสอบ API ด้วย `curl http://localhost` ต้องทำอย่างไร?

   > _คำตอบ_: ดู log ของ backend:
docker logs backend -f
จะเห็นบรรทัดแบบนี้:
Server running on port 3001
curl ด้วย port จริง:
curl http://localhost:3001
ถ้าเป็น REST API → ควรได้ response JSON

⚠️ วิธีนี้ใช้ได้ทันที แต่ host port ต้องตรงกับ port ที่ Node.js app listen

วิธีที่ 2: แก้ backend ให้ listen บน port 5000 (ตามที่คุณอยากใช้)
เปิดไฟล์ backend/server.js
แก้บรรทัด port:
// เปลี่ยนจาก 3001 เป็น process.env.BACKEND_PORT
const PORT = process.env.BACKEND_PORT || 5000;
app.listen(PORT, '0.0.0.0', () => {
  console.log(`Server running on port ${PORT}`);
});
build Docker image ใหม่:
docker build -t booking-backend:1.0 ./backend --progress=plain
รัน container ใหม่ map port 5000:
docker run -d --name backend \
  --network booking-network \
  -p 5000:5000 \
  -e DB_PATH=/app/data/booking.db \
  -v ${PWD}/test-data:/app/data \
  booking-backend:1.0
ทดสอบด้วย curl:
curl http://localhost:5000
ตอนนี้จะได้ response ของ API จริง ๆ
...
> **📝 บันทึกผล**: booking-frontend:1.0 ขนาด ` 450` MB | booking-backend:1.0 ขนาด `689` MB
...
1. ขนาดของ `booking-frontend:1.0` เทียบกับ `booking-backend:1.0` ต่างกันอย่างไร? เพราะเหตุใด?
   ...
   > _คำตอบ_: Backend ใหญ่กว่า frontend เพราะต้องรวม Node runtime + libraries ทั้งหมด และมักใช้ full Node image
Frontend ส่วนใหญ่เป็น ไฟล์ static หลัง build → เล็กกว่า backend
สามารถลด size ของทั้งสองฝั่งได้ด้วย multi-stage build และ alpine base image

2. `location /api/` ใน nginx.conf ทำหน้าที่อะไร?

   > _คำตอบ_: ซ่อน URL จริงของ backend → frontend ไม่ต้องรู้ port ของ backend
แก้ปัญหา CORS → Browser จะมองว่า request มาจาก server เดียว (localhost:3000)
รวม route frontend และ backend บน port เดียว → deploy ง่าย

3. ทำไม Frontend Container ถึงใช้ชื่อ `backend` ในการ Proxy ได้?

   > _คำตอบ_: Frontend container ใช้ชื่อ booking-backend ใน proxy ได้เพราะ Docker network มี DNS ภายใน ที่ map ชื่อ container → IP ให้ ทำให้ container ติดต่อกันโดยไม่ต้องรู้ IP จริง
...
1. docker-compose.yml คืออะไร มีประโยชน์อย่างไร?
   ...
   > _คำตอบ_: เป็น ไฟล์ configuration ของ Docker Compose
ใช้ กำหนด container หลายตัว, network, volume, environment variable ไว้ในไฟล์เดียว
เขียนด้วย YAML format

2. `restart: unless-stopped` หมายความว่าอย่างไร?

   > _คำตอบ_: restart: unless-stopped = ให้ container รันตลอดเวลาและ restart อัตโนมัติเมื่อ crash แต่ ไม่ restart ถ้าเรา stop ด้วยมือ

3. Named Volume `sqlite_data` ต่างจาก Bind Mount อย่างไร? และทำไมต้องใช้กับ SQLite?

   > _คำตอบ_: Named Volume คืออะไร
เป็น storage ที่ Docker จัดการเอง
Docker จะเก็บข้อมูลใน พื้นที่พิเศษของ Docker (เช่น /var/lib/docker/volumes/...)
ใน docker-compose.yml

ทำไม SQLite ต้องใช้ Volume หรือ Mount
SQLite เป็น file-based database → ข้อมูลทั้งหมดเก็บในไฟล์ .db
ถ้าไม่ใช้ volume / mount →
ข้อมูลจะอยู่ เฉพาะใน container
ถ้า container ถูกลบ → ข้อมูลหายหมด
ใช้ Named Volume:
ปลอดภัยและง่ายที่สุด → Docker จัดการ path, permission
ใช้ Bind Mount:
เหมาะถ้าต้องการ backup / ดูไฟล์ database บน host
---
...
1. `docker compose ps` แสดงสถานะ Service อย่างไร?
   ...
   > _คำตอบ_: ocker compose ps แสดง สถานะของ container/service ที่รันด้วย Docker Compose โดยบอกชื่อ, คำสั่ง, สถานะ (Up/Exited/Restarting), และ port mapping ทำให้รู้ว่าแต่ละ service ของ project รันปกติหรือไม่

2. IP Address ของแต่ละ Container ในผลจาก `docker network inspect` คืออะไร?

   > _คำตอบ_: IP Address ของแต่ละ container ใน docker network inspect คือ IP ภายใน Docker network ที่ container ใช้สื่อสารกันเอง ไม่ใช่ IP ของเครื่อง host และช่วยให้ frontend, backend ติดต่อกันโดยไม่ต้องรู้ IP จริงของเครื่อง

3. SQLite database file (`booking.db`) ถูกสร้างใน Path ใดภายใน Container?

   > _คำตอบ_: 1️⃣ Path ของ booking.db ใน container

ใน backend Dockerfile / docker-compose.yml ของ Booking App เรามักกำหนดแบบนี้:

# .env
DB_PATH=/app/data/booking.db
/app/data/booking.db คือ path ภายใน container
ตัว container จะสร้างไฟล์ SQLite ที่นี่
Code backend (Node.js/Express) ใช้ path จาก process.env.DB_PATH ในการเชื่อมต่อ SQLite
2️⃣ Volume / Mount

เพื่อให้ ข้อมูลไม่หายเมื่อ container ถูกลบ เราใช้ Named Volume:

services:
  backend:
    image: booking-backend:1.0
    volumes:
      - sqlite_data:/app/data

volumes:
  sqlite_data:
Volume sqlite_data map ไปที่ /app/data ใน container
ดังนั้น booking.db อยู่ใน volume และถูกเก็บข้าม container restart หรือ rebuild
3️⃣ ตรวจสอบจริงใน container

ถ้า container รันอยู่ สามารถเช็คไฟล์ได้:

docker exec -it booking-backend ls -l /app/data

ตัวอย่าง output:

total 4
-rw-r--r-- 1 root root 20480 Apr  2 13:00 booking.db
แสดงว่า booking.db อยู่ที่ /app/data/booking.db
