...
-p 8080:80 → map port (เข้า localhost:8080 → ไป container ที่ port 80)
nginx:alpine → ใช้ image nginx เวอร์ชันเบา (Alpine)
---
...
> **📝 บันทึกผล**: Container ID คือ: `30c7dc9a1b27` และ IP Address คือ: `172.17.0.2`
...
```
---
![alt text](images/Node.js-Container.png)

### ❓ คำถามท้ายขั้นตอนที่ 2

1. `docker ps` กับ `docker ps -a` ต่างกันอย่างไร?

   > _คำตอบ_: docker ps → ดู เฉพาะ container ที่กำลังรันอยู่
              docker ps -a → ดู container ทั้งหมด (รวมตัวที่หยุดแล้ว)

2. ขนาดของ `nginx:alpine` image คือเท่าไร?

   > _คำตอบ_: ~5–10 MB (compressed image)
เมื่อแตก (uncompressed) จะอยู่ประมาณ ~20–30 MB

3. flag `--rm` ใน `docker run` มีประโยชน์ในกรณีใด?

   > _คำตอบ_: ใช้ --rm เมื่อ “ไม่ต้องการเก็บ container หลังใช้งาน”

4. `docker exec -it` ต่างจาก `docker run -it` อย่างไร?

   > _คำตอบ_: docker run -it เป็นการ “สร้างและเข้าใช้งาน Container ใหม่” ส่วน docker exec -it เป็นการ “เข้าไปใช้งาน Container ที่มีอยู่แล้ว
...
```
Multi-stage Build คือเทคนิคใน Docker ที่ใช้ หลายขั้นตอน (หลาย stage) ในการ build image ภายใน Dockerfile เดียว เพื่อให้ได้ image สุดท้ายที่ เล็ก สะอาด และปลอดภัยกว่า
```
---
...
> **📝 บันทึกผล**: ขนาดของ booking-backend:1.0 คือ: `370` MB 
...
คือ กำหนดตัวแปรให้ container ว่า ไฟล์ database อยู่ที่ไหน
-v $(pwd)/test-data:/app/data
คือ เชื่อมโฟลเดอร์จากเครื่องเราเข้าไปใน container
```
...
```
---
![alt text](images/back-runing.png)

### ❓ คำถามท้ายขั้นตอนที่ 3

1. `booking-backend:1.0` คืออะไร?

   > _คำตอบ_: booking-backend:1.0 คือ ชื่อและเวอร์ชันของ Docker image สำหรับแอปพลิเคชัน backend ของระบบจอง ซึ่ง booking-backend คือชื่อแอป ส่วน 1.0 คือเวอร์ชันของ image ใช้เพื่อรันเป็น container ใน Docker ได้ง่าย ๆ โดยระบุเวอร์ชันที่ต้องการ

2. ถ้าต้องการให้ ทดสอบ API ด้วย `curl http://localhost` ต้องทำอย่างไร?

   > _คำตอบ_: 1. รัน container ให้ backend เปิดพอร์ตที่ localhost
              2. ใช้ curl เรียก URL ตามพอร์ตและ path ของ API
...
1. ขนาดของ `booking-frontend:1.0` เทียบกับ `booking-backend:1.0` ต่างกันอย่างไร? เพราะเหตุใด?
   ...
   > _คำตอบ_: ขนาดของ Docker image ทั้งสองต่างกันมาก คือ booking-frontend:1.0 มีขนาดเพียง 92.9 MB ในขณะที่ booking-backend:1.0 มีขนาดใหญ่ถึง 698 MB ความต่างนี้เกิดจากลักษณะการใช้งานของแต่ละส่วน: frontend ส่วนใหญ่เป็นไฟล์ static เช่น HTML, JavaScript, CSS และอาจรันผ่าน Nginx ทำให้ไม่ต้องรวม runtime ขนาดใหญ่ ส่วน backend ต้องรัน application logic, libraries และ runtime สำหรับเชื่อมต่อฐานข้อมูลและประมวลผลคำขอ ทำให้ต้องรวม dependencies เยอะ จึงมีขนาด image ใหญ่กว่ามาก

2. `location /api/` ใน nginx.conf ทำหน้าที่อะไร?

   > _คำตอบ_: ocation /api/ ใน nginx.conf ทำหน้าที่ จับทุก request ที่เริ่มด้วย /api/ แล้วจัดการตามคำสั่งใน block เช่นส่งต่อไป backend container เพื่อให้ frontend และ backend ทำงานร่วมกันได้

3. ทำไม Frontend Container ถึงใช้ชื่อ `backend` ในการ Proxy ได้?

   > _คำตอบ_: Frontend ใช้ชื่อ backend ใน proxy_pass ได้ เพราะ Docker network ให้ DNS ภายใน container ทำให้สามารถใช้ ชื่อ container เป็น hostname ติดต่อไปยัง container อื่น ๆ แทนการใช้ IP
---
...
1. docker-compose.yml คืออะไร มีประโยชน์อย่างไร?
   ...
   > _คำตอบ_: docker-compose.yml คือ ไฟล์กำหนดการตั้งค่า Docker Compose ซึ่งใช้สำหรับจัดการหลาย ๆ container ให้ทำงานร่วมกันในรูปแบบ multi-container application

2. `restart: unless-stopped` หมายความว่าอย่างไร?

   > _คำตอบ_: restart: unless-stopped → container จะ รีสตาร์ทอัตโนมัติถ้าเกิด crash หรือหลังเครื่อง reboot แต่จะไม่เริ่มใหม่ถ้าผู้ใช้หยุดมันเอง

3. Named Volume `sqlite_data` ต่างจาก Bind Mount อย่างไร? และทำไมต้องใช้กับ SQLite?

   > _คำตอบ_: Named Volume คือที่เก็บข้อมูลที่ Docker จัดการให้, portable และปลอดภัยกว่า
  ใช้กับ SQLite เพราะต้องการให้ไฟล์ database อยู่รอดแม้ container ถูก restart หรือลบ
  Bind Mount ใช้ได้แต่ต้องระบุ host path, OS ต่างกันอาจไม่ portable, และความปลอดภัยต่ำกว่า
...
1. `docker compose ps` แสดงสถานะ Service อย่างไร?
   ...
   > _คำตอบ_: คำสั่ง docker compose ps ใช้แสดง รายการ Service/Container ทั้งหมดใน Compose Project ปัจจุบัน โดยสรุปเป็นตารางสั้น ๆ ที่บอก ชื่อ Container, คำสั่งที่รัน, สถานะปัจจุบัน (running, exited, paused) และพอร์ตที่เชื่อมต่อกับ Host ทำให้สามารถตรวจสอบได้ว่าแต่ละ Service ทำงานอยู่หรือหยุดไปแล้วโดยไม่ต้องเข้าไปตรวจสอบ Container ทีละตัว

2. IP Address ของแต่ละ Container ในผลจาก `docker network inspect` คืออะไร?

   > _คำตอบ_: IP Address ของแต่ละ Container ที่ได้จาก docker network inspect คือ ที่อยู่ภายในเครือข่าย Docker ของ Container แต่ละตัว ใช้สำหรับให้ Container สื่อสารกันเองภายใน Network เดียวกัน ไม่ใช่ IP ของเครื่อง Host และไม่จำเป็นต้องเปิดพอร์ตเพื่อสื่อสารภายใน Network 

3. SQLite database file (`booking.db`) ถูกสร้างใน Path ใดภายใน Container?

   > _คำตอบ_: ไฟล์ SQLite (booking.db) จะถูกสร้าง ภายใน Container ตาม Path ที่แอปกำหนด และถ้าไม่ได้ทำ Volume Mapping ไฟล์จะอยู่เฉพาะใน Container เท่านั้น แต่ถ้ามี Volume Mapping ไฟล์ก็จะสะท้อนออกมาที่โฟลเดอร์บน Host
