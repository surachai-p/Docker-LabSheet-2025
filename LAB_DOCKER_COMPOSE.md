...
1. `docker ps` กับ `docker ps -a` ต่างกันอย่างไร?
   ...
   > _คำตอบ_: docker ps ใช้สำหรับแสดงรายการเฉพาะคอนเทนเนอร์ที่ กำลังทำงานอยู่ ในปัจจุบันเท่านั้น ส่วน docker ps -a จะแสดงรายการคอนเทนเนอร์ ทั้งหมด ที่มีอยู่ในระบบ ไม่ว่าจะอยู่ในสถานะกำลังทำงาน หรือหยุดทำงานแล้ว

2. ขนาดของ `nginx:alpine` image คือเท่าไร?

   > _คำตอบ_: อิมเมจรุ่น Alpine ถูกออกแบบมาให้มีขนาดเล็กมาก โดยปกติจะมีขนาดประมาณ 5 MB ถึง 40 MB ขึ้นอยู่กับเวอร์ชันเฉพาะและการปรับปรุงข้อมูล ณ ขณะนั้น 

3. flag `--rm` ใน `docker run` มีประโยชน์ในกรณีใด?

   > _คำตอบ_:ทำความสะอาดระบบโดยอัตโนมัติ โดยระบบจะลบคอนเทนเนอร์ทิ้งทันทีหลังจากที่คอนเทนเนอร์นั้นหยุดทำงาน หรือสิ้นสุดภารกิจ เหมาะสำหรับการทดสอบคำสั่งชั่วคราวเพื่อไม่ให้เกิดคอนเทนเนอร์ที่หยุดทำงานแล้วค้างอยู่ในเครื่อง

4. `docker exec -it` ต่างจาก `docker run -it` อย่างไร?

   > _คำตอบ_: docker run -it ใช้สำหรับ สร้างและเริ่มต้นคอนเทนเนอร์ใหม่ จากอิมเมจ ส่วน docker exec -it ใช้สำหรับ เข้าไปสั่งงานหรือรันกระบวนการเพิ่มเติม ในคอนเทนเนอร์ที่ กำลังทำงานอยู่แล้ว
...
7. เมื่อลบ image แล้วให้ทำการ pull image กลับมาเหมือนเดิม พร้อมทำการรัน container
   ```bash
   docker run -d --name <ชื่อคอนเทนเนอร์> nginx:alpine
   ```
---
...
```
ตMulti-stage Build เป็นเทคนิคใน Dockerfile ที่ช่วยให้เราสามารถแบ่งขั้นตอนการสร้าง Image ออกเป็นหลายๆStageได้ โดยเราสามารถคัดเลือกเอาเฉพาะไฟล์ที่จำเป็นจริงๆ จาก Stage ก่อนหน้า มาใส่ไว้ใน Stage สุดท้าย
```
---
...
```
1.การกำหนดค่า DB_PATH ด้วย -e
คือการส่งชื่อและตำแหน่งไฟล์ฐานข้อมูลจากภายนอกเข้าไปในคอนเทนเนอร์เพื่อให้โปรแกรมทำงานได้ถูกต้องตามที่เราต้องการโดยไม่ต้องแก้โค้ดข้างในซึ่งช่วยให้การตั้งค่าระบบมีความยืดหยุ่นและเปลี่ยนค่าได้ง่ายทันทีผ่านคำสั่งรัน
2.การเชื่อมโยงข้อมูลด้วย -v
คือการเชื่อมโฟลเดอร์ในเครื่องคอมพิวเตอร์ของเราเข้ากับโฟลเดอร์ในคอนเทนเนอร์เพื่อให้ข้อมูลที่โปรแกรมเขียนขึ้นถูกดึงออกมาเก็บไว้ที่เครื่องเราแบบเรียลไทม์ซึ่งช่วยป้องกันข้อมูลสูญหายแม้จะมีการลบคอนเทนเนอร์ทิ้งไปแต่ไฟล์ข้อมูลจะยังคงปลอดภัยอยู่ในเครื่องเราเสมอ
...
1. `booking-backend:1.0` คืออะไร?
   ...
   > _คำตอบ_: คือการกำหนดชื่อและเวอร์ชันให้กับ Docker Image เพื่อใช้ในการระบุตัวตนและบริหารจัดการระบบ โดยคำว่า booking-backend คือชื่อของ Repository ที่ระบุว่าเป็นส่วนงานหลังบ้านของระบบจอง และตัวเลข 1.0 คือ Tag ที่ใช้ระบุหมายเลขเวอร์ชันของซอฟต์แวร์ชุดนั้น การกำหนดชื่อในลักษณะนี้ช่วยให้ผู้พัฒนาสามารถควบคุมการอัปเดตระบบได้อย่างเป็นระเบียบ ป้องกันการเขียนทับข้อมูลเดิม และช่วยให้สามารถเลือกใช้งานหรือย้อนกลับไปยังเวอร์ชันที่ต้องการได้อย่างแม่นยำในการติดตั้งใช้งานจริง

2. ถ้าต้องการให้ ทดสอบ API ด้วย `curl http://localhost` ต้องทำอย่างไร?

   > _คำตอบ_: 1.การเชื่อมโยงพอร์ต:ต้องรันคอนเทนเนอร์โดยระบุพารามิเตอร์ -p 80:80 เพื่อจับคู่พอร์ตมาตรฐานของเครื่องโฮสต์เข้ากับพอร์ตภายในคอนเทนเนอร์ 2.สถานะการทำงาน: ตรวจสอบให้แน่ใจว่าคอนเทนเนอร์กำลังทำงานอยู่ โดยใช้คำสั่ง docker ps ก่อนเริ่มการทดสอบ 3.การส่งคำสั่ง: เปิด Terminal แล้วพิมพ์ curl http://localhost ซึ่งระบบจะส่งคำขอไปยังพอร์ต 80 ของเครื่องโดยอัตโนมัติ 4.การตรวจสอบผลลัพธ์: หากการเชื่อมต่อถูกต้อง คุณจะได้รับข้อมูลตอบกลับ (Response Body) จาก API ของคุณปรากฏบนหน้าจอ 4.ตัวเลือกเพิ่มเติม: แนะนำให้ใช้ curl -I http://localhost หากต้องการตรวจสอบเฉพาะส่วนหัว (HTTP Header) เพื่อยืนยันสถานะการตอบรับ
...
1. docker-compose.yml คืออะไร มีประโยชน์อย่างไร?
   ...
   > _คำตอบ_: คือไฟล์ในรูปแบบ YAML ที่ใช้สำหรับ กำหนดและกำหนดค่าบริการ (Services) ทั้งหมดของแอปพลิเคชันที่มีหลายคอนเทนเนอร์ มีประโยชน์ในการช่วยให้ผู้พัฒนาสามารถสั่งเริ่มทำงาน (Up) หรือหยุดทำงาน (Down) ระบบทั้งหมดได้ด้วยคำสั่งเดียว แทนการรันแยกทีละคอนเทนเนอร์ ซึ่งช่วยลดความผิดพลาดและเพิ่มความรวดเร็วในการทำงาน
2. `restart: unless-stopped` หมายความว่าอย่างไร?

   > _คำตอบ_: หมายถึงนโยบายการสั่งให้คอนเทนเนอร์ เริ่มทำงานใหม่โดยอัตโนมัติ หากเกิดข้อผิดพลาดจนเครื่องหยุดทำงาน หรือเมื่อมีการรีสตาร์ทเครื่องโฮสต์ ยกเว้นในกรณีที่ผู้ใช้งานเป็นคนสั่งหยุดการทำงานของคอนเทนเนอร์นั้นด้วยตัวเอง (Manual Stop) ระบบจะไม่สั่งรันใหม่จนกว่าจะมีการสั่งรันใหม่อีกครั้ง
3. Named Volume `sqlite_data` ต่างจาก Bind Mount อย่างไร? และทำไมต้องใช้กับ SQLite?

   > _คำตอบ_: Named Volume คือพื้นที่เก็บข้อมูลที่ Docker เป็นผู้จัดการโดยตรงในโฟลเดอร์ระบบ ซึ่งต่างจาก Bind Mount ที่เป็นการเชื่อมต่อกับโฟลเดอร์ที่ผู้ใช้ระบุบนเครื่องโดยตรงเหตุผลที่ใช้กับ SQLite: เพื่อประสิทธิภาพและความปลอดภัย เนื่องจาก Named Volume ถูกจัดการโดย Docker Engine ทำให้มีประสิทธิภาพการเขียนอ่าน (I/O) ที่เสถียรกว่า และป้องกันปัญหาเรื่อง Permission หรือการเข้าถึงไฟล์พร้อมกันจาก OS ของเครื่องโฮสต์ ซึ่งอาจทำให้ไฟล์ฐานข้อมูล SQLite เสียหายได้
