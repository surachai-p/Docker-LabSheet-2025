...
![บันทึกรูปภาพผลการตรวจสอบ version](imgl/st.png)
---
...
```text
docker run: สร้างและเริ่มใช้งาน Container

-d: ให้ทำงานเบื้องหลัง (ไม่ค้างหน้าจอ)

--name my-nginx: ตั้งชื่อ Container ว่า "my-nginx"

-p 8080:80: เข้าผ่านเครื่องเราที่พอร์ต 8080 แล้วส่งไปที่พอร์ต 80 ใน Container

nginx:alpine: ใช้ Image ของ Nginx รุ่นประหยัดพื้นที่ (Alpine)
...
> **📝 บันทึกผล**: Container ID คือ: `17454c596d65` และ IP Address คือ: `172.17.0.2`
...
1. `docker ps` กับ `docker ps -a` ต่างกันอย่างไร?
   ...
   > _docker ps: ดูเฉพาะ Container ที่ กำลังรันอยู่ 
  > docker ps -a: ดู Container ทั้งหมด(ทั้งที่รันอยู่และหยุดทำงานไปแล้ว)
...
3. flag `--rm` ใน `docker run` มีประโยชน์ในกรณีใด?
   ...
   > ใช้ ลบ Container ทิ้งอัตโนมัติ ทันทีที่มันหยุดทำงาน (เหมาะกับการรันเทสแบบครั้งเดียวทิ้ง)

4. `docker exec -it` ต่างจาก `docker run -it` อย่างไร?

   > exec -it: มุดเข้าไปสั่งงานใน Container ตัวเดิมที่รันอยู่แล้ว

    >run -it: สร้าง Container ตัวใหม่ ขึ้นมา แล้วมุดเข้าไปสั่งงาน
...
6. ต้องการลบ image ชื่อ nginx:alpine จะต้องเขียนคำสั่งอย่างไร หากมีหลายขั้นตอนที่ต้องเขียน ให้เขียนลำดับคำสั่งให้ถูกต้อง
   ```bash
   docker rm -f my-nginx (ลบ Container ที่อ้างอิง Image)
    หรือ 
    docker rmi nginx:alpine (ลบตัว Image)
   ```
...
```
-e DB_PATH=... (Environment):
เป็นการ "บอกทาง" ให้โปรแกรมใน Container รู้ว่าไฟล์ฐานข้อมูลอยู่ที่ไหน (เหมือนตั้งค่า Config)

-v $(pwd)/test-data:/app/data (Volume):
เป็นการ "เชื่อมโฟลเดอร์" ระหว่างเครื่องเรากับ Container เพื่อให้ข้อมูลไม่หายไปไหนแม้ปิดเครื่องจำลอง (Data Persistence)
```
...
1. `booking-backend:1.0` คืออะไร?
   ...
   > booking-backend: ชื่อโปรเจกต์ (Repository)
  > 1.0: เลขเวอร์ชัน (Tag) เพื่อระบุชุดของ Code ที่เรา Build ไว้

2. ถ้าต้องการให้ ทดสอบ API ด้วย `curl http://localhost` ต้องทำอย่างไร?

   > ต้องเปลี่ยน port mapping ให้ทำการ map จาก port 80 ของ host ไปยัง port 5000 ของ container backend โดยใช้คำสั่ง `docker run -d --name backend --network booking-network -p 80:5000 -e DB_PATH=/app/data/booking.db -v "$PWD/test-data:/app/data" booking-backend:1.0` เพื่อให้สามารถเข้าถึงได้ผ่าน http://localhost ตรงๆ ได้เลย
...
1. ขนาดของ `booking-frontend:1.0` เทียบกับ `booking-backend:1.0` ต่างกันอย่างไร? เพราะเหตุใด?
   ...
   > _คำตอบ_: Backend ใหญ่กว่าเพราะต้องมี Node.js Runtime และ Build Tools (Python/g++) สำหรับ SQLite ส่วน Frontend เล็กกว่าเพราะเหลือแค่ ไฟล์ Static รันบน Nginx ที่เบามาก

2. `location /api/` ใน nginx.conf ทำหน้าที่อะไร?

   > _คำตอบ_: เป็น Reverse Proxy ทำหน้าที่คอยรับ Request จากหน้าเว็บที่เรียกมายัง /api/ แล้ว "ส่งต่อ" ไปให้เครื่อง Backend จัดการต่อให้เอง

3. ทำไม Frontend Container ถึงใช้ชื่อ `backend` ในการ Proxy ได้?

   > _คำตอบ_: เพราะรันอยู่ใน Docker Network เดียวกัน ทำให้ Container สามารถคุยกันได้ผ่าน "ชื่อ Container" (แทนการใช้ IP Address) ด้วยระบบ DNS ของ Docker
---
...
1. docker-compose.yml คืออะไร มีประโยชน์อย่างไร?
   ...
   > _คำตอบ_: คือไฟล์ที่ใช้ "นิยามและรัน Container หลายตัวพร้อมกัน"

    > _ประโยชน์_: ช่วยให้ไม่ต้องพิมพ์คำสั่ง docker run ยาวๆ ทีละตัว แค่ใช้ docker-compose up ครั้งเดียว ก็เริ่มทำงานได้ทั้งระบบ (Frontend + Backend + DB)

2. `restart: unless-stopped` หมายความว่าอย่างไร?

   > _คำตอบ_: คือการสั่งให้ Container "เปิดตัวเองใหม่โดยอัตโนมัติ" หากโปรแกรมค้าง (Crash) หรือรีสตาร์ทเครื่องคอมพิวเตอร์ ยกเว้นกรณีเดียวคือเราเป็นคนสั่ง stop มันเองด้วยมือ

3. Named Volume `sqlite_data` ต่างจาก Bind Mount อย่างไร? และทำไมต้องใช้กับ SQLite?

   > _คำตอบ_: ความต่าง: Bind Mount คือการระบุตำแหน่งโฟลเดอร์บนเครื่องเราเอง ส่วน Named Volume คือการให้ Docker สร้างพื้นที่เก็บข้อมูลถาวรไว้ในระบบของมันเอง (จัดการง่ายกว่าและเร็วขึ้น)

    > _ทำไมใช้กับ SQLite_ : เพราะ SQLite ต้องการความเร็วในการอ่าน/เขียนสูง (I/O) และป้องกันปัญหาเรื่อง File Permissions (สิทธิ์การเข้าถึงไฟล์) ที่อาจทำให้ฐานข้อมูลพังหากรันข้ามระบบปฏิบัติการ (เช่น Windows ไป Linux)
...
1. `docker compose ps` แสดงสถานะ Service อย่างไร?
   ...
   > _คำตอบ_: แสดงรายการ Container ทั้งหมดที่ถูกจัดการโดย Docker Compose ไฟล์นั้นๆ โดยระบุข้อมูลสำคัญดังนี้:

    > _NAME_: ชื่อของ Container

    > _IMAGE_: Image ที่ใช้รัน

    > _COMMAND_: คำสั่งเริ่มต้นที่รันใน Container

    > _SERVICE_: ชื่อ Service ที่ระบุในไฟล์ .yml

    > _STATUS_: สถานะการทำงาน (เช่น running, exited, restarting)

    > _PORTS_: การเชื่อมต่อ Port ระหว่างเครื่อง Host และ Container
...
3. SQLite database file (`booking.db`) ถูกสร้างใน Path ใดภายใน Container?
   ...
   > _คำตอบ_: ถูกสร้างที่ Path /app/data/booking.db
