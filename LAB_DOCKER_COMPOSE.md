...
คำสั่งพื้นฐานสำหรับสร้าง Container ใหม่ และรันมันขึ้นมาทันที-d
ย่อมาจาก detached modeหมายถึงให้ Container รันอยู่เบื้องหลัง (background)
--name my-nginxใช้ตั้งชื่อ Container
-p 8080:80ใช้สำหรับ port mapping
---
...
> **📝 บันทึกผล**: Container ID คือ: `64a3e98606aa` และ IP Address คือ: `172.17.0.2`
...
**บันทึกรูปผลการทดลอง**
```textUnable to find image 'node:20-alpine' locally
...
10.8.2
/ # node -e "console.log('Hello from Docker! Running in:', process.env.HOSTNAME)"
Hello from Docker! Running in: 732550a73e92
/ # exit
...
1. `docker ps` กับ `docker ps -a` ต่างกันอย่างไร?
   ...
   > _คำตอบ_: `docker ps` แสดงเฉพาะ container ที่กำลังรันอยู่ ส่วน `docker ps -a` แสดงทุก container ทั้งที่รันอยู่, หยุดแล้ว และ exited

2. ขนาดของ `nginx:alpine` image คือเท่าไร?

   > _คำตอบ_: ประมาณ 20-30MB (ขึ้นกับเวอร์ชัน, โดยเฉลี่ยประมาณ 22MB)

3. flag `--rm` ใน `docker run` มีประโยชน์ในกรณีใด?

   > _คำตอบ_: ใช้สำหรับ container ชั่วคราว เมื่อ container หยุดแล้วจะลบออกทันที ไม่ค้างใน `docker ps -a`

4. `docker exec -it` ต่างจาก `docker run -it` อย่างไร?

   > _คำตอบ_: `docker run -it` สร้าง container ใหม่และรันคำสั่งแบบ interactive ส่วน `docker exec -it` เข้าไปรันคำสั่งภายใน container ที่มีอยู่แล้ว
...
```
Multi-stage build คือเทคนิคใน Dockerfile ที่แบ่งการสร้าง image ออกเป็นหลาย stage (เช่น build stage และ production stage) ซึ่งช่วยให้ image สุดท้ายเล็กลง โดยคัดลอกเฉพาะไฟล์ที่ต้องใช้จริงมาใน stage ท้ายสุดและทิ้ง dependencies/เครื่องมือ build ที่ไม่จำเป็น
```
---
...
```
-e DB_PATH=/app/data/booking.db: เซ็ต environment variable `DB_PATH` ให้ backend รู้ตำแหน่งไฟล์ฐานข้อมูล SQLite ภายใน container
-v $(pwd)/test-data:/app/data: mount folder `test-data` จากเครื่อง host ไปยัง `/app/data` ใน container เพื่อให้ข้อมูล persist ผ่านการลบ container
```
S
```bash
...
1. `booking-backend:1.0` คืออะไร?
   ...
   > _คำตอบ_: เป็น Docker Image ที่สร้างจาก Dockerfile ในโฟลเดอร์ backend โดยประกอบด้วย Node.js runtime (node:20-alpine), Express.js framework, SQLite driver (better-sqlite3), และ source code ของ Backend API ที่พร้อมสำหรับรัน Container

2. ถ้าต้องการให้ ทดสอบ API ด้วย `curl http://localhost` ต้องทำอย่างไร?

   > _คำตอบ_: ต้องเปลี่ยน port mapping ให้ทำการ map จาก port 80 ของ host ไปยัง port 5000 ของ container backend โดยใช้คำสั่ง `docker run -d --name backend --network booking-network -p 80:5000 -e DB_PATH=/app/data/booking.db -v "$PWD/test-data:/app/data" booking-backend:1.0` เพื่อให้สามารถเข้าถึงได้ผ่าน http://localhost ตรงๆ ได้เลย
...
> **📝 บันทึกผล**: booking-frontend:1.0 ขนาด `26.1MB` MB | booking-backend:1.0 ขนาด `183MB` MB
...
1. ขนาดของ `booking-frontend:1.0` เทียบกับ `booking-backend:1.0` ต่างกันอย่างไร? เพราะเหตุใด?
   ...
   > _คำตอบ_: booking-frontend:1.0 มีขนาดเล็กกว่า booking-backend:1.0 มาก (26.1MB vs 183MB) เพราะ frontend ใช้ nginx:stable-alpine ซึ่งเป็น web server เบาๆ ที่ serve static files เท่านั้น ไม่มี Node.js runtime, npm dependencies, หรือ SQLite driver เหมือน backend

2. `location /api/` ใน nginx.conf ทำหน้าที่อะไร?

   > _คำตอบ_: location /api/ ใน nginx.conf ทำหน้าที่ proxy API requests จาก frontend ไปยัง backend container โดยส่ง request ที่ขึ้นต้นด้วย /api/ ไปยัง http://backend:5000 เพื่อให้ frontend สามารถเรียก API ได้แม้จะรันใน container แยกกัน

3. ทำไม Frontend Container ถึงใช้ชื่อ `backend` ในการ Proxy ได้?

   > _คำตอบ_: เพราะใน Docker network, containers สามารถค้นหาซึ่งกันและกันได้โดยใช้ service name ใน docker-compose.yml ซึ่งในที่นี้ backend คือชื่อ service ของ backend container ทำให้ frontend สามารถเรียก backend ได้โดยตรงผ่าน DNS resolution ใน network
...
1. docker-compose.yml คืออะไร มีประโยชน์อย่างไร?
   ...
   > _คำตอบ_: docker-compose.yml คือไฟล์ configuration ที่นิยาม multi-container application โดยใช้ YAML syntax มีประโยชน์ในการจัดการ services, networks, volumes ในที่เดียว และสามารถ start/stop/restart services ได้พร้อมกัน

2. `restart: unless-stopped` หมายความว่าอย่างไร?

   > _คำตอบ_: restart: unless-stopped หมายความว่า container จะ restart อัตโนมัติเมื่อ container หยุดทำงาน ยกเว้นกรณีที่ผู้ใช้ stop container ด้วยตนเอง

3. Named Volume `sqlite_data` ต่างจาก Bind Mount อย่างไร? และทำไมต้องใช้กับ SQLite?

   > _คำตอบ_: Named Volume จัดการโดย Docker และอยู่ที่ /var/lib/docker/volumes/ บน host, ขณะที่ Bind Mount mount โฟลเดอร์จาก host โดยตรง ใช้กับ SQLite เพราะต้องการให้ database file อยู่รอดข้าม container lifecycle และ Named Volume ปลอดภัยกว่า Bind Mount ในการจัดการสิทธิ์และ portability
...
1. `docker compose ps` แสดงสถานะ Service อย่างไร?
   ...
   > _คำตอบ_: แสดงชื่อ service, status (Up/Exited/Healthy/Unhealthy), ports mapping และ container IDs ที่ใช้งานอยู่ใน compose stack

2. IP Address ของแต่ละ Container ในผลจาก `docker network inspect` คืออะไร?

   > _คำตอบ_: frontend = `172.18.0.3/16`, backend = `172.18.0.2/16`
